...
!----------------------------
  ...
  REAL :: field1mass,field2mass,field3mass,drymass,drymasstrop
  REAL :: badco2mass,newco2mass,co2fixfactor,co2fixorig
...
!DEPENDS ON: tracer_mass
  call tracer_mass(       &
       ...
       timestep_number,-99,                 &
       field1mass,field2mass,field3mass,drymass,drymasstrop)
...
! rml 5/1/16 Test impact of only applying fixer to troposphere
!Calculate fix faxtor
!  co2fixfactor=newco2mass/badco2mass
   co2fixorig=newco2mass/badco2mass
   co2fixfactor = 1. - (badco2mass-newco2mass)*drymass/(badco2mass*drymasstrop)

!Change CO2 concentrations:
!  CO2(:,:,:)=CO2(:,:,:)*co2fixfactor
! Only fix lower atmosphere
   CO2(:,:,1:28)=CO2(:,:,1:28)*co2fixfactor

!Check tracer mass for modified CO2
!DEPENDS ON: tracer_mass
  call tracer_mass(       &
       halo_i, halo_j, offx, offy,   &
       offx, offy, 1,             &
...
! rml 4/12/15 overwrite top model level with level below to avoid 
! odd interactions between mass fixer and (lack of) transport into 
! top level. This will slightly degrade mass fix but will hopefully
! prevent top of model drifting (to low CO2) and never stabilising
  CO2(:,:,model_levels) = CO2(:,:,model_levels-1)

!Write out results to output file
if (mype == 0) then
   write(6,*) ''
   write(6,*) 'TRACER_MASSFIX RML: Re-setting CO2 mass'
   write(6,*) 'drymass, drymasstrop',drymass,drymasstrop
   write(6,*) 'Prev CO2 Mix rat: ',prevtmass(1),' CO2 emit: ', co2emitppm
   write(6,*) 'Bad CO2 Mix rat: ',badco2mass,' New CO2 Mix rat: ',newco2mass
   write(6,*) 'CO2 Fix Factor orig: ',co2fixorig,1.-(badco2mass-newco2mass)/badco2mass
   write(6,*) 'CO2 fix factor (<=level 28): ',co2fixfactor
   write(6,*) 'Mix rat for corrected (<=28) CO2: ',field1mass/drymass
endif
