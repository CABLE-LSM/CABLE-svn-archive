...
#include "argsts.h"
     ...
     &, PREV_YR_SFRAC,WOOD_HVEST_C, WOOD_HVEST_N, WOOD_HVEST_P          &
     &, WOOD_FLUX_C, WOOD_FLUX_N, WOOD_FLUX_P,WRESP_C,WRESP_N,WRESP_P   &
     &, THINNING,FNSNET,FNLEACH,FNUP,FNLOSS,FNDEP,FNFIX)
...
!      INTEGER                                                           &
     ...
     &, PREV_YR_SFRAC(land_points,NTILES)                               &
     &, WOOD_HVEST_C(land_points,NTILES,3)                              &
     &, WOOD_HVEST_N(land_points,NTILES,3)                              &
     &, WOOD_HVEST_P(land_points,NTILES,3)                              &
     &, WOOD_FLUX_C(land_points,NTILES)                                 &
     &, WOOD_FLUX_N(land_points,NTILES)                                 &
     &, WOOD_FLUX_P(land_points,NTILES)                                 &
     &, WRESP_C(land_points,NTILES,3)                                   &
     &, WRESP_N(land_points,NTILES,3)                                   &
     &, WRESP_P(land_points,NTILES,3)                                   &
     &, THINNING(land_points,NTILES)                                    &
     &, FNSNET(land_points,NTILES)					&
     &, FNLEACH(land_points,NTILES)                                     &
     &, FNUP(land_points,NTILES)                                        &
     &, FNLOSS(land_points,NTILES)                                      &
     &, FNDEP(land_points,NTILES)                                       &
     &, FNFIX(land_points,NTILES)                                       
...
!
! Previous Years Fractions for Casa-CNP (PREV_YR_SFRAC)
! ----------------------------------------------------------------------

! Items 835 Tiled PREV_YR_SFRAC
     ...
     &      ICODE,CMESSAGE)
         IF (ICODE >  0) THEN
           write(cmessage,"(a,i3.3,a)")                  &
     &       "imp_ctl  : error in set_pseudo_list(item ", vcode, "=prev_yr_sfrac)"
           goto 9999
         END IF
         PSLEVEL_OUT=0
         DO PSLEVEL=1,NTILES
           IF (PLLTILE(PSLEVEL)) THEN
              ...
              CALL FROM_LAND_POINTS (                                   &
              STASHWORK(SI(vcode,3,im_index)+(PSLEVEL_OUT-1)            &
                *row_length*rows),prev_yr_sfrac(1,PSLEVEL_OUT),         &
                land_sea_mask,row_length*rows,land_points_dum)
             END IF
          END DO
       END IF
...
!
! Nitrogen deposition (FNDEP)
! ----------------------------------------------------------------------
! Item 884 Tiled Nitrogen deposition

      vcode=884
      write(6,*) "MRD DIAG_BL 884", sf(884,3)
      IF (SF(vcode,3)) THEN
         ! DEPENDS ON: set_pseudo_list
         CALL SET_PSEUDO_LIST(NTILES,LEN_STLIST,         &
     &      STLIST(1,STINDEX(1,vcode,3,im_index)),       &
     ...
     &      ICODE,CMESSAGE)
         IF (ICODE >  0) THEN
           write(cmessage,"(a,i3.3,a)")                  &
     &       "imp_ctl  : error in set_pseudo_list(item ", vcode, "=fndep)"
           goto 9999
         END IF
         PSLEVEL_OUT=0
         DO PSLEVEL=1,NTILES
           IF (PLLTILE(PSLEVEL)) THEN
              ...
              CALL FROM_LAND_POINTS (                                   &
              STASHWORK(SI(vcode,3,im_index)+(PSLEVEL_OUT-1)            &
                *row_length*rows),fndep(1,PSLEVEL_OUT),         &
                land_sea_mask,row_length*rows,land_points_dum)
             END IF
          END DO
       END IF
...
!
! Nitrogen fixation (FNFIX)
! ----------------------------------------------------------------------
! Item 885 Tiled Nitrogen fixation

      vcode=885
      write(6,*) "MRD DIAG_BL 885", sf(885,3)
      IF (SF(vcode,3)) THEN
         ! DEPENDS ON: set_pseudo_list
         CALL SET_PSEUDO_LIST(NTILES,LEN_STLIST,         &
     &      STLIST(1,STINDEX(1,vcode,3,im_index)),       &
     ...
     &      ICODE,CMESSAGE)
         IF (ICODE >  0) THEN
           write(cmessage,"(a,i3.3,a)")                  &
     &       "imp_ctl  : error in set_pseudo_list(item ", vcode, "=fnfix)"
           goto 9999
         END IF
         PSLEVEL_OUT=0
         DO PSLEVEL=1,NTILES
           IF (PLLTILE(PSLEVEL)) THEN
              ...
              CALL FROM_LAND_POINTS (                                   &
              STASHWORK(SI(vcode,3,im_index)+(PSLEVEL_OUT-1)            &
                *row_length*rows),fnfix(1,PSLEVEL_OUT),         &
                land_sea_mask,row_length*rows,land_points_dum)
             END IF
          END DO
       END IF
...
!
! Nitrogen release from soil and litter (FNSNET)
! ----------------------------------------------------------------------
! Item 917 Tiled Nitrogen net remineralisation

      vcode=917
      write(6,*) "MRD DIAG_BL 917", sf(917,3)
      IF (SF(vcode,3)) THEN
         ! DEPENDS ON: set_pseudo_list
         CALL SET_PSEUDO_LIST(NTILES,LEN_STLIST,         &
     &      STLIST(1,STINDEX(1,vcode,3,im_index)),       &
     ...
     &      ICODE,CMESSAGE)
         IF (ICODE >  0) THEN
           write(cmessage,"(a,i3.3,a)")                  &
     &       "imp_ctl  : error in set_pseudo_list(item ", vcode, "=fnsnet)"
           goto 9999
         END IF
         PSLEVEL_OUT=0
         DO PSLEVEL=1,NTILES
           IF (PLLTILE(PSLEVEL)) THEN
              ...
              CALL FROM_LAND_POINTS (                                   &
              STASHWORK(SI(vcode,3,im_index)+(PSLEVEL_OUT-1)            &
                *row_length*rows),fnsnet(1,PSLEVEL_OUT),         &
                land_sea_mask,row_length*rows,land_points_dum)
             END IF
          END DO
       END IF
...
!
! Nitrogen leaching (FNLEACH)
! ----------------------------------------------------------------------
! Item 918 Tiled Nitrogen leaching
     ...
     &      ICODE,CMESSAGE)
         IF (ICODE >  0) THEN
           write(cmessage,"(a,i3.3,a)")                  &
     &       "imp_ctl  : error in set_pseudo_list(item ", vcode, "=fnleach)"
           goto 9999
         END IF
         PSLEVEL_OUT=0
         DO PSLEVEL=1,NTILES
           IF (PLLTILE(PSLEVEL)) THEN
              ...
              CALL FROM_LAND_POINTS (                                   &
              STASHWORK(SI(vcode,3,im_index)+(PSLEVEL_OUT-1)            &
                *row_length*rows),fnleach(1,PSLEVEL_OUT),         &
                land_sea_mask,row_length*rows,land_points_dum)
             END IF
          END DO
       END IF
...
!
! Nitrogen uptake (FNUP)
! ----------------------------------------------------------------------
! Item 919 Tiled Nitrogen uptake
     ...
     &      ICODE,CMESSAGE)
         IF (ICODE >  0) THEN
           write(cmessage,"(a,i3.3,a)")                  &
     &       "imp_ctl  : error in set_pseudo_list(item ", vcode, "=fnup)"
           goto 9999
         END IF
         PSLEVEL_OUT=0
         DO PSLEVEL=1,NTILES
           IF (PLLTILE(PSLEVEL)) THEN
              ...
              CALL FROM_LAND_POINTS (                                   &
              STASHWORK(SI(vcode,3,im_index)+(PSLEVEL_OUT-1)            &
                *row_length*rows),fnup(1,PSLEVEL_OUT),         &
                land_sea_mask,row_length*rows,land_points_dum)
             END IF
          END DO
       END IF
...
!
! Nitrogen loss (FNLOSS)
! ----------------------------------------------------------------------
! Item 920 Tiled Nitrogen net remineralisation
     ...
     &      ICODE,CMESSAGE)
         IF (ICODE >  0) THEN
           write(cmessage,"(a,i3.3,a)")                  &
     &       "imp_ctl  : error in set_pseudo_list(item ", vcode, "=fnloss)"
           goto 9999
         END IF
         PSLEVEL_OUT=0
         DO PSLEVEL=1,NTILES
           IF (PLLTILE(PSLEVEL)) THEN
              ...
              CALL FROM_LAND_POINTS (                                   &
              STASHWORK(SI(vcode,3,im_index)+(PSLEVEL_OUT-1)            &
                *row_length*rows),fnloss(1,PSLEVEL_OUT),         &
                land_sea_mask,row_length*rows,land_points_dum)
             END IF
          END DO
       END IF
...
!
! Wood Flux from Casa-CNP (WOOD_Flux)
! ----------------------------------------------------------------------

! Items 895 Tiled WOOD Flux
     ...
     &      ICODE,CMESSAGE)
         IF (ICODE >  0) THEN
           write(cmessage,"(a,i3.3,a)")&
     &       "imp_ctl  : error in set_pseudo_list(item ",vcode,"=woodflux)"
           goto 9999
         END IF
         PSLEVEL_OUT=0
         DO PSLEVEL=1,NTILES
           IF (PLLTILE(PSLEVEL)) THEN
              ...
              CALL FROM_LAND_POINTS ( &
              STASHWORK(SI(vcode,3,im_index)+(PSLEVEL_OUT-1) &
                *row_length*rows),wood_flux_c(1,PSLEVEL_OUT),&
                land_sea_mask,row_length*rows,land_points_dum)
             END IF
          END DO
       END IF
     ...
     &       "imp_ctl  : error in set_pseudo_list(item",vcode,"=woodflux)"
         ...
         DO PSLEVEL=1,NTILES
           IF (PLLTILE(PSLEVEL)) THEN
              ...
              CALL FROM_LAND_POINTS ( &
              STASHWORK(SI(vcode,3,im_index)+(PSLEVEL_OUT-1) &
                *row_length*rows),wood_flux_n(1,PSLEVEL_OUT),&
                land_sea_mask,row_length*rows,land_points_dum)
             END IF
          END DO
       END IF
     ...
     &       "imp_ctl  : error in set_pseudo_list(item ",vcode,"=woodflux)"
         ...
         DO PSLEVEL=1,NTILES
           IF (PLLTILE(PSLEVEL)) THEN
              ...
              CALL FROM_LAND_POINTS ( &
              STASHWORK(SI(vcode,3,im_index)+(PSLEVEL_OUT-1) &
                *row_length*rows),wood_flux_p(1,PSLEVEL_OUT),&
                land_sea_mask,row_length*rows,land_points_dum)
             END IF
          END DO
       END IF
...
!
! Wood Harvest from Casa-CNP (WOOD_HVEST)
! ----------------------------------------------------------------------

! Items 898-900 Tiled WOOD HVEST
     ...
     &        ICODE,CMESSAGE)
           IF (ICODE >  0) THEN
             write(cmessage,"(a,i3.3,a)")&
     &         "imp_ctl  : error in set_pseudo_list(item ", vcode, "=woodharvest)"
             goto 9999
           END IF
           PSLEVEL_OUT=0
           DO PSLEVEL=1,NTILES
             IF (PLLTILE(PSLEVEL)) THEN
                ...
                CALL FROM_LAND_POINTS (&
      &         STASHWORK(SI(vcode,3,im_index)+(PSLEVEL_OUT-1)&
      &           *row_length*rows),wood_hvest_c(1,PSLEVEL_OUT,k),&
      &           land_sea_mask,row_length*rows,land_points_dum)
               END IF
            END DO
         END IF
...
! Items 901-903 Tiled WOOD HVEST
     ...
     &         "imp_ctl  : error in set_pseudo_list(item ", vcode,"=woodharvest)"
           ...
           DO PSLEVEL=1,NTILES
             IF (PLLTILE(PSLEVEL)) THEN
                ...
                CALL FROM_LAND_POINTS (&
      &         STASHWORK(SI(vcode,3,im_index)+(PSLEVEL_OUT-1)&
      &           *row_length*rows),wood_hvest_n(1,PSLEVEL_OUT,k),&
      &           land_sea_mask,row_length*rows,land_points_dum)
               END IF
            END DO
         END IF
...
! Items 904-906 Tiled WOOD HVEST
     ...
     &         "imp_ctl  : error in set_pseudo_list(item ", vcode,"=woodharvest)"
           ...
           DO PSLEVEL=1,NTILES
             IF (PLLTILE(PSLEVEL)) THEN
                ...
                CALL FROM_LAND_POINTS (&
      &         STASHWORK(SI(vcode,3,im_index)+(PSLEVEL_OUT-1)&
      &           *row_length*rows),wood_hvest_p(1,PSLEVEL_OUT,k),&
      &           land_sea_mask,row_length*rows,land_points_dum)
               END IF
            END DO
         END IF
...
!
! Wood Respiration from Casa-CNP (WOOD_RESP)
! ----------------------------------------------------------------------

! Items 907-909 Tiled WOOD RESP
     ...
     &        ICODE,CMESSAGE)
           IF (ICODE >  0) THEN
             write(cmessage,"(a,i3.3,a)")&
     &         "imp_ctl  : error in set_pseudo_list(item ", vcode,"=woodresp)"
             goto 9999
           END IF
           PSLEVEL_OUT=0
           DO PSLEVEL=1,NTILES
             IF (PLLTILE(PSLEVEL)) THEN
                ...
                CALL FROM_LAND_POINTS (&
      &         STASHWORK(SI(vcode,3,im_index)+(PSLEVEL_OUT-1)&
      &           *row_length*rows),wresp_c(1,PSLEVEL_OUT,k),&
      &           land_sea_mask,row_length*rows,land_points_dum)
               END IF
            END DO
         END IF
     ...
     &         "imp_ctl  : error in set_pseudo_list(item ",vcode,"=woodresp)"
           ...
           DO PSLEVEL=1,NTILES
             IF (PLLTILE(PSLEVEL)) THEN
                ...
                CALL FROM_LAND_POINTS (&
      &         STASHWORK(SI(vcode,3,im_index)+(PSLEVEL_OUT-1)&
      &           *row_length*rows),wresp_n(1,PSLEVEL_OUT,k),&
      &           land_sea_mask,row_length*rows,land_points_dum)
               END IF
            END DO
         END IF
     ...
     &         "imp_ctl  : error in set_pseudo_list(item ",vcode,"=woodresp)"
           ...
           DO PSLEVEL=1,NTILES
             IF (PLLTILE(PSLEVEL)) THEN
                ...
                CALL FROM_LAND_POINTS (&
      &         STASHWORK(SI(vcode,3,im_index)+(PSLEVEL_OUT-1)&
      &           *row_length*rows),wresp_p(1,PSLEVEL_OUT,k),&
      &           land_sea_mask,row_length*rows,land_points_dum)
               END IF
            END DO
         END IF
...
!
! Thin Ratios for Casa-CNP (THINNING)
! ----------------------------------------------------------------------

! Items 916 Tiled THINNING
     ...
     &      ICODE,CMESSAGE)
         IF (ICODE >  0) THEN
           write(cmessage,"(a,i3.3,a)")                  &
     &       "imp_ctl  : error in set_pseudo_list(item ", vcode,"=thinRatio)"
           goto 9999
         END IF
         PSLEVEL_OUT=0
         DO PSLEVEL=1,NTILES
           IF (PLLTILE(PSLEVEL)) THEN
              ...
              CALL FROM_LAND_POINTS (              &
              STASHWORK(SI(vcode,3,im_index)+(PSLEVEL_OUT-1)&
                *row_length*rows),thinning(1,PSLEVEL_OUT),&
                land_sea_mask,row_length*rows,land_points_dum)
             END IF
          END DO
       END IF
