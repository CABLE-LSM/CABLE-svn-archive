...
IMPLICIT NONE
   ...
   WHERE (canopy%precis > 0.0 .and.  ssnow%isflag > 0)
      ...
      WHERE( canopy%precis > 0.0 )
         ...
         !layer 2
         sgamm = ssnow%ssdn(:,2) * Ccgsnow * ssnow%sdepth(:,2)
         osm = ssnow%smass(:,2)
         ...
         !layer 3        
         sgamm = ssnow%ssdn(:,3) * Ccgsnow * ssnow%sdepth(:,3)
         osm = ssnow%smass(:,3)
