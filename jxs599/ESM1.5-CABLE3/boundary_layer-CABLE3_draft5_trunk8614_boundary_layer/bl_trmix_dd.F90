...
! get i_year variable to calculate leap years
#include "cmaxsize.h"
#include "ctime.h"
!
! Externals
...
!CABLE_ESM1.5  
!integer :: jhi, jhj, jhk 
integer, parameter :: pools=3
real :: wresp_c(land_points,ntype,pools)
real :: wresp_c_AllPools(land_points,ntype)
real :: wresp_c_AP_AllTiles(land_points)

wresp_c_AllPools = 0.
wresp_c_AP_AllTiles = 0.

!do i=1,land_points
!   do j=1,ntype
!      do k=1,pools
!         wresp_c_AllPools(i,j) = wresp_c_AllPools(i,j) + wresp_c(i,j,k)
!      end do
!   end do
!end do
...
!do i=1,land_points
!   do j=1,ntype
!      wresp_c_AP_AllTiles(i) = wresp_c_AP_AllTiles(i) + wresp_c_AllPools(i,j)
!   end do
!end do 

do k=1,pools
   wresp_c_AllPools(:,:) = wresp_c_AllPools(:,:) + wresp_c(:,:,k)
end do

do j=1,ntype
   wresp_c_AP_AllTiles (:) = wresp_c_AP_AllTiles (:) + wresp_c_AllPools(:,j) 
end do


! convert units from gC/m2/yr to kgC/m2/s, mind leap year
IF (MOD(i_year,4) == 0 .AND. (MOD(i_year,400) == 0 .OR. MOD(i_year,100) /= 0)) THEN
   wresp_c_AP_AllTiles(:) = wresp_c_AP_AllTiles(:)/60/60/24/366/1000
ELSE
   wresp_c_AP_AllTiles(:) = wresp_c_AP_AllTiles(:)/60/60/24/365/1000
END IF

! multiply by land fractions (NO tile fractions required)
DO L=1,LAND_POINTS
   J = (LAND_INDEX(L)-1)/ROW_LENGTH + 1
   I =  LAND_INDEX(L) - (J-1)*ROW_LENGTH
   wresp_c_AP_AllTiles(L) = FLANDG(I,J)*wresp_c_AP_AllTiles(L)
ENDDO
...
!          LAND_CO2(L) = (RESP_S(L) - NPP(L)) * M_CO2 / M_CARBON
          LAND_CO2(L) = (RESP_S(L) + wresp_c_AP_AllTiles(L) - NPP(L)) * M_CO2 / M_CARBON
...
!            TRACER_FLUX1(I,J)= (RESP_S(L) - NPP(L)) * M_CO2 / M_CARBON
            TRACER_FLUX1(I,J)= (RESP_S(L) + wresp_c_AP_AllTiles(L) - NPP(L)) * M_CO2 / M_CARBON
          ENDDO
        endif
