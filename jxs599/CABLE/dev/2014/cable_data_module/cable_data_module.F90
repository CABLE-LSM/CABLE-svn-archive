...
!jhan: remove hard wired values
   integer, parameter :: ms= 6, & ! soil levels must be same as UM
                         msn = 3   
!jhan: UM used these names
   ...
   real, dimension(:,:,:), pointer, save ::           &
      SOIL_Temp_CABLE,        &  ! Soil temperature in each layer [K]
!jhan: units?
      SMCL_CABLE,             &  ! Soil Moisture in each layer []
!jhan: units?
      STHF_CABLE                 ! Frozen Moisture FRACTION in each soil layer
...
!------------------------------------------------------------------------------
   ...
   type model_dimensions 
      ...
      ! spatial dimensions
      INTEGER, POINTER ::                                                      &
         row_length, rows,    &  ! grid dimensions
         sm_levels,           &  ! # soil levels
         land_pts                ! # grid points with land 

      ! spatial dimensions
      INTEGER, POINTER ::                                                      &
         endstep,             &  ! end time step # of experiment      
         timestep_number,     &  ! current time step # of experiment
!jhan: UM used REAL, POINTER ::                                                &
         timestep_width          ! width of time step in experiment [seconds]
 
      INTEGER, POINTER ::                                                      &
         mype                    ! # of processors in model run 
      
     	REAL, DIMENSION(:), POINTER ::                                           &
         dzsoil                  ! thickness of soil levels [m]
      
! [degrees] (jhan: check this)
      REAL, DIMENSION(:,:), POINTER ::                                         &
         latitude, longitude     ! [degrees] (jhan: check this)

!jhan: mmoved to LC
      ! land type description dimensions
      INTEGER, POINTER ::                                                      &
         ntiles,              &  ! total (max.) # tiles: available 
                              &  ! PFTs + non-veg land-cover types
         npft                 &  ! total (max.) # of distinct PFTs available

   end type model_dimensions 
   
!------------------------------------------------------------------------------

   ! these are prescribed initializations that are generally not calculated
...
!------------------------------------------------------------------------------
!jhan: we can just pass these without re-direction
...
!------------------------------------------------------------------------------
   ...
   type model 
      ...
      type (land_cover) :: land 
      
      type (model_dimensions) :: mp
      type (land_cover) :: ppar
