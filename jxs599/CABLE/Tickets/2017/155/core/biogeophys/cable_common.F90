...
! This work is licensed under the C SIRO Open Source Software License
...
!        
!        
!      ALLOCATE ( vegin%rp20( mvtype ),     vegin%rpcoef( mvtype ),             &
!                 vegin%rs20( mvtype ),     vegin%wai( mvtype ),                &
!                 vegin%rootbeta( mvtype ), vegin%shelrb( mvtype ),             &
!                 vegin%vegcf( mvtype ),    vegin%frac4( mvtype ) )
!
!      ALLOCATE ( vegin%xalbnir( mvtype ), vegin%extkn( mvtype ),               &
!                 vegin%tminvj( mvtype ), vegin%tmaxvj( mvtype ),               &
!                 vegin%vbeta( mvtype ) )
!      
!      ALLOCATE ( vegin%froot( ms, mvtype  ) )
!
!      ALLOCATE ( vegin%cplant( ncp, mvtype ), vegin%ratecp( ncp, mvtype ) )
!
!      ALLOCATE ( vegin%csoil( ncs, mvtype ),  vegin%ratecs( ncs, mvtype ) )
!
!      ALLOCATE ( vegin%refl( nrb, mvtype ),   vegin%taul( nrb, mvtype ) )
!
...
!               
!            vegin%hc(jveg)       = 0.6
!            vegin%xfang(jveg)    = 0.01 
!            vegin%width(jveg)    = 0.005
!            vegin%length(jveg)   = 0.100
!            vegin%frac4(jveg)    = 0.0
!
!            vegin%refl(1,jveg) = 0.100 
!            vegin%refl(2,jveg) = 0.400 
!            vegin%refl(3,jveg) = 0.010 
!            vegin%taul(1,jveg) = 0.200 
!            vegin%taul(2,jveg) = 0.430 
!            vegin%taul(3,jveg) = 0.010 
!
!            vegin%xalbnir(jveg) = 1.0 
!            
!            vegin%wai(jveg)      = 0.0 
!            vegin%canst1(jveg)   = 0.10 
!            vegin%shelrb(jveg)   = 2.0  
!            vegin%vegcf(jveg)    = 5.00
!            vegin%extkn(jveg)    = 0.001
!            
!            vegin%vcmax(jveg)    = 40.0e-6
!            vegin%rp20(jveg)     = 1.0000
!            vegin%rpcoef(jveg)   = 0.0832
!            vegin%rs20(jveg)     = 1.0
!
!            vegin%tminvj(jveg)   = -15  
!            vegin%tmaxvj(jveg)   = -10  
!            vegin%vbeta(jveg)    = 4.0  
!            vegin%rootbeta(jveg) = 0.964
!            
!            vegin%cplant(1,jveg) = 159 
!            vegin%cplant(2,jveg) = 5000
!            vegin%cplant(3,jveg) = 500 
!            vegin%csoil(1,jveg)  = 100
!            vegin%csoil(2,jveg)  = 250
!            
!           vegin%ratecp(1,jveg) = 1.0 
!           vegin%ratecp(2,jveg) = 0.03
!           vegin%ratecp(3,jveg) = 0.14
!           vegin%ratecs(1,jveg) = 2.0
!           vegin%ratecs(2,jveg) = 0.5
!
!         END DO
!
!      
