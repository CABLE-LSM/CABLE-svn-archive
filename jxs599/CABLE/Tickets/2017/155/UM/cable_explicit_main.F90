...
19 format(  "CABLE_LSM: ", A20, " @", I8.1, " on ",I3.1  )
  ...
  call cable_explicit_driver(  row_length, rows, land_pts, ntiles,npft,        &
                              ...
                              ls_rain,     &
                              ls_snow, &
                              tl_1, qw_1, vshr_land, pstar, z1_tq, z1_uv, &
                              canopy_tile,   &
                       !for testing - install known inconsitency if use
                       !canopy_tile again instead of Fland
                       !canopy_tile,   &
                              Fland,                                       &
                              CO2_MMR, &
                              ! rml 2/7/13 pass 3d co2 through to cable if required
                              !CO2_3D,CO2_DIM_LEN,CO2_DIM_ROW,L_CO2_INTERACTIVE,   &
                              !only used to aggregate sthu
                              !sthu_tile, &
                              soil_moist_cable,                        &
                              ...
                              !would be very useful for checks - re-implement
                              !from 8.5 or 10.1
                              !LAND_MASK, &
                              !non necessary to pass
                              !FTL_TILE_CAB, FTL_CAB, &
                              FTL_TILE, FQW_TILE, &
                  !LE_TILE_CAB, LE_CAB, &
                              TSTAR_TILE,   &
                  !TSTAR_TILE_CAB, TSTAR_CAB, 
                              U_S, U_S_STD_TILE,&
