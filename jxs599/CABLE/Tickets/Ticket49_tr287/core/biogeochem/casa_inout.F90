...
!     PRINT *, 'nv10',nv10
  ...
  DO nv=1,mvtype
    ...
    casabiome%litterrate(nv,cwd)  = deltcasa/cwdage(nv)     casabiome%soilrate(nv,mic)    = deltcasa/micage(nv)
    casabiome%soilrate(nv,slow)   = deltcasa/slowage(n)
    casabiome%soilrate(nv,pass)   = deltcasa/passage(n)
    casabiome%xkleafcoldmax(nv)   = deltcasa * xxkleafoldmax(nv)
    casabiome%xkleafdrymax(nv)    = deltcasa * xxkleafrymax(nv)
