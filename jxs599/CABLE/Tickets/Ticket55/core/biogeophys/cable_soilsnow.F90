...
SUBROUTINE trimb (a, b, c, rhs, kmax)
   ...
   REAL(), DIMENSION(:,:), INTENT(INOUT)  :: rhs ! right hand side of eq
...
SUBROUTINE smoisturev (dels,ssnow,soil,veg)
   ...
   IF (nmeth <= 0) THEN
      ...
      DO k = 1, ms-1
         ...
         IF (.not. cable_user%l_new_runoff_speed) THEN
            wbl_k = MAX( 0.01_, ssnow%wb(:,k) - ssnow%wbice(:,k) )
            wbl_kp = MAX( 0.01_, ssnow%wb(:,k+1) - ssnow%wbice(:,k+1) )
         ELSE
            wbl_k = MAX( 0.001_, ssnow%wb(:,k) - ssnow%wbice(:,k) )
            wbl_kp = MAX( 0.001_, ssnow%wb(:,k+1) - ssnow%wbice(:,k+1) )
         ENDIF
         ...
         rat = delt(:,k - 1) / ( delt(:,k) + SIGN( REAL( 1.0e-20,  ),       &
               delt(:,k) ) )

         phi = MAX( 0.0_, MIN( 1.0, 2.0 * rat ),                    &
               MIN( 2.0_, rat ) ) ! 0 for -ve rat
         fluxhi = wh
         fluxlo = wbl_k

         ! scale speed to grid lengths per dels & limit speed for stability
         ! 1. OK too for stability
         speed_k = MIN( speed_k, REAL( 0.5 * soil%zse(k) / dels ,  ) )
      ...
      WHERE( ssnow%wb(:,ms) > soil%sfc(:) )
         ...
         wbl_k = MAX( 0.001_, ssnow%wb(:,ms) - ssnow%wbice(:,ms) )
         wbl_kp = MAX( 0.001_, soil%ssat(:) - ssnow%wbice(:,ms) )
      ...
      DO k = 2, ms
         ...
         ssatcurr_k = REAL( soil%ssat,  ) - ssnow%wbice(:,k)
         ...
         pwb_wbh = ( soil%hsbh * ( 1. - MIN( 2. * MIN (0.1_, MAX(           &
                  ssnow%wbice(:,k-1) / MAX( 0.01_, ssnow%wb(:,k-1) ),       &
                  ssnow%wbice(:,k)   / MAX( 0.01_, ssnow%wb(:,k) ) ) )      &
                  , 0.1_ ) ) )                                              &
   ...
   IF ( nmeth > 0 ) THEN
      ...
      DO k = 1, ms
         ...
         ssatcurr(:,k) = REAL(soil%ssat,) - ssnow%wbice(:,k)
      ...
      END IF
         ...
         DO k = 1, ms
            ssnow%wblf(:,k) = MAX( 0.0_, MIN( ssnow%wblf(:,k), 1.0 ) )
         END DO
      END IF
...
!jhan:cable.nml
   ...
   DO k = 1, ms
      xxx = REAL( soil%ssat, )
      ssnow%rnof1 = ssnow%rnof1 + REAL( MAX( ssnow%wb(:,k) - xxx, 0.0_ )  &
                    * 1000.0 )  * soil%zse(k)
...
SUBROUTINE stempv(dels, canopy, ssnow, soil)
   ...
   REAL(), DIMENSION(mp, -2:ms) ::                                          &
      at, bt, ct, rhs !
   
   REAL(), DIMENSION(mp,-2:ms+1) :: coeff
   
   REAL(), DIMENSION(mp,ms+3)    :: tmp_mat ! temp. matrix for tggsn & tgg
      ...
      DO j = 1, mp
         ...
         ELSE
            ...
            IF( direct2min) THEN
               ...
               ccnsw(j,k) = 1.5 * MAX( 1.0_, SQRT( MIN( 2.0, 0.5 *      &
                            soil%ssat(j) /                                     &
                            MIN( ew(j), 0.5_ * soil%ssat(j) ) ) ) )

            ELSE         
               
               ccnsw(j,k) = MIN( soil%cnsd(j) * EXP( exp_arg ), 1.5_ )      &
                            * MAX( 1.0_, SQRT( MIN( 2.0, 0.5 *          &
                            soil%ssat(j) /                                     &
                            MIN( ew(j), 0.5_ * soil%ssat(j) ) ) ) )
   ...
   tmp_mat(:,1:3) = REAL(ssnow%tggsn,) 
   tmp_mat(:,4:(ms+3)) = REAL(ssnow%tgg,)
...
SUBROUTINE snowl_adjust(dels, ssnow, canopy )
   ...
   WHERE( ssnow%isflag > 0 )
      ...
      WHERE( ssnow%sdepth(:,1) > ssnow%t_snwlr )
         ...
         ssnow%ssdn(:,2) = REAL( MAX( 120.0_, MIN( REAL( max_ssdn ),   &
      ...
      ELSEWHERE ! ssnow%sdepth(:,1) < ssnow%t_snwlr
         ...
         ssnow%ssdn(:,1) = REAL( MAX( 120.0_, MIN( REAL( max_ssdn),    &
...
SUBROUTINE soilfreeze(dels, soil, ssnow)
   ...
   TYPE(soil_snow_type), INTENT(INOUT)      :: ssnow
   TYPE(soil_parameter_type), INTENT(INOUT) :: soil
   REAL(), DIMENSION(mp)           :: sicefreeze
   REAL(), DIMENSION(mp)           :: sicemelt
   ...
   DO k = 1, ms
      ...
      WHERE (ssnow%tgg(:,k) < C%TFRZ &
         ...
         sicefreeze = MIN( MAX( 0.0_, ( frozen_limit * ssnow%wb(:,k) -      &
         ...
         xx = soil%css * soil%rhosoil
         ssnow%gammzz(:,k) = MAX(                                              &
             REAL((1.0 - soil%ssat) * soil%css * soil%rhosoil ,)            &
             + (ssnow%wb(:,k) - ssnow%wbice(:,k)) * REAL(cswat * rhowat,)   &
             + ssnow%wbice(:,k) * REAL(csice * rhowat * 0.9,),              &
             REAL(xx,)) * REAL( soil%zse(k))
      ...
      ELSEWHERE( ssnow%tgg(:,k) > C%TFRZ .AND. ssnow%wbice(:,k) > 0. )
         ...
         ssnow%wbice(:,k) = MAX( 0.0_, ssnow%wbice(:,k) - sicemelt          &
                            / (soil%zse(k) * 1000.0) )
         xx = soil%css * soil%rhosoil
         ssnow%gammzz(:,k) = MAX(                                              &
              REAL((1.0-soil%ssat) * soil%css * soil%rhosoil,)             &
...
SUBROUTINE remove_trans(dels, soil, ssnow, canopy, veg)
   ...
   REAL(), DIMENSION(mp,0:ms) :: diff 
   REAL(), DIMENSION(mp)      :: xx,xxd,evap_cur
   INTEGER k
...
SUBROUTINE soil_snow(dels, soil, ssnow, canopy, met, bal, veg)
   ...
   REAL(), DIMENSION(mp) :: xx,deltat,sinfil1,sinfil2,sinfil3 
   REAL                :: zsetot
   INTEGER, SAVE :: ktau =0 
   ...
   DO k = 1, ms ! for stempv
      ...
      ssnow%wblf(:,k) = MAX( 0.01_, (ssnow%wb(:,k) - ssnow%wbice(:,k)) )    &
           & / REAL(soil%ssat,)
   ...
   ssnow%wbtot = 0.0
   DO k = 1, ms
      ssnow%wbtot = ssnow%wbtot + REAL(ssnow%wb(:,k)*1000.0*soil%zse(k),)
   END DO
