...
SUBROUTINE surface_albedo(ssnow, veg, met, rad, soil, canopy,dels)
   ...
   DO b = 1, 2        
      ...
      WHERE (mask)                
         ...
         rad%cexpkbm(:,b) = real(exp(-min(rad%extkbm(:,b)*canopy%vlaiw, 20.)))
...
SUBROUTINE surface_albedosn_orig(ssnow, veg, met, soil)
...
END SUBROUTINE surface_albedosn_orig
...
SUBROUTINE surface_albedosn(ssnow, veg, met, soil,dels)
   ...
   WHERE ( ssnow%snowd > 1. .AND. .NOT. cable_runtime%um_radiation )
      ...
      ar1 = 5000. * (1. / (C%TFRZ-0.01) - 1. / tmp) ! crystal growth  (-ve)
      ar2 = 10. * ar1 ! freezing of melt water
      snr = ssnow%snowd / max (ssnow%ssdnn, 50.)
  ...
  ENDWHERE        ! snowd > 0
  ! write(*,*) EXP( ar1 ),  EXP( ar2 ) ,  ar3
  !write(*,*) 'snowage', dels, ssnow%snage
