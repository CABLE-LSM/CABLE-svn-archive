...
!ESM15!  REAL(r_2), parameter :: q10soil=1.72  ! based on Zhou et al. 2009
!ESM15!  REAL(r_2), DIMENSION(17)      :: xkoptlitter,xkoptsoil
!ESM15!  ! parameter for Q10 function
!ESM15!  data xkoptlitter/0.4, 0.4, 0.4, 0.4, 0.4, 0.4, 0.4, 0.4, 0.4, 0.4, &
!ESM15!                 0.4, 0.4, 0.4, 0.4, 0.4, 0.4, 0.4/
!ESM15!  !  change from the above on 4 aug 2011 by ypwang, calibrated by ypw using igbp soil c (CNP cycle simulation only)
!ESM15!  data xkoptsoil/0.33,0.60,0.15,0.60,0.16, &
!ESM15!                  0.40,0.30,0.20,0.20, &
!ESM15!                  0.25,1.0,0.65,0.5,2.0,0.5,1.0,1.0/
!ESM15!
...
DO nland=1,mp
       IF(casamet%iveg2(nland)/=icewater) THEN
          ...
          IF(icycle >2) THEN
             ...
             ! net mineralization
!where is the Phosphorus leaching init made appropriate to each model re fPleach, 1e^-4 
!curiously 1^-4 is the order of mag of fpLeach
    !IF(cable_runtime%esm15_dailyPhosLeach) THEN 
...
SUBROUTINE casa_Prequire(xpCnpp,Preqmin,Preqmax,PtransPtoP,veg, &
    ...
    Preqmax(:,:)       = 0.0
    PtransPtoP(:,:)    = 0.0
  !jhan: quick scan suggests esm1.5 WHERE replaced by DO/IF
    DO np=1,mp
       IF(casamet%iveg2(np)/=icewater) THEN
