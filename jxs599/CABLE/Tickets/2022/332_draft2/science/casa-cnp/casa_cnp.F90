...
SUBROUTINE casa_allocation(lalloc, veg,soil,casabiome,casaflux,casapool,casamet,phen)
...
!        leaf area
  ...
  if(cable_user%esm15_casa_allocation) casaflux%fracClabile = 0.0
  fracCallocx = 0.0
  newLAI = 0.0
  
  !LALLOC=0 is only valid CASE for online applications at present
  SELECT CASE (LALLOC)
!subroutine casa_wolf is excluded from um build
#ifndef UM_BUILD
    CASE(2)   !
  ...
  WHERE(casamet%iveg2/=icewater) 
    ...
    IF(cable_user%esm15_casa_allocation) THEN 
    ...
    IF(.NOT. cable_user%esm15_casa_allocation) THEN 
  ...
  ENDWHERE !  WHERE(casamet%iveg2/=icewater) 

    ELSE ! IF (LALLOC.NE.(3)) THEN
  ...
  casaflux%clabloss = 0.0
    ...
    IF (cable_user%CALL_climate) THEN
      ...
      WHERE(casamet%iveg2/=icewater)
        ...
        ENDWHERE !WHERE(casamet%tairk >250.0)
  ...
  WHERE(casamet%iveg2/=icewater)
    WHERE(casamet%tairk >250.0)
      WHERE(casapool%cplant(:,wood)>1.0e-6)
						...
						IF(.NOT. cable_user%esm15_casa_rplant) THEN
...
!     fromPtoL(mp,mlitter,mplant): fraction of senesced plant biomass to litter pool (fraction)
  ...
  ! using max function to avoid dividing by zero, ypw 14/may/2008
    ...
    IF( cable_user%esm15_casa_coeffplant) THEN
...
DO npt=1,mp
  ...
  IF(casamet%iveg2(npt)/=icewater) THEN
    ...
    IF( .NOT.cable_user%esm15_casa_delplant) THEN
    ...
    IF(icycle > 1) THEN
      ...
      IF( cable_user%esm15_casa_delplant) THEN
    ...
    IF(icycle > 1) THEN
      !esm15!casaflux%FluxNtolitter(npt,str) = casaflux%fromPtoL(npt,str,leaf) * casaflux%kplant(npt,leaf)  &
      !esm15!                        * casapool%cplant(npt,leaf)       * ratioNCstrfix              &     
      !esm15!                        + casaflux%fromPtoL(npt,str,froot)* casaflux%kplant(npt,froot) &
      !esm15!                        * casapool%cplant(npt,froot)      * ratioNCstrfix  
...
DO nland=1,mp
  IF(casamet%iveg2(nland)/=icewater) THEN
    ...
    IF( .NOT.cable_user%esm15_casa_delsoil) THEN
      IF(icycle > 1) THEN
   ...
   IF(icycle >2) THEN
    ...
    IF(cable_user%esm15_casa_delsoil ) THEN 
      casaflux%Pleach(nland)  =  (1.0e-4) &
                                 * max(0.0,casapool%Psoillab(nland))
    ELSE
      casaflux%Pleach(nland)  =  casaflux%fPleach(nland) * MAX(0.0,casapool%Psoillab(nland))
    ENDIF  
      DO k=1,msoil
         DO j=1,mlitter
...
! we don't need this anymore as comes in casabiome% 
...
SUBROUTINE casa_Nrequire(xnCnpp,Nreqmin,Nreqmax,NtransPtoP,veg, &
  ...
  IF(casamet%iveg2(np)/=icewater) THEN
    ...
    IF( .NOT. cable_user%esm15_casa_Nrequire) THEN
...
SUBROUTINE casa_Prequire(xpCnpp,Preqmin,Preqmax,PtransPtoP,veg, &
  ...
  Preqmax(:,:)       = 0.0
  PtransPtoP(:,:)    = 0.0
  !jhan: quick scan suggests esm15WHERE replaced by DO/IF
    DO np=1,mp
       IF(casamet%iveg2(np)/=icewater) THEN
...
!
  ...
  INTEGER i,j,k,np,nland
    ...
    IF(.NOT. cable_user%esm15_casa_cnpcycle) THEN
  ...
  ELSE  
    ...
    IF(icycle >1) THEN
      ...
      casapool%Nsoil(np,:)   = casapool%Nsoil(np,:)   &
             ...
             ! vh ! put lower bound of 1.e-3 to prevent Nsoilmin from going negative ! Ticket #108
...
SUBROUTINE casa_cnpbal(veg,casamet,casapool,casaflux,casabal)
  ...
  !C balance
   ...
   !why do these at all?
   !casabal%cplantlast  = casapool%cplant
    ! tot needed for IO? casa_inout
    IF( .NOT. cable_user%esm15_casa_cnpbal) THEN
...
SUBROUTINE casa_ndummy(casamet,casabal,casapool)
  ...
  IF( .NOT. cable_user%esm15_casa_ndummy) THEN
...
SUBROUTINE casa_pdummy(casamet,casabal,casaflux,casapool)
  ...
  IF( .NOT. cable_user%esm15_casa_Pdummy) THEN
