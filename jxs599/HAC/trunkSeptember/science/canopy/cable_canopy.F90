    ...
    DO iter = 1, NITER
       ...
       IF (cable_user%l_new_roughness_soil)                                     &
            CALL ruff_resist( veg, rough, ssnow, canopy, &
                              !H!hgt_pft, lai_pft )
                              veg%hc, canopy%vlaiw, reducedLAIdue2snow )
...
write(6,*) "SLI is not an option right now"
       ...
       CALL update_zetar(mp, sunlit_veg_mask)
...
SUBROUTINE update_zetar(mp, sunlit_veg_mask)
...
logical :: sunlit_veg_mask(mp) 
  ...
  IF (iter < NITER) THEN ! dont compute zetar on the last iter
     ...
     IF (NITER == 2) THEN
        ...
        DO j=1,mp
          IF ( sunlit_veg_mask(j) ) canopy%zetar(j,2) = 0.5 * canopy%zetar(j,2)
        ENDDO
