...
!creater dummt to avoid de-allocating for HaC1.3 - pseudo call to rad on first
!timestep
...
!    if(  first_call )  then 
         DO h = ifmp, fmp          ! over each patch in current grid
            ...
            !veg%hc(h)       = vegin%hc(veg%iveg(h)) ! Ticket #198
...
!    if( .NOT. first_call ) & 
!     deALLOCATE (                                                               &
!         vegin%canst1, vegin%dleaf,                        &
!         vegin%length, vegin%width,                        &
!         vegin%vcmax,  vegin%ejmax,                        &
!         vegin%hc, vegin%xfang,                            &
!         vegin%rp20, vegin%rpcoef,&
!         vegin%rs20, vegin%wai,&
!         vegin%rootbeta, vegin%shelrb,                     &
!         vegin%vegcf, vegin%frac4,                         &
!         vegin%xalbnir, vegin%extkn,                       &
!         vegin%tminvj, vegin%tmaxvj,                       &
!         vegin%vbeta, vegin%froot,                     &
!         vegin%cplant, vegin%csoil,              &
!         vegin%ratecp, vegin%ratecs,             &
!         vegin%refl , vegin%taul,                 &
!         vegin%a1gs, vegin%d0gs,                               &
!         vegin%alpha, vegin%convex, vegin%cfrd,          &
!         vegin%gswmin,vegin%conkc0, vegin%conko0,      &
!         vegin%ekc, vegin%eko,                                 &
!         vegin%g0, vegin%g1,                               &
!         vegin%zr, vegin%clitt, veg_desc )
if(icall>2) then
