...
USE jules_surface_types_mod, ONLY: jules_surface_types,                       &
...
!-----------------------------------------------------------------------------
CALL log_info("init_jules_surface_types", "Reading JULES_SURFACE_TYPES namelist...")

OPEN(namelist_unit, FILE=(TRIM(nml_dir) // '/' // 'jules_surface_types.nml'), &
     STATUS='old', POSITION='rewind', ACTION='read', IOSTAT = error,          &
     IOMSG = iomessage)
IF ( error /= 0 )                                                             &
  CALL log_fatal("init_jules_surface_types",                                        &
...
! Code Description:
!   Language: Fortran 90.
!   This code is written to CABLE coding standards v1.
!-----------------------------------------------------------------------------
USE io_constants, ONLY: namelist_unit
...
USE cable_surface_types_mod, ONLY: cable_surface_types,                       &
    ...
    ice, lakes, ncpft, nnpft, nnvg, barren, urban,                            &
...
IF ( lakes > 0 ) THEN
  CALL log_info("init_cable_surface_types",                                         &
                "Lakes (inland water) is type #" // TRIM(to_string(lakes)))
ELSE
  CALL log_info("init_cable_surface_types",                                         &
...
IF ( barren > 0 ) THEN
  CALL log_info("init_cable_surface_types",                                         &
                "Barren is type #" // TRIM(to_string(barren)))
ELSE
  CALL log_info("init_cable_surface_types",                                         &
...
! JULES expects the field 'soil' to be set. It is equivalent to CABLE's
! 'barren' field
soil = barren
