...
SUBROUTINE river_control(                                                     &
  ...
  smvcst_soilt, smvcwt_soilt, surf_roff,                                      &
  ...
  rivers_call,                                                                &
  !REAL, INTENT(OUT)                                                          &
  inlandout_atm_gb, inlandout_riv, riverout, riverout_rgrid,                  &
  box_outflow,                                                                &
  box_inflow, rflow, rrun                                                     &
#endif
  )
...
USE jules_rivers_mod,         ONLY: l_inland, rivers_um_trip, rivers_speed,   &
                                    rivers_meander
...
USE jules_rivers_mod,         ONLY: rivers_sto_rp, rivers_boxareas_rp,        &
                                    rivers_sto_per_m2_on_landpts,             &
                                    np_rivers, rivers_count,                  &
...
USE parallel_mod,             ONLY: is_master_task, gather_land_field, scatter_land_field
...
! Array arguments with intent(in)
REAL(KIND=real_jlslsm), INTENT(IN) :: sub_surf_roff(land_pts)
                                             ! Sub-surface runoff (kg m-2 s-1)
REAL(KIND=real_jlslsm), INTENT(IN) :: surf_roff(land_pts)
                                             ! Surface runoff (kg m-2 s-1)

! Array arguments with intent(inout)
REAL(KIND=real_jlslsm), INTENT(OUT) :: rflow(land_pts)
                                      ! River flow diagnostic on land points
REAL(KIND=real_jlslsm), INTENT(OUT) :: rrun(land_pts)
                                      ! Runoff diagnostic on land points

! Local array variables.
REAL(KIND=real_jlslsm)              :: riverout_rgrid(np_rivers),             &
...
#if defined(UM_JULES)
REAL(KIND=real_jlslsm) :: inlandout_atmos(row_length,rows)
#endif
