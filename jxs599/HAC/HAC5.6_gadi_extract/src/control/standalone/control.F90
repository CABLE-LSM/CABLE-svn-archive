...
!CABLE_LSM{i have no idea why task_id and lat/lon are not working
!USE parallel_mod, ONLY : task_id
USE cable_fFile_module, ONLY : qprint
USE jules_surface_types_mod, ONLY: npft
use model_grid_mod, ONLY : latitude
USE cbl_sinbet_mod, ONLY : sinbet
!USE model_grid_mod, ONLY : lattitude, longitude
USE jules_soil_mod, ONLY : sm_levels, dzsoil
USE cable_types_mod,      ONLY: mp_cbl => mp, nrb_cbl => nrb
USE cable_types_mod,      ONLY: L_tile_pts_cbl =>L_tile_pts
USE cable_types_mod,      ONLY : msn_cable     
USE cable_types_mod,      ONLY : SurfaceTypeID_cbl !veg%iveg (not passed?)
USE cable_types_mod,      ONLY : VegXfang
USE cable_types_mod,      ONLY : VegTaul
USE cable_types_mod,      ONLY : VegRefl

!data:   !cable progs are set here
USE allocate_cable_progs_mod, ONLY : SoilTemp_CABLE,             &
                                     SnowTemp_CABLE,                &
                                     ThreeLayerSnowFlag_CABLE,      &
                                     OneLyrSnowDensity_CABLE
!data: constants                                        
USE cable_other_constants_mod,  ONLY : z0surf_min_cbl => z0surf_min
USE cable_other_constants_mod,  ONLY : lai_thresh_cbl => lai_thresh
...
!CABLE_LSM: SW forcing 2 CABLE - but check
...
END IF
!CABLE_LSM: for testing purposes, to use the same zenith angle, CABLE's, we need the current day
curr_day_number = day_of_year(                                                &
            current_time%year, current_time%month, current_time%day,          &
            l_360, l_leap)
!current_hour =  (current_time%time/3600.) !-.25  + .5 -23 this gets it wrong b/c
!forcing starts for Loobos from 11PM on NYE 
current_hour =  (current_time%time/3600.) + .25

call qprint(7771,"/home/599/jxs599/sinbet" ) 
write (7771,* ) "%mth %day hr #day ", current_time%month, current_time%day,curr_day_number,  real(current_hour)

!Calculate the cosine of the zenith angle
IF ( l_cosz ) THEN
  !CALL zenith(cosz_ij)
  cosz_ij = sinbet(real(curr_day_number),real(52.16658),real(current_hour) ) 
!CABLE_LSM:End 
ELSE
...
ddmfx(:,:)         = 0.0
charnock_w(:,:)    = 0.0
!CABLE_LSM: check the rad here 
!Jhan:Assume at this early stage that NIR/VISis ~ 50:50
surf_down_sw_NIR(:,:) = 0.5*sw_down_ij(:,:)
surf_down_sw_VIS(:,:) = 0.5*sw_down_ij(:,:)
