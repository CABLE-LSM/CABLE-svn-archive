...
IF ( have_layers ) THEN
  ...
  !-------------------------------------------------------------------------
  ! Calculate the fraction of roots in each soil layer.
  !-------------------------------------------------------------------------
  DO n = 1,npft
...
#if !defined(UM_JULES)
  CASE ( soil_model_ecosse )
    ! Get soil temperature on soil N layers.
...
IF ( soil_bgc_model == soil_model_ecosse ) THEN
  ...
  !---------------------------------------------------------------------------
  ! Placeholder code until ECOSSE is completed.
  ! In time we will also need to remove the the minimum-allowed amount of
  ! inorganic N here.
  !---------------------------------------------------------------------------

  ! All PFTs can access the same soil N.
  DO n = 1,npft
    n_inorg_avail_pft(:,n,:) = n_amm_soilt(:,1,:) + n_nit_soilt(:,1,:)
  END DO

  n_inorg_soilt_lyrs(:,1,:)  = n_amm_soilt(:,1,:) + n_nit_soilt(:,1,:)
