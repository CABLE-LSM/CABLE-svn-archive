...
SUBROUTINE soil_n_deposition( land_pts, trif_pts, r_gamma, trif_index,        &
                              f_root_pft, n_inorg_avail_pft,                  &
                              n_inorg_soilt_lyrs, deposition )
...
IF ( soil_bgc_model == soil_model_rothc ) THEN
  ...
  DO t = 1,trif_pts
    ...
    ! The weighting by f_root_pft means generally means that only a fraction
    ! of deposition is made available to plants.
    !-------------------------------------------------------------------------
...
REAL ::                                                                       &
  n_fix_denom,                                                                &
    ! Denominator for fixation calculation with l_layeredc=T.
  scale_to_step
    ! Scaling from 360 days to the TRIFFID timestep.
...
IF ( soil_bgc_model == soil_model_rothc .OR.                                  &
  ...
  IF ( have_layers ) THEN
    ...
    IF ( soil_bgc_model == soil_model_rothc ) THEN
      ...
      ! available to plants (because of the factor f_root_pft).
      DO n = 1,npft
...
DO t = 1,trif_pts
  ...
  DO n = 1,npft
    ...
    !-------------------------------------------------------------------------
    IF ( have_layers ) THEN
      ! We don't have a gridbox variable with l_layeredC.
