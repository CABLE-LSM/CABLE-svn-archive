...
! imported scalars that are not changed
    ...
    l_scale_resp_pm, photo_model, stomata_model
...
! imported arrays that are not changed
    ...
    r_grow, sigl, glmin, lma, nmass, kn, knl, tupp, tlow,  q10_leaf, nsw,     &
    nr, hw_sw
...
,jnumerator_const                                                             &
   ! Constant part of Jmax numerator that depends only on PFT.
,kc_val                                                                       &
...
,vnumerator_const
   ! Constant part of Vcmax numerator that depends only on PFT.
...
,fsmc_scale(land_pts)                                                         &
                            ...
                            ! respiration.
,denom(land_pts)                                                              &
   ! Denominator in temperature-dependency of Vcmax.
,ccp(land_pts)                                                                &
...
CASE ( photo_farquhar )
  ...
  vnumerator_const  = 1.0 + EXP( ( t_ref * ds_vcmax(ft) - deact_vcmax(ft) )   &
                           / ( tref_rmol ) )
  jnumerator_const  = 1.0 + EXP( ( t_ref * ds_jmax(ft)  - deact_jmax(ft) )    &
                           / ( tref_rmol ) )
...
!$OMP        ccp, deact_jmax, deact_vcmax, denom, ds_jmax, ds_vcmax, ft, i2,  &
!$OMP        jmax_temp, jnumerator_const, km, oa, q10_leaf, qtenf_term,       &
!$OMP        tstar, vcmax_temp, vnumerator_const) SCHEDULE(STATIC)
  DO m = 1,veg_pts
    ...
    ! Components of the temperature response of Vcmax and Jmax, Eq.17 of
    ! Medlyn et al. (2002).
    vcmax_numerator = vnumerator_const * EXP( act_vcmax(ft) * t_minus_ref     &
                                        / ( tref_rmol * tstar(l) ) )
    vcmax_temp(l)   = vcmax_numerator                                         &
                      / ( 1.0 + EXP(                                          &
                            ( tstar(l) * ds_vcmax(ft) - deact_vcmax(ft) )     &
                            / ( tstar(l) * rmol )                             &
                                   ) )
    jmax_numerator  = jnumerator_const * EXP( act_jmax(ft) * t_minus_ref      &
                                        / ( t_ref  * rmol * tstar(l) ) )
    jmax_temp(l)   = jmax_numerator                                           &
                     / ( 1.0 + EXP(                                           &
                           ( tstar(l) * ds_jmax(ft) - deact_jmax(ft) )        &
                                 / ( tstar(l) * rmol )                        &
                                  ) )
...
!$OMP END PARALLEL DO
    ...
    CALL calc_photo_parameters( ft, land_pts, pft_photo_model, veg_pts,       &
                                veg_index, denom, jmax_temp, nleaf_layer,     &
                                qtenf_term, vcmax_temp,                       &
                                jmax, rd_dark, vcmax )
...
CASE ( 1 )
  ...
  !---------------------------------------------------------------------------
  CALL calc_photo_parameters( ft, land_pts, pft_photo_model, veg_pts,         &
                              veg_index, denom, jmax_temp, nleaf_top,         &
                              qtenf_term, vcmax_temp,                         &
                              jmax, rd, vcmax )
...
SUBROUTINE calc_photo_parameters( ft, land_pts, pft_photo_model, veg_pts,     &
                                  veg_index, denom, jmax_temp, nleaf,         &
                                  qtenf_term, vcmax_temp,                     &
                                  jmax, rd_dark, vcmax )
...
REAL, INTENT(IN) ::                                                           &
  denom(land_pts),                                                            &
    ! Denominator in equation for vcmax.
  jmax_temp(land_pts),                                                        &
    ! Factor expressing the effect of temperature on Jmax.
...
!-----------------------------------------------------------------------------
! Calculate Vcmax at the reference temperature.
!-----------------------------------------------------------------------------
!$OMP PARALLEL IF(veg_pts > 1)  DEFAULT(NONE)                                 &
!$OMP PRIVATE(l, m)                                                           &
!$OMP SHARED(ft, pft_photo_model, veg_index, veg_pts, denom, fd, jmax,        &
!$OMP        jmax_temp, jv25_ratio, neff, nleaf, qtenf_term, rd_dark, vcmax,  &
!$OMP        vcmax_ref, vcmax_temp, vint, vsl, l_trait_phys )
...
!$OMP DO SCHEDULE(STATIC)
  DO m = 1,veg_pts
    ...
    vcmax(l) = vcmax_ref(l) * vcmax_temp(l)
    jmax(l)  = vcmax_ref(l) * jmax_temp(l) * jv25_ratio(ft)
