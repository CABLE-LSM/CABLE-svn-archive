...
TYPE strnewsfdiag
  ...
  LOGICAL :: st1p5           = .FALSE. ! 236, 328 1.5m temp
  LOGICAL :: sq1p5           = .FALSE. ! 237, 329 1.5m q
  ...
  REAL, ALLOCATABLE :: z0h_eff_gb(:,:)
  !                    27      effective roughness length for heat
  REAL, ALLOCATABLE :: z0m_gb(:,:)
  ...
  REAL, ALLOCATABLE :: gpp_pft(:,:)
  !                    289      Gross prim prod on plant func types (kg C/m2/s)
  REAL, ALLOCATABLE :: rib_surft(:,:)
  ...
  REAL, ALLOCATABLE :: g_leaf_pft(:,:)
  !                    325      Leaf turnover rate on pfts
  REAL, ALLOCATABLE :: rib_ssi(:,:)
  ...
  REAL, ALLOCATABLE :: cdr10m_n_u(:,:)
  REAL, ALLOCATABLE :: cdr10m_n_v(:,:)
  REAL, ALLOCATABLE :: cd10m_n(:,:)
  REAL, ALLOCATABLE :: cd10m_n_u(:,:)
  REAL, ALLOCATABLE :: cd10m_n_v(:,:)
  ...
  REAL, ALLOCATABLE :: vshr_land(:,:) ! 389 wind shear land
  REAL, ALLOCATABLE :: vshr_ssi(:,:)  ! 390 wind shear sea and sea ice
...
CASE DEFAULT
  ...
  sf_diag%sq_t1p5      = (l_apply_diag .AND.                                  &
                         ( sf(236,3) .OR. sf(237,3) .OR. sf(245,3) .OR.       &
                           ...
                           sf(281,3) .OR. sf(282,3) .OR. sf(283,3)))
...
CASE DEFAULT
  ...
  sf_diag%sq_t1p5 = (sf(236,3) .OR. sf(237,3) .OR. sf(245,3) .OR.             &
       ...
       sf(281,3) .OR. sf(282,3) .OR. sf(283,3)) .AND. l_apply_diag
