...
! CABLE: variables that need to be passed in 
        dtl_1, dqw_1, resp_s_tot, rhokpm_pot                      &
)
...
REAL :: D_T, ds_ratio, lh ! these vars are absent in JULES-3.1?

REAL, dimension(land_pts,ntiles) :: rhokpm_pot
#include "c_epslon_cable.h"
...
!print *,"jhan:sf_exch:tile_pts",tile_pts

IF( L_cable ) then
  ! call cable_setup()
  ! call cable_main()

  CALL setup_cable( a_step)

!"row_length, rows ",  row_length, rows
!"land_pts ",  land_pts
!"land_index ",  land_index
!"ntiles, npft ",  ntiles, npft
!"sm_levels ",  sm_levels
!"timestep, timestep_number ",  timestep, timestep_number
!"endstep ",  endstep
!"mype ",  mype
!"latitude ",  latitude
!"longitude ", longitude
!"Fland ",  Fland
!"tile_frac ",  tile_frac
!"tile_pts, tile_index ",  tile_pts, tile_index
!"dzsoil ",  lai, dzsoil
!"DIM_CS1,DIM_CS2 ",  DIM_CS1,DIM_CS2

!     open(unit=8080,file='sf_exch.dat', status="replace", &
!          action="write")
!
!write(8080,*) "canht_ft ",  canht_ft
!write(8080,*) "lai_ft, ",  lai
!write(8080,*) "bexp, ",  bexp
!write(8080,*) "hcon ",  hcon
!write(8080,*) "satcon, ",  satcon
!write(8080,*) "sathh ",  sathh
!write(8080,*) "smvcst ",  smvcst
!write(8080,*) "smvcwt ",  smvcwt
!write(8080,*) "smvccl, ",  smvccl
!write(8080,*) "albsoil ",  albsoil
!write(8080,*) "sthu ",  sthu
!write(8080,*) "snow_tile ",  snow_tile
!write(8080,*) "canopy_tile ", canopy
!write(8080,*) "CO2_MMR ",  CO2_MMR
!write(8080,*) "sw_down ",  sw_down
!write(8080,*) "lw_down ",  lw_down
!write(8080,*) "ls_rain, ",  ls_rain
!write(8080,*) "ls_snow ",  ls_snow
!write(8080,*) "cos_zenith_angle ",  cosz_cable
!write(8080,*) "tl_1, ",  tl_1
!write(8080,*) "qw_1 ",  qw_1
!write(8080,*) "vshr_land, ",  vshr_land
!write(8080,*) "pstar ",  pstar
!write(8080,*) "z1_tq ", z1_tq
!write(8080,*) "z1_uv ", z1_uv
!write(8080,*) "dtl_1, ", dtl_1
!write(8080,*) "dqw_1 ", dqw_1
!write(8080,*) "TSOIL, ",  T_SOIL
!write(8080,*) "SMCL ",  SMCL
!write(8080,*) "STHF ",  STHF
!write(8080,*) "FTL_1 ",  FTL_1
!write(8080,*) "FQW_1 ",  FQW_1
!write(8080,*) "SURF_HT_FLUX_LAND ",  SURF_HT_FLUX_LAND
!write(8080,*) "ECAN_TILE, ",  ECAN_TILE
!write(8080,*) "ESOIL_TILE ",  ESOIL_TILE
!write(8080,*) "EI_TILE ",  EI_TILE
!write(8080,*) "GS, ",  GS
!write(8080,*) "T1P5M_TILE ",  T1P5M_TILE
!write(8080,*) "Q1P5M_TILE ",  Q1P5M_TILE
!write(8080,*) "CANOPY_GB, ",  CANOPY_GB
!write(8080,*) "MELT_TILE ",  MELT_TILE
!write(8080,*) "NPP, ",  NPP
!write(8080,*) "NPP_FT ",  NPP_FT
!write(8080,*) "GPP, ",  GPP
!write(8080,*) "GPP_FT ",  GPP_FT
!write(8080,*) "RESP_S ",  RESP_S
!write(8080,*) "RESP_S_TOT, ",  RESP_S_TOT
!write(8080,*) "RESP_P ",  RESP_P
!write(8080,*) "RESP_P_FT ",  RESP_P_FT
!write(8080,*) "G_LEAF ",  G_LEAF
!write(8080,*) "isnow_flg3l, ",  isnow_flg3l
!write(8080,*) "snow_rho1l ",  snow_rho1l
!write(8080,*) "snage_tile ",  snage_tile
!write(8080,*) "snow_rho3l, ", snow_rho3l
!write(8080,*) "snow_depth3l ", snow_depth3l
!write(8080,*) "snow_mass3l ",  snow_tmp3l
!write(8080,*) "snow_mass3l ",  snow_mass3l
!write(8080,*) "sthu_tile ",  sthu_tile
!write(8080,*) "smcl_tile ",  smcl_tile
!write(8080,*) "sthf_tile, ", sthf_tile
!write(8080,*) "tsoil_tile ", tsoil_tile
!write(8080,*) "snow_cond ",  snow_cond
!write(8080,*) "FTL_TILE ",  FTL_TILE
!write(8080,*) "FQW_TILE, ",  FQW_TILE
!write(8080,*) "TSTAR_TILE ",  TSTAR_TILE
!write(8080,*) "U_S_STD_TILE ",  U_S_STD_TILE
!write(8080,*) "U_S, ",  U_S
!write(8080,*) "RADNET_TILE, ",  RADNET_TILE
!write(8080,*) "FRACA ",  FRACA
!write(8080,*) "rESFS ",  rESFS
!write(8080,*) "RESFT ",  RESFT
!write(8080,*) "Z0H_TILE ",  Z0H_TILE
!write(8080,*) "Z0M_TILE ",  Z0M_TILE
!write(8080,*) "EPOT_TILE ",  EPOT_TILE 
!write(8080,*) ""
!write(8080,*) ""
!write(8080,*) ""
!write(8080,*) ""
...
!close(8080)
  ...
  CALL cable_explicit(                                           &
           ...
           lw_down,   &
           ls_rain, ls_snow, &
           cosz_cable, & ! constructed for CABLE
...
!     open(unit=8081,file='sf_exch_post_cable.dat', status="replace", &
!          action="write")
!
!write(8081,*) "canht_ft ",  canht_ft
!write(8081,*) "lai_ft, ",  lai
!write(8081,*) "bexp, ",  bexp
!write(8081,*) "hcon ",  hcon
!write(8081,*) "satcon, ",  satcon
!write(8081,*) "sathh ",  sathh
!write(8081,*) "smvcst ",  smvcst
!write(8081,*) "smvcwt ",  smvcwt
!write(8081,*) "smvccl, ",  smvccl
!write(8081,*) "albsoil ",  albsoil
!write(8081,*) "sthu ",  sthu
!write(8081,*) "snow_tile ",  snow_tile
!write(8081,*) "canopy_tile ", canopy
!write(8081,*) "CO2_MMR ",  CO2_MMR
!write(8081,*) "sw_down ",  sw_down
!write(8081,*) "lw_down ",  lw_down
!write(8081,*) "ls_rain, ",  ls_rain
!write(8081,*) "ls_snow ",  ls_snow
!write(8081,*) "cos_zenith_angle ",  cosz_cable
!write(8081,*) "tl_1, ",  tl_1
!write(8081,*) "qw_1 ",  qw_1
!write(8081,*) "vshr_land, ",  vshr_land
!write(8081,*) "pstar ",  pstar
!write(8081,*) "z1_tq ", z1_tq
!write(8081,*) "z1_uv ", z1_uv
!write(8081,*) "dtl_1, ", dtl_1
!write(8081,*) "dqw_1 ", dqw_1
!write(8081,*) "TSOIL, ",  T_SOIL
!write(8081,*) "SMCL ",  SMCL
!write(8081,*) "STHF ",  STHF
!write(8081,*) "FTL_1 ",  FTL_1
!write(8081,*) "FQW_1 ",  FQW_1
!write(8081,*) "SURF_HT_FLUX_LAND ",  SURF_HT_FLUX_LAND
!write(8081,*) "ECAN_TILE, ",  ECAN_TILE
!write(8081,*) "ESOIL_TILE ",  ESOIL_TILE
!write(8081,*) "EI_TILE ",  EI_TILE
!write(8081,*) "GS, ",  GS
!write(8081,*) "T1P5M_TILE ",  T1P5M_TILE
!write(8081,*) "Q1P5M_TILE ",  Q1P5M_TILE
!write(8081,*) "CANOPY_GB, ",  CANOPY_GB
!write(8081,*) "MELT_TILE ",  MELT_TILE
!write(8081,*) "NPP, ",  NPP
!write(8081,*) "NPP_FT ",  NPP_FT
!write(8081,*) "GPP, ",  GPP
!write(8081,*) "GPP_FT ",  GPP_FT
!write(8081,*) "RESP_S ",  RESP_S
!write(8081,*) "RESP_S_TOT, ",  RESP_S_TOT
!write(8081,*) "RESP_P ",  RESP_P
!write(8081,*) "RESP_P_FT ",  RESP_P_FT
!write(8081,*) "G_LEAF ",  G_LEAF
!write(8081,*) "isnow_flg3l, ",  isnow_flg3l
!write(8081,*) "snow_rho1l ",  snow_rho1l
!write(8081,*) "snage_tile ",  snage_tile
!write(8081,*) "snow_rho3l, ", snow_rho3l
!write(8081,*) "snow_depth3l ", snow_depth3l
!write(8081,*) "snow_mass3l ",  snow_tmp3l
!write(8081,*) "snow_mass3l ",  snow_mass3l
!write(8081,*) "sthu_tile ",  sthu_tile
!write(8081,*) "smcl_tile ",  smcl_tile
!write(8081,*) "sthf_tile, ", sthf_tile
!write(8081,*) "tsoil_tile ", tsoil_tile
!write(8081,*) "snow_cond ",  snow_cond
!write(8081,*) "FTL_TILE ",  FTL_TILE
!write(8081,*) "FQW_TILE, ",  FQW_TILE
!write(8081,*) "TSTAR_TILE ",  TSTAR_TILE
!write(8081,*) "U_S_STD_TILE ",  U_S_STD_TILE
!write(8081,*) "U_S, ",  U_S
!write(8081,*) "RADNET_TILE, ",  RADNET_TILE
!write(8081,*) "FRACA ",  FRACA
!write(8081,*) "rESFS ",  rESFS
!write(8081,*) "RESFT ",  RESFT
!write(8081,*) "Z0H_TILE ",  Z0H_TILE
!write(8081,*) "Z0M_TILE ",  Z0M_TILE
!write(8081,*) "EPOT_TILE ",  EPOT_TILE 
!write(8081,*) ""
!write(8081,*) ""
!write(8081,*) ""
!write(8081,*) ""
