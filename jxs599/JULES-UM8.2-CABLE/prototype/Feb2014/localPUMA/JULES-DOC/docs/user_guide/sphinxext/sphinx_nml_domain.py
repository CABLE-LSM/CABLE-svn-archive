...
class MemberDirective(ObjectDescription):
    ...
    doc_field_types = [
        ...
        Field('permitted', label=l_('Permitted'), has_arg=False, names=('permitted')),
        Field('default', label=l_('Default'), has_arg=False, names=('default')),
    ]
    ...
    def add_target_and_index(self, name, sig, signode):
        ...
        if indextext:
            self.indexnode['entries'].append(('single', indextext, fullname, fullname))
...
class NamelistDirective(Directive):
    ...
    def run(self):
        ...
        if not noindex:
            ...
            inode = addnodes.index(entries=[('single', indextext, 'namelist-' + nmlname, nmlname)])
            ret.append(inode)
...
class NamelistIndex(Index):
    ...
    name = 'modindex'
    localname = l_('Fortran Namelist Index')
    shortname = l_('namelists')
    ...
    def generate(self, docnames = None):
        ...
        modules = sorted(self.domain.data['namelists'].iteritems(), key=lambda x: x[0].lower())
...
class NamelistDomain(Domain):
    ...
    object_types = {
        'namelist': ObjType(l_('namelist'), 'lst', 'obj'),
        'member': ObjType(l_('member'), 'mem', 'obj')
    }
    ...
    def clear_doc(self, docname):
        ...
        for nml, (fn, _, _) in self.data['namelists'].items():
            if fn == docname:
    ...
    def get_objects(self):
        for nml, info in self.data['namelists'].iteritems():
