   ...
   DO iter = 1, NITER
      ...
      DO j=1,mp
         ...
         IF(canopy%vlaiw(j) > C%LAI_THRESH) THEN
            ...
            ! print*, 'j     : ', j
            ! print*, 'vlaiw : ', canopy%vlaiw(j)
            ! print*, 'dleaf : ', veg%dleaf(j)
            ! print*, 'max s : ', MAX(rough%usuh(j),1.e-6)
            ! print*, 'visc  : ', air%visc(j)
            ! print*, 'shelrb: ', veg%shelrb(j) 

            gbvtop(j) = air%cmolar(j)*C%APOL * air%visc(j) / C%prandt /        &
                        MAX(veg%dleaf(j),1.e-6)                                &
                        * (canopy%us(j) / MAX(rough%usuh(j),1.e-6)             &
                        * veg%dleaf(j) / air%visc(j) )**0.5                    &
                        * C%prandt**(1.0/3.0) / veg%shelrb(j)

            ! print*, 'gbvtop : ', gbvtop(j)
            ! print*, '--------------------'
      ...
      DO i=1,mp
         ...
         IF (canopy%vlaiw(i) > C%LAI_THRESH .AND. abs_deltlf(i) > 0.1) THEN
            ...
            gbhf(i,1) = rad%fvlai(i,1) * air%cmolar(i) * 0.5*C%dheat           &
                       * ( gras(i)**0.25 ) / MAX(veg%dleaf(i),1.e-6)
