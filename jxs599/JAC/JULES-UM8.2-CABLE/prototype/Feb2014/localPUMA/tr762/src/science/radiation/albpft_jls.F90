...
#else
    CALL jules_print('calc_baseflow_jls','')
    WRITE(jules_message,'(a)') 'albpft_jls called to modify albedo to obs'
    CALL jules_print('calc_baseflow_jls',jules_message)
    CALL jules_print('calc_baseflow_jls','')
    WRITE(jules_message,'(a)') &
        '  when l_albedo_obs and l_spec_albedo are not both true'
    CALL jules_print('calc_baseflow_jls',jules_message)
    STOP
#endif
  END IF
