...
SUBROUTINE init_switches(nml_dir)
...
!-----------------------------------------------------------------------------
  ...
  OPEN(NAMELIST_UNIT, FILE=(TRIM(nml_dir) // '/' // 'switches.nml'),          &
...
! The snow scheme only calculates spectrally varying albedos
  IF ( l_snow_albedo .AND. .NOT. l_spec_albedo )                              &
    CALL log_fatal("init_switches",                                           &
                   "l_snow_albedo = T cannot be used with l_spec_albedo = F"//&
                   " as the snow scheme only produces spectrally-varying " // &
                   "albedos")
...
!------------------------------------------------------------------------------
! If not using TRIFFID, switch off competing vegetation
!------------------------------------------------------------------------------
    l_veg_compete = .FALSE.
  END IF
