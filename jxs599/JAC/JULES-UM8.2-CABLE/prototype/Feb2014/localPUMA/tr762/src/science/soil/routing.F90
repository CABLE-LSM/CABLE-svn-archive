...
!       Reset accumulated runoff to zero.
        roffAccumLand(:) = 0.0
      CASE default
        WRITE(jules_message,*)'ERROR: route_drive: do not recognise routeType=',TRIM(routeType)
        CALL jules_print('routing',jules_message)
        STOP
    END SELECT
...
!           At present I'm not sure if we'd ever get here! But just in case...
            WRITE(jules_message,*) 'ERROR: route_drive div by routeCount=0'
            CALL jules_print('routing',jules_message)
            STOP
          ENDIF
        ENDIF
...
!-------------------------------------------------------------------------------
  IF ( .NOT. routeRegLatLon ) THEN
    WRITE(jules_message,*)'ERROR: route_drive_trip: code only exists for regular lat/lon grids.'
    CALL jules_print('routing',jules_message)
    WRITE(jules_message,*)'Sorry. Maybe you''d like to write some new code...?'
    CALL jules_print('routing',jules_message)
    STOP
  ENDIF
