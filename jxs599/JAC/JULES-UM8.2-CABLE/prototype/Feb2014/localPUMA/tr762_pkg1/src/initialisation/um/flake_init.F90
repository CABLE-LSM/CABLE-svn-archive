...
USE dyn_coriolis_mod, ONLY : f3_at_u
  ...
  USE switches, ONLY : l_albedo_obs 
...
! scale the snow free albedo, using the scaling from tile_albedo,
! using VIS albedo as, even if using spectral albedos, lakes use the VIS
            lake_albedo(l) = albsnf_nvg(npft+nnvg-lake) *               &
                                       albobs_scaling(l,lake,1)
          ELSE 
! no scaling: 
            lake_albedo(l) = albsnf_nvg(npft+nnvg-lake) 
          END IF   
