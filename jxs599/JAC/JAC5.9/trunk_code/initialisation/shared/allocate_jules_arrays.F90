...
USE jules_vegetation_mod,     ONLY: l_triffid, l_phenol, l_nitrogen,          &
                                    l_use_pft_psi
USE jules_irrig_mod,          ONLY: l_irrig_dmd, irr_crop
...
USE c_elevate,                ONLY: c_elevate_alloc
...
USE jules_mod,                ONLY: jules_alloc
USE jules_internal,           ONLY: jules_internal_alloc
USE jules_irrig_mod,          ONLY: irrig_vars_alloc
USE jules_surface_mod,        ONLY: jules_surface_alloc
...
#if !defined(UM_JULES)
USE boundary_layer_var_mod,   ONLY: boundary_layer_var_alloc
USE gridmean_fluxes,          ONLY: gridmean_fluxes_alloc
USE update_mod,               ONLY: update_alloc
...
USE orog,                     ONLY: orog_alloc
USE u_v_grid,                 ONLY: u_v_grid_alloc
...
CALL c_elevate_alloc(land_pts, t_i_length, t_j_length,                        &
                     nsurft)
...
CALL jules_alloc(land_pts,ntype,nsurft,rad_nband,                             &
                l_albedo_obs)

CALL jules_internal_alloc(land_pts,npft,                                      &
                          cansnowtile)

CALL jules_surface_alloc(t_i_length,t_j_length)
...
#if !defined(UM_JULES)
CALL boundary_layer_var_alloc(t_i_length,t_j_length,                          &
                              bl_levels,                                      &
                              l_deposition)

CALL gridmean_fluxes_alloc(t_i_length,t_j_length)

CALL update_alloc(land_pts,nsoilt,sm_levels)
...
CALL orog_alloc(land_pts,t_i_length,t_j_length)

CALL u_v_grid_alloc(t_i_length,t_j_length)
