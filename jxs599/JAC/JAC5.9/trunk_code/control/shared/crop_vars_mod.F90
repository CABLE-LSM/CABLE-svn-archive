...
TYPE :: crop_vars_data_type
  ...
  REAL(KIND=real_jlslsm), ALLOCATABLE ::  dvimax_gb(:)
...
TYPE :: crop_vars_type
  ...
  REAL(KIND=real_jlslsm), POINTER ::         dvimax_gb(:)
...
ALLOCATE(crop_vars_data%dvimax_gb(land_pts))
...
DEALLOCATE(crop_vars_data%dvimax_gb)
...
crop_vars%dvimax_gb => crop_vars_data%dvimax_gb
