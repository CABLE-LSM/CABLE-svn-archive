...
SUBROUTINE c_elevate_alloc(land_pts, t_i_length, t_j_length,                  &
                     nsurft)
...
CHARACTER(LEN=*), PARAMETER :: RoutineName='C_ELEVATE_ALLOC'
...
ALLOCATE( surf_hgt_surft(land_pts,nsurft))
ALLOCATE( lw_down_elevcorr_surft(land_pts,nsurft))
surf_hgt_surft(:,:)         = 0.0
lw_down_elevcorr_surft(:,:) = 0.0

#if !defined(UM_JULES)
ALLOCATE( z_land_ij(t_i_length,t_j_length))
z_land_ij(:,:) = 0.0
#endif

IF (lhook) CALL dr_hook(ModuleName//':'//RoutineName,zhook_out,zhook_handle)
RETURN
END SUBROUTINE c_elevate_alloc
