...
USE jules_irrig_mod, ONLY: l_irrig_dmd, irr_crop, l_irrig_limit
...
CASE ( jules )
  ...
  IF ( l_irrig_dmd ) THEN
    CALL irrig_dmd(land_pts, sm_levels, crop_vars%frac_irr_soilt,             &
                   a_step, crop_vars%plant_n_gb,                              &
                   psparms%sthf_soilt, psparms%smvccl_soilt,                  &
                   psparms%smvcst_soilt, psparms%smvcwt_soilt,                &
                   sthzw_soilt, crop_vars%sthu_irr_soilt, psparms%sthu_soilt, &
                   progs%smcl_soilt, irr_crop, crop_vars%dvi_cpft,            &
                   !New arguments replacing USE statements
                   !jules_rivers_mod
                   rivers_sto_per_m2_on_landpts, rivers_adj_on_landpts,       &
                   !crop_vars_mod
                   nday_crop, crop_vars%irrDaysDiag_gb,                       &
                   crop_vars%irrig_water_gb, crop_vars%dvimax_gb)
    ...
    IF ( irr_crop == 1 ) THEN
      CALL calc_crop_date(ainfo%land_index, land_pts, t_i_length, t_j_length,  &
                          nsurft,                                              &
                          ainfo%frac_surft, sw_surft, progs%tstar_surft,       &
                          lw_down, tl_1_ij,                                    &
