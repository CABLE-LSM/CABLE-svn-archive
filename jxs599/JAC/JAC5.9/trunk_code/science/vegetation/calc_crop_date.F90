...
IF ( day_of_yr <= ndpy ) THEN
  ...
  DO l = 1,land_pts
    ...
    crop_vars%tl_1_day_av_gb(l)   = crop_vars%tl_1_day_av_gb(l) + tl_1(i,j) / &
                                    REAL(tspd)
    crop_vars%prec_1_day_av_gb(l) = crop_vars%prec_1_day_av_gb(l) +           &
                          (con_rain(i,j) + con_snow(i,j)                      &
                          + ls_rain(i,j) + ls_snow(i,j))                      &
                          / REAL(tspd)
    crop_vars%rn_1_day_av_gb(l)   = crop_vars%rn_1_day_av_gb(l) + rn(l) /     &
                                    REAL(tspd)
  ...
  IF ( icount(day_of_yr,iyear) == tspd) THEN ! after 1 day
    ...
    DO l = 1,land_pts
      crop_vars%tl_1_day_av_use_gb(l,day_of_yr,iyear)   =                     &
              crop_vars%tl_1_day_av_gb(l)
      crop_vars%prec_1_day_av_use_gb(l,day_of_yr,iyear) =                     &
              crop_vars%prec_1_day_av_gb(l)
      crop_vars%rn_1_day_av_use_gb(l,day_of_yr,iyear)   =                     &
              crop_vars%rn_1_day_av_gb(l)
    END DO
    ...
    DO l = 1,land_pts
      crop_vars%tl_1_day_av_gb(l)     = 0.0
      crop_vars%prec_1_day_av_gb(l)   = 0.0
      crop_vars%rn_1_day_av_gb(l)     = 0.0
    END DO
  ...
  IF ( (iyear /= iyear_old) .AND. datetime_ne ) THEN
    ...
    CALL opt_crop_date(land_pts, ndpy, nyav, nday_crop,                       &
                       crop_vars%plant_n_gb, crop_vars%icntmax_gb,            &
                       crop_vars%tl_1_day_av_use_gb,                          &
                       crop_vars%prec_1_day_av_use_gb,                        &
                       crop_vars%rn_1_day_av_use_gb)
