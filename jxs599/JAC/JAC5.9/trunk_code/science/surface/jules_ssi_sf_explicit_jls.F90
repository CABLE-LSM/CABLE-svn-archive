...
! IN everything not covered so far :
 pstar,rhostar,lw_down,zh,ddmfx,lq_mix_bl,ti,ti_cat,                          &
...
! INOUT diagnostics
 sf_diag,                                                                     &
! INOUT data :
 z0msea,fqw_1,ftl_1,t1_sd,q1_sd,cdr10m,                                       &
...
USE jules_science_fixes_mod, ONLY: l_fix_ctile_orog
...
DO j = tdims%j_start,tdims%j_end
  DO i = tdims%i_start,tdims%i_end
    IF ( flandg(i,j) <  1.0 ) THEN
      IF (nice_use > 1) THEN
        !     ! Include effect of leads.
        rhokh_sea(i,j) = rhostar(i,j) * ch_ssi(i,j) * vshr_ssi(i,j)
        DO n = 1,nice_use
