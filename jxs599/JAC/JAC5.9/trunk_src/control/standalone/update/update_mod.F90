...
SUBROUTINE update_alloc(land_pts,nsoilt,sm_levels)
...
!Arguments
INTEGER, INTENT(IN) :: land_pts,nsoilt,sm_levels
...
CHARACTER(LEN=*), PARAMETER :: RoutineName='UPDATE_ALLOC'

!End of header

IF (lhook) CALL dr_hook(ModuleName//':'//RoutineName,zhook_in,zhook_handle)

ALLOCATE( sthuf_soilt(land_pts,nsoilt,sm_levels))
sthuf_soilt(:,:,:)  = 0.0

IF (lhook) CALL dr_hook(ModuleName//':'//RoutineName,zhook_out,zhook_handle)
RETURN
END SUBROUTINE update_alloc
