...
CONTAINS
  ...
  SUBROUTINE write_output(dels, ktau, met, canopy, casaflux, casapool, casamet, ssnow,                       &
    ...
    IF(output%radiation .OR. output%Albedo) THEN
       ! Add current timestep's value to total of temporary output variable:
       !H!out%Albedo = out%Albedo + REAL((rad%albedo(:, 1) + rad%albedo(:, 2))    &
       !H!     * 0.5, 4)
       out%Albedo = out%Albedo + REAL( (  &
       !Diffuse only
       !rad%reffdf(:, 1) + rad%reffdf(:, 2) )     &
       !Direct only
       rad%reffbm(:, 1) ) &!+ rad%reffbm(:, 2) )     &
       !rad%reffdf(:, 1) + rad%reffdf(:, 2) + rad%reffbm(:, 1) + rad%reffbm(:, 2))     &
            , 4 )!* 0.5, 4)
       ! output calc of soil albedo based on colour? - Ticket #27
       IF (calcsoilalbedo) THEN
