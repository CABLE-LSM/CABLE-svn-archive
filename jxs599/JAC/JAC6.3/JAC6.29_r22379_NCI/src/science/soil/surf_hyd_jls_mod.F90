...
ELSE
  ...
  DO n = 1,nsurft
    ...
    CALL frunoff (land_pts, surft_pts(n), surft_index(:,n), n, frac_cov,       &
    ...
    CALL sieve (land_pts, surft_pts(n), surft_index(:,n), n, frac_cov,         &
    ...
    CALL sieve (land_pts, surft_pts(n), surft_index(:,n), n,                   &
                ls_rainfrac_land, catch_surft(:,n), ls_rain_land,              &
                tile_frac(:,n), timestep, canopy_surft(:,n), tot_tfall_gb,     &
                tot_tfall_surft(:,n))

    CALL frunoff (land_pts, surft_pts(n), surft_index(:,n), n,                 &
                  ls_rainfrac_land, catch_surft(:,n), canopy_surft(:,n),       &
                  infil_surft(:,n), ls_rain_land, tile_frac(:,n), timestep,    &
                  surf_roff_gb, surf_roff_surft(:,n))
    ...
    CALL frunoff (land_pts, surft_pts(n), surft_index(:,n), n,                 &
                  ...
                  infil_surft(:,n), con_rain_land, tile_frac(:,n), timestep,   &
                  surf_roff_gb, surf_roff_surft(:,n))
