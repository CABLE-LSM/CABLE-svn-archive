...
TYPE(progs_cbl_vars_type), INTENT(INOUT) :: progs_cbl
TYPE(work_vars_type), INTENT(OUT)        :: work_cbl
TYPE(params_io_type), INTENT(IN)         :: pars_cbl
...
!-----------------------------------------------------------------------
!CABLE:Dodge OMP for now
DO n = 1, nsurft
  DO l = 1, land_pts
    MELT_surft(L,N) = 0.
    ! from ACCESS_CM2 inits to avoid packing problems. 
    radnet_surft(l,n) = 0.0
    le_surft(l,n) = 0.0
  END DO
END DO

call cable_implicit_main( progs_cbl, work_cbl, pars_cbl )
...
!CABLE: sf_evap stuff might be better to call sf_evap_cbl()
!WARNING: Impact of (:) over soil types for esoil_soilt is unknown
    DO N=1,Nsurft
      DO L=1,LAND_PTS
        ELAKE_surft(L,N) = 0.
      ENDDO
    ...
    DO N=1,Nsurft
     DO K=1,surft_PTS(N)
       ...
       ESOIL_soilt(I,J,:) = ESOIL_soilt(I,J,:) + TILE_FRAC(L,N)*ESOIL_surft(L,N)          
     ENDDO
    ENDDO   
    EXT_soilt = 0.
!CABLE_LSM:End
...
!$OMP DO SCHEDULE(STATIC)
  DO k = 1,surft_pts(n)
    ...
    !CABLE: replace surf_htf_surft w CABLE field 
