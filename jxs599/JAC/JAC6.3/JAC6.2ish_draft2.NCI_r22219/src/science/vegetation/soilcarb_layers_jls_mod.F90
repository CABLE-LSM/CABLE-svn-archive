...
SUBROUTINE soilcarb_layers (land_pts, trif_pts, trif_index, forw, r_gamma,     &
                            ...
                            resp_s_pot, cs, frac_c_label_pool,                 &
...
REAL(KIND=real_jlslsm) ::                                                      &
  ...
  cs_orig(land_pts,dim_cslayer,4),                                             &
...
DO t = 1,trif_pts
  ...
  DO n = 1,dim_cslayer
    ...
    !-------------------------------------------------------------------------
    cs_orig(l,n,1) = cs(l,n,1)
    cs_orig(l,n,2) = cs(l,n,2)
    cs_orig(l,n,3) = cs(l,n,3)
    cs_orig(l,n,4) = cs(l,n,4)
...
!-----------------------------------------------------------------------------
! Update old (labelled and traced) soil carbon tracer
!-----------------------------------------------------------------------------
IF (l_label_frac_cs) THEN
  l_soilage = .FALSE.
  CALL soilcarb_tracer_age(land_pts, trif_pts, trif_index, mix_s, resp_s,      &
                       resp_frac, cs_orig, lit_c_t, lit_frac, dpm_ratio_gb,    &
                       frac_c_label_pool, r_gamma, l_soilage)
END IF
...
DO t = 1,trif_pts
  ...
  DO n = 1,dim_cslayer
    ...
    dcs(l,n,1) = cs(l,n,1) - cs_orig(l,n,1)
    dcs(l,n,2) = cs(l,n,2) - cs_orig(l,n,2)
    dcs(l,n,3) = cs(l,n,3) - cs_orig(l,n,3)
    dcs(l,n,4) = cs(l,n,4) - cs_orig(l,n,4)
