...
IF ( l_imogen ) THEN
  ...
  DO l = 1,land_pts
    ...
    forcing%pstar_ij(i,j)    = pstar_out(l,current_time%month,current_time%day,insd)
    ...
    forcing%ls_snow_ij(i,j)  =                                                 &
      ...
      / REAL(secs_in_day)
    forcing%sw_down_ij(i,j)  = sw_out(l,current_time%month,current_time%day,insd)
    forcing%lw_down_ij(i,j)  = lw_out(l,current_time%month,current_time%day,insd)
    forcing%qw_1_ij(i,j)     = qhum_out(l,current_time%month,current_time%day,insd)
    forcing%tl_1_ij(i,j)     = t_out(l,current_time%month,current_time%day,insd)
  END DO
