...
USE jules_rivers_mod, ONLY:                                                    &
     ...
     rflow_rp, rrun_rp, rrun_surf_rp, rrun_sub_surf_rp
...
ELSE
  ...
  !---------------------------------------------------------------------------
  ! Create INTEGER version of rivers_dir. The REAL version is not used after
  ! this.
  !---------------------------------------------------------------------------
  DO iy = 1, ny_rivers
    DO ix = 1, nx_rivers
      rivers_dir_int(ix,iy) = NINT( rivers_dir(ix,iy) )
    END DO
  END DO
  ...
  l_cyclic_x = .FALSE.
  IF ( rivers_reglatlon ) THEN
    IF ( MAXVAL(rivers_xgrid) - rivers_lon1 + rivers_dlon                      &
         > global_lon_threshold ) THEN
      l_cyclic_x = .TRUE.
      CALL log_info("init_rivers_props","River grid is cyclic in longitude.")
    END IF
  END IF
  ...
  error_sum = error_sum + ERROR
  ALLOCATE(rflow_rp(np_rivers),           STAT = ERROR)
  error_sum = error_sum + ERROR
  ALLOCATE(rrun_rp(np_rivers),            STAT = ERROR)
  error_sum = error_sum + ERROR
  ALLOCATE(rrun_surf_rp(np_rivers),       STAT = ERROR)
  error_sum = error_sum + ERROR
  ALLOCATE(rrun_sub_surf_rp(np_rivers),   STAT = ERROR)
  error_sum = error_sum + ERROR
  ...
  DO ix = 1,nx_rivers
    ...
    DO iy = i1,i2,step
      ...
      IF ( ANY( rivers_dir_int(ix,iy) == flow_dir_river(:) ) ) THEN
        ...
        rivers_dir_rp(ip) = rivers_dir_int(ix,iy)
        rivers_dra_rp(ip) = rivers_dra(ix,iy)
        rivers_seq_rp(ip) = NINT(rivers_seq(ix,iy))
        IF ( rivers_seq_rp(ip) > nseqmax ) THEN
          nseqmax = rivers_seq_rp(ip)
        END IF

        IF ( i_river_vn == rivers_rfm ) THEN
          ! Initialise drainage area from iarea ancillary
          IF ( set_dra ) THEN
            rfm_iarea_rp(ip) = NINT( rivers_dra(ix,iy) )
          ELSE
          ...
          IF (rfm_iarea_rp(ip) > a_thresh) THEN
            rfm_land_rp(ip) = rfm_river  ! river
          ELSE
            rfm_land_rp(ip) = rfm_land   ! land
          END IF
        END IF  !  RFM
  ...
  DO ip = 1,np_rivers
    ...
    CASE ( 1:8 )
      ...
      inext = ix + flow_dir_delta( rivers_dir_rp(ip),1 )
      jnext = iy + flow_dir_delta( rivers_dir_rp(ip),2 )
      ...
      IF ( inext >= 1 .AND. inext <= nx_rivers .AND. jnext >=1                 &
        ...
        ELSE
          ...
          IF ( rivers_dir_int(inext,jnext) == flow_dir_sea ) THEN
            ...
            ! to a river mouth (but there is no need to change rivers_dir_rp
            ! as it is not used again).
          ...
          ELSE
            ...
            ! of river points).
            rivers_next_rp(ip) = -rivers_dir_rp(ip)
          END IF
        END IF  !  mapfr
      ELSE
    ...
    CASE DEFAULT
      CALL log_fatal("init_rivers_props",                                      &
                     ...
                     TRIM(to_string(rivers_dir_rp(ip))) )

    END SELECT  !  rivers_dir_rp

  END DO  !  ip

  ! At this point all values of rivers_next_rp are one of the following:
  ...
  !         domain, and possibly off the river grid (value = -1*rivers_dir_rp)
  !    >0 : next point is on the grid.

  ! Note that the science code could use rivers_next_rp values in -8:-1 to
  ...
  IF ( l_riv_overbank .AND. use_rosgen ) THEN
    DO ip = 1,np_rivers
      contribarea = (rivers_boxareas_rp(ip) * m2tokm2) *                       &
                    ( 1.0 + MAX(0.0, REAL(rfm_iarea_rp(ip))) )
