...
USE jules_forcing_mod,            ONLY: forcing_data_type,                     &
                                        forcing_type,                          &
                                        forcing_assoc
! USE jules_rivers_mod,           ONLY: rivers_(data)_type, &
                                        !rivers_(data)_type, &
                                        !rivers_assoc
...
!TYPE(rivers_(data)_type), INTENT(IN OUT) :: rivers_(data)
...
!CALL jules_rivers_assoc(rivers,rivers_data)
...
CALL init_ic(nml_dir, crop_vars,psparms,toppdm,ainfo,trif_vars, soilecosse,    &
             urban_param, progs, jules_vars, lake_vars)
