...
IF ( rivers_first ) THEN
  IF ( is_master_task() ) THEN
    ...
    DO ip = 1, np_rivers
      ...
      ! Initialise rivers_next_rp to indicate that the point is sea.
      rivers_next_rp(ip)   = sea
      il_river_grid(ip)    = 0
...
IF ( rivers_first ) THEN
  IF ( is_master_task() ) THEN
    ...
    DO l = 1,global_land_pts
      ...
      ! rivers_next_rp retains the value with which it was initialised above).
      IF ( gather_r_inext(gf) >= -1.0 .AND. gather_r_jnext(gf) >= -1.0 ) THEN
        ...
        ELSE
          ...
          ! Set rivers_next_rp to a value in the range -8 to -1 (as used in
      ...
      IF ( i_river_vn == rivers_rfm ) THEN
        IF (rfm_iarea_rp(ip) < 0) THEN
          rfm_land_rp(ip) = rfm_sea    ! Gridcell is sea
        ELSE IF (rfm_iarea_rp(ip) > a_thresh) THEN
          rfm_land_rp(ip) = rfm_river  ! Gridcell is river
        ELSE
          rfm_land_rp(ip) = rfm_land   ! Gridcell is land
        END IF
      END IF
    ...
    ! Nothing more is done at points at which rivers_next_rp=0.
    ! Note that the standalone JULES code (subroutine init_rivers_props) does
    ! not have rivers_next_rp=0 because in that code a sea point cannot be a
    ! river point.
...
USE jules_rivers_mod,  ONLY:                                                   &
  ...
  np_rivers                                                                    &
  !  imported arrays with intent(inout)
  ,rfm_substore_rp, rfm_surfstore_rp, rfm_flowin_rp                            &
  ,rfm_bflowin_rp
...
IF ( is_master_task() ) THEN
  CALL rivpts_to_landpts( np_rivers, rfm_flowin_rp, global_land_pts,           &
                          flowin_lp )
  CALL rivpts_to_landpts( np_rivers, rfm_bflowin_rp, global_land_pts,          &
                          bflowin_lp )
  CALL rivpts_to_landpts( np_rivers, rfm_surfstore_rp, global_land_pts,        &
                          surfstore_lp )
  CALL rivpts_to_landpts( np_rivers, rfm_substore_rp, global_land_pts,         &
                          substore_lp )
END IF
