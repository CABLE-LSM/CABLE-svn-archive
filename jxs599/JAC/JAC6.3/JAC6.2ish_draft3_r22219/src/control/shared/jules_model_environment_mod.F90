...
INTEGER, PARAMETER, PUBLIC :: rivers = 3
CHARACTER(LEN=6), DIMENSION(3) :: lsm_name = [ 'JULES ', 'CABLE ', 'RIVERS' ]
...
!-----------------------------------------------------------------------------
SELECT CASE ( lsm_id )
CASE ( jules, cable, rivers )
  ! Allowed LSMs
  WRITE(jules_message,'(A)') "Land surface model selected is " //              &
     lsm_name(lsm_id)
