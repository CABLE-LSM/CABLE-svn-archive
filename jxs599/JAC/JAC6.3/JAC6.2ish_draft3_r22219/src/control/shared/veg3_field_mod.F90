...
!-------------------------------------------------------------------------------
SUBROUTINE veg3_set_fields(land_pts,nsurft,nnpft,npft,nmasst,ainfo,progs)
...
IF (l_veg3 .AND. l_triffid) THEN
  ...
  DO k = 1, nmasst
    DO n  = 1, nnpft
      DO l = 1, land_pts
        red_state%plantNumDensity(l,n,k) = progs%plantNumDensity(l,n,k)
      END DO
    END DO
  END DO

  !Make sure non-veg fields are written into veg state
  ...
  ainfo%frac_surft(:,:)  = veg_state%frac(:,:)
...
IF (l_veg3) THEN
  ...
  CALL veg3_set_fields(land_pts,nsurft,nnpft,npft,nmasst,ainfo,progs)
END IF
