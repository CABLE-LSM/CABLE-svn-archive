...
!$OMP SCHEDULE(STATIC)
  DO l = 1, land_pts
    ...
    ELSE IF (     (dzsoil(1)  >  0.0)                                          &
             .AND. (dzsoil(1) <= lake_h_ice_gb(l))) THEN
    ...
    ELSE
      ...
      hcon_lake(l) = ( hcice * lake_h_ice_gb(l)                                &
                      +hcwat * lake_depth_gb(l)                                &
                             * nusselt_gb(l)                                   &
                      +hcons_mean_soil(l) * (dzsoil(1) - lake_h_ice_gb(l)      &
                                             - lake_depth_gb(l)))              &
                    / dzsoil(1)
    ...
    ELSE IF ( dzsoil(1) <= lake_h_ice_gb(l) ) THEN

      ! Near surface layer is entirely within the ice. Use piecewise linear
      ! interpolation to find temperature at midpoint of near surface layer.
      ts1_lake_gb(l) = lake_t_ice_gb(l) + (dzsoil(1) / 2.0) *                  &
                       (lake_t_mxl_gb(l) - lake_t_ice_gb(l)) / lake_h_ice_gb(l)
...
!$OMP SCHEDULE(STATIC)
  DO l = 1,land_pts
    ...
    IF (     (l_flake_model)                                                   &
        .AND. ( .NOT. l_aggregate)                                             &
        .AND. (n == lake  )) THEN
      ! zero-layer snow OR no snow
      tsurf(l) = ts1_lake_gb(l) + t_elev(l,n) - tl_1(i,j)
