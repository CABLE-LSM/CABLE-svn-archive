...
! *****************************COPYRIGHT*******************************
MODULE im_sf_pt2_mod_cbl

USE um_types, ONLY: real_jlslsm

IMPLICIT NONE

CHARACTER(LEN=*), PARAMETER, PRIVATE :: ModuleName='IM_SF_PT2_MOD_CBL'
...
CHARACTER(LEN=*), PARAMETER :: RoutineName='IM_SF_PT2_CBL'
...
IF ( .NOT. l_correct ) THEN
  ...
  ! the new scheme (BL solver) using its discretization.
#if !defined(LFRIC)
  ! This code is written for an (i,j) latitude-longitude grid and therefore
  ! not compatible with LFRic, which has its own version
  !-------------------------------------------------------------------------
...
!$OMP DO SCHEDULE(STATIC)
    DO k = 1,surft_pts(n)
      ...
      !CABLE:omitted code defining apart(), bpart()
    END DO
!$OMP END DO
  END DO
...
!$OMP BARRIER
  ...
  !CABLE:from ACCESS_CM2:Ian review if still necessary here
...
!$OMP DO SCHEDULE(STATIC)
    DO k = 1,surft_pts(n)
      ...
      !CABLE:omitted code see Ticket #132 (NCI)
