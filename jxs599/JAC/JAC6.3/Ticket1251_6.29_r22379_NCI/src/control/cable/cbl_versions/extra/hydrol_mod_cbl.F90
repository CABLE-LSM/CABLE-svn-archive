...
SUBROUTINE hydrol_cbl (                                                        &
    ...
    t_soil_soilt, tsurf_elev_surft,            smc_soilt, snow_melt,           &
    sub_surf_roff_gb, surf_roff_gb, tot_tfall_gb, tot_tfall_surft,             &
    non_lake_frac,                                                             &
    ...
    l_top, l_pdm, fexp_soilt, ti_mean_soilt, cs_ch4_soilt, cs_pool_soilt,      &
                    drain_soilt, fsat_soilt, fwetl_soilt, qbase_soilt,         &
...
!CABLE; fulfill INTENT(OUT) component:
...
IF (soil_bgc_model == soil_model_rothc .AND. l_nitrogen) THEN
  IF (nsoilt > 1) THEN
    errorstatus = 101
    CALL ereport("check hydrol_cbl", errorstatus,                              &
                 "nsoilt>1 and l_nitrogen - n_leach not currently coded")
  END IF
  CALL n_leach(land_pts, timestep, smcl_soilt, smcl_old_soilt, w_flux_soilt,   &
