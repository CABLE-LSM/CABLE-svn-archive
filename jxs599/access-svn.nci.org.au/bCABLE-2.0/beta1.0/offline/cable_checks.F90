...
!
! MODULEs used: cable_diag_module
!
!==============================================================================
...
! Name: units_in
!
! Purpose: Convert units if required and initialises some met variables
!
! CALLed from: 
!
! CALLs: rh_sh
...
!jh SUBROUTINE units_in(met,rad,dels)
!jh 
!jh    ! Input arguments
!jh    TYPE (met_type), INTENT(INOUT)   :: met
!jh    TYPE (radiation_type),INTENT(IN) :: rad
!jh    REAL,INTENT(IN)             :: dels
!jh 
!jh    ! Local variables
!jh    REAL                        :: temp_hum ! temporary humidity variable
!jh    INTEGER                     :: i        ! do loop counter
!jh        
!jh    ! Adjust rainfall units:
!jh    IF(units%Rainf=='s') THEN
!jh       met%precip = met%precip*dels
!jh    ELSE IF(units%Rainf=='h') THEN
!jh       met%precip = met%precip*dels/3600.0
!jh    ELSE IF(units%Rainf=='t') THEN
!jh       ! no change required
!jh    ELSE
!jh       WRITE(*,*) 'unknown rainfall units'
!jh    END IF
!jh    ! Adjust surface pressure units:
!jh    IF(units%PSurf=='P') THEN
!jh       met%pmb = met%pmb*0.01
!jh    ELSE IF(units%PSurf=='h') THEN
!jh       ! no change required
!jh    ELSE
!jh       WRITE(*,*) 'unknown pressure units'
!jh    END IF
!jh    ! Adjust air temperature units:
!jh    IF(units%Tair=='K') THEN
!jh       ! no change required
!jh !!     met%tc = met%tc - tfrz
!jh    ELSE IF(units%Tair=='C') THEN
!jh       met%tk = met%tk + tfrz
!jh    ELSE
!jh       WRITE(*,*) 'unknown temperature units'
!jh    END IF
!jh !!   ! Create Kelvin temperature:
!jh !!   met%tk = met%tc + tfrz
!jh    ! Adjust humidity units:
!jh    IF(units%Qair=='%') THEN
!jh       DO i=1,mp
!jh          temp_hum = met%qv(i)
!jh          CALL rh_sh(temp_hum, met%tk(i), met%pmb(i), met%qv(i))
!jh       END DO
!jh    ELSE IF(units%Qair=='g') THEN
!jh       ! no change required
!jh    ELSE
!jh       WRITE(*,*) 'unknown humidity units'
!jh    END IF
!jh    ! Adjust CO2 concentration units:
!jh    IF(units%CO2air=='p') THEN
!jh       met%ca = met%ca/1000000.0
!jh    ELSE IF(units%CO2air=='m') THEN
!jh       ! no change required
!jh    ELSE
!jh       WRITE(*,*) 'unknown temperature units'
!jh    END IF
!jh    
!jh    ! Initialise other met variables:
!jh    WHERE(met%tk<tfrz)
!jh       met%precip_sn = met%precip
!jh    ELSEWHERE
!jh       met%precip_sn=0.0
!jh    END WHERE
!jh    met%tvair = met%tk 
!jh    met%tvrad = met%tk 
!jh    met%coszen = sinbet(met%doy, rad%latitude, met%hod)
!jh    
!jh    ! Check ranges are okay:
!jh    ! Multiply acceptable Rainf ranges by time step size:
!jh    ranges%Rainf = ranges%Rainf*dels ! range therefore depends on dels
!jh    IF(ANY(met%fsd<ranges%SWdown(1)).OR.ANY(met%fsd>ranges%SWdown(2))) THEN
!jh       WRITE(*,*)'SWdown out of specified ranges! Check units.'
!jh       STOP
!jh    ELSE IF(ANY(met%fld<ranges%LWdown(1)).OR.ANY(met%fld>ranges%LWdown(2)))THEN
!jh       WRITE(*,*) 'LWdown out of specified ranges! Check units.'
!jh       STOP
!jh    ELSE IF(ANY(met%qv<ranges%Qair(1)).OR.ANY(met%qv>ranges%Qair(2))) THEN
!jh       WRITE(*,*) 'Qair out of specified ranges! Check units.'
!jh       STOP
!jh    ELSE IF(ANY(met%precip<ranges%Rainf(1)).OR.                                 &
!jh            ANY(met%precip>ranges%Rainf(2))) THEN
!jh       WRITE(*,*) 'Rainf out of specified ranges! Check units'
!jh       STOP
!jh    ELSE IF(ANY(met%ua<ranges%Wind(1)).OR.ANY(met%ua>ranges%Wind(2))) THEN
!jh       WRITE(*,*) 'Wind out of specified ranges! Check units'
!jh       STOP
!jh    ELSE IF(ANY(met%tk<ranges%Tair(1)).OR.ANY(met%tk>ranges%Tair(2))) THEN
!jh       WRITE(*,*)  'Tair out of specified ranges! Check units.'
!jh       STOP
!jh    ELSE IF(ANY(met%pmb<ranges%PSurf(1)).OR.ANY(met%pmb>ranges%PSurf(2))) THEN
!jh       WRITE(*,*) 'PSurf out of specified ranges! Check units'
!jh       WRITE(*,*) met%pmb
!jh       STOP
!jh    END IF
!jh 
!jh END SUBROUTINE units_in
