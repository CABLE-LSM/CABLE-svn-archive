module cable_diag_module
   ...
   integer :: gopenstatus,galloctest=1
   ...
   interface put_var_nc 
      module procedure put_var_ncr1, put_var_ncr2 
   end interface put_var_nc 


   interface get_var_nc 
      module procedure get_var_ncr1, get_var_ncr2 
  ...
  subroutine cable_stat( routname)
   ...
   subroutine cable_diag(funit, Nvars, basename, dimx, dimy, timestep, node, vname1, var1 )
      implicit none
      integer, intent(in) :: Nvars,dimx, dimy, timestep,node, funit
      real(r_2), intent(in), dimension(:) :: var1
      character(len=*), intent(in) :: basename, vname1
      character(len=30) :: filename, chnode
      logical, save :: first_call=.true.      
         if(first_call) & 
   print *, "funit, Nvars, basename, dimx, dimy, timestep, node, vname1", funit, Nvars, basename, dimx, dimy, timestep, node, vname1
      ...
      10 format(i2.2)   
         ...
         if(timestep==1) & 
         call cable_diag_desc1( funit, Nvars, trim(filename), dimx, dimy, vname1 )
         gopenstatus=1
         call cable_diag_data1( funit, Nvars, trim(filename), dimx, timestep, dimy, var1 )
      return 
   end subroutine cable_diag

   subroutine cable_diag_desc1( funit, Nvars, filename, dimx, dimy, vname1 )
      implicit none
      integer, intent(in) ::  funit,Nvars,dimx,dimy 
      character(len=*), intent(in) :: filename, vname1
   
   !print *, "funit, Nvars, basename, dimx, dimy, vname1", funit, Nvars, filename, dimx, dimy, vname1
         open(unit=funit+1,file=filename//'.dat', status="replace",action="write", iostat=gopenstatus )
         if(gopenstatus==gok) then
               write (funit+1,*) 'Number of var(s): '
               write (funit+1,*) Nvars
               write (funit+1,*) 'Name of var(s): '
               write (funit+1,7139) vname1 
   7139        format(a)            
               write (funit+1,*) 'dimension of var(s) in x: '
               write (funit+1,*) dimx 
               write (funit+1,*) 'dimension of var(s) in y: '
               write (funit+1,*) dimy 
         else
   ...
   subroutine cable_diag_data1(  funit,Nvars, filename, dimx, timestep, kend, var1  )
      implicit none
      integer, intent(in) :: funit, Nvars, dimx, timestep, kend
      real(r_2), intent(in), dimension(:) :: var1
      character(len=*), intent(in) :: filename
         if(timestep==1) then
   print *, "funit, Nvars, basename, dimx, dimy, timestep, ", funit, Nvars, filename, dimx, kend, timestep
         open(unit=funit,file=filename//'.bin',status="unknown",action="write", &
  ...
  11 format(i2)   
   ...
   subroutine put_var_ncr1(ncid, var_name, var, n_call )
      use netcdf
      implicit none
   ...
   subroutine put_var_ncr2(ncid, var_name, var, n_call)
      ...
      integer :: ncok, varID
         ...
         ncok = NF90_INQ_VARID(ncid, var_name, varId )
            if (ncok /= nf90_noerr ) call stderr_nc('inquire var ', var_name)      
         ncok = NF90_PUT_VAR(ncid, varId, var, start=(/1,1,1,n_call /), & 
                              count=(/1,1,6,1/))
            if (ncok /= nf90_noerr ) call stderr_nc('putting var ', var_name)      
      return
   ...
   subroutine get_var_ncr1(ncid, var_name, var, n_call, kend )
      use netcdf
      use define_dimensions, only : r_2,mp
      implicit none
      character(len=*), intent(in) :: var_name
      ...
      real, dimension(mp) :: temp 
         ...
         var = real( temp, r_2 )    
   end subroutine get_var_ncr1
  
   subroutine get_var_ncr2(ncid, var_name, var, n_call, kend)
      use netcdf
      use define_dimensions, only : r_2, mp, ms
      implicit none
      character(len=*), intent(in) :: var_name
      real(r_2), dimension(mp,ms),intent(out) :: var
      integer, intent(in) :: ncid, kend, n_call 
      integer :: ncok, varID
      real, dimension(mp,ms) :: temp 
       
         ncok = NF90_INQ_VARID(ncid, var_name, varId )
            if (ncok /= nf90_noerr ) call stderr_nc('inquire var ', var_name)

         ncok = NF90_GET_VAR(ncid, varId, temp, start=(/1,1,1,n_call /), & 
                              count=(/1,1,6, 1/))
            if (ncok /= nf90_noerr ) call stderr_nc('putting var ', var_name)      
         var = real( temp, r_2 )    
