...
CONTAINS
   ...
   SUBROUTINE init_radiation(met,rad,veg,canopy)
      ...
      REAL, DIMENSION(nrb)     :: cos3  ! cos(15 45 75 degrees)
      REAL, DIMENSION(mp,nrb)  :: xvlai2 ! 2D vlai
      REAL, DIMENSION(mp,nrb)  :: xk    ! extinct. coef.for beam rad. and black leaves
      REAL, DIMENSION(mp)    :: xphi1 ! leaf angle parmameter 1
      REAL, DIMENSION(mp)    :: xphi2 ! leaf angle parmameter 2
      INTEGER :: ictr
   ...
   SUBROUTINE radiation(bal, soil, ssoil, veg, air, met, rad, canopy, dels)
      ...
      REAL, DIMENSION(mp)  :: cf1    ! (1.0 - rad%transb * cexpkdm) / (extkb + extkdm(:,b))
      REAL, DIMENSION(mp)  :: cf3    ! (1.0 - rad%transb * cexpkbm) / (extkb + extkbm(:,b))
      REAL, DIMENSION(mp)  :: cf2n   ! exp(-extkn * vlai) (nitrogen)
      REAL, DIMENSION(mp)  :: emair  ! air emissivity
      REAL, DIMENSION(mp)  :: flpwb  ! black-body long-wave radiation
      REAL, DIMENSION(mp)  :: flwv   ! vegetation long-wave radiation (isothermal)
      INTEGER              :: b ! rad. band 1=visible, 2=near-infrared, 3=long-wave
      LOGICAL, DIMENSION(mp)    :: mask   ! select points for calculation
      REAL, DIMENSION(mp)  :: xx1,tssp   ! vegetation long-wave radiation (isothermal)
...
!jhan:cahnge Mk3l to read veg%taul like UM 
  ...
  ELEMENTAL FUNCTION sinbet(doy,xslat,hod) RESULT(z)
    ...
    REAL, INTENT(IN) :: xslat  ! latitude (degrees north)
    REAL, INTENT(IN) :: hod    ! hour of day
