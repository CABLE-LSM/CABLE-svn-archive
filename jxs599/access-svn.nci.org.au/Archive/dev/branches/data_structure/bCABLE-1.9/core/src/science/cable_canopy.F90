...
module canopy_module
   ...
   subroutine define_canopy(bal,rad,rough,air,met,dels,ssoil,soil,veg, canopy &
      ...
      use cable_data_module, only : const, air_auto_type
      ...
      type (air_auto_type), intent(out) :: o
     REAL, DIMENSION(mp) :: met_tvair   ! within canopy air temperature (oK)
     REAL, DIMENSION(mp) :: met_pmb     ! surface air pressure (mbar)
...
!                        (canopy%us(j) / MIN(rough%usuh(j), 0.2) * &
...
!           endif
            ...
            do j=1,mp
               if ( canopy%vlaiw(j) > LAI_THRESH .and. rough%hruff(j) > rough%z0soilsn(j) ) then
                  rad%lwabv(j) = capp * rmair * (tlfy(j)-met%tk(j))* sum_rad_gradis(j) &
                            + canopy%fhvw(j)*sum_rad_gradis(j) /max(0.001,ghwet(j))
            ...
            !if(cable_runtime%um) then
               ssoil%potev =  Humidity_deficit_method(ssoil%qstss ) 
            !else
...
!            print 21,idjd1,ktau_gl,ssoil%qstss(idjd1),ssoil%tss(idjd1),ssoil%qstss(idjd1)-met%qv(idjd1), &
!           ssoil%rtsoil(idjd1),ssoil%potev(idjd1) ,rad%qssabs(idjd1),canopy%tv(idjd1),canopy%fns(idjd1), &
!           ssoil%wetfac(idjd1),canopy%fess(idjd1),canopy%fesp(idjd1),canopy%fes(idjd1),met%tk(idjd1)
!21         format(1x,'idjd1canopy',2i5,f7.4,f6.1,f7.4,f6.0,f7.0,f6.0,x,f6.1,f6.0,f5.2,x,4f6.1)
!           print 22,idjd2,ktau_gl,ssoil%qstss(idjd2),ssoil%tss(idjd2),ssoil%qstss(idjd2)-met%qv(idjd2), &
!           ssoil%rtsoil(idjd2),ssoil%potev(idjd2) ,rad%qssabs(idjd2),canopy%tv(idjd2),canopy%fns(idjd2), &
!           ssoil%wetfac(idjd2),canopy%fess(idjd2),canopy%fesp(idjd2),canopy%fes(idjd2),met%tk(idjd2)
!22     format(1x,'idjd2canopy',2i5,f7.4,f6.2,f7.4,f6.0,f7.0,f6.0,x,f6.1,f6.0,f5.2,x,4f6.1)
!           print 23,idjd3,ktau_gl,ssoil%qstss(idjd3),ssoil%tss(idjd3),ssoil%qstss(idjd3)-met%qv(idjd3), &
!           ssoil%rtsoil(idjd3),ssoil%potev(idjd3) ,rad%qssabs(idjd3),canopy%tv(idjd3),canopy%fns(idjd3), &
!           ssoil%wetfac(idjd3),canopy%fess(idjd3),canopy%fesp(idjd3),canopy%fes(idjd3),met%tk(idjd3)
!23     format(1x,'idjd3canopy',2i5,f7.4,f6.1,f7.4,f6.0,f7.0,f6.0,x,f6.1,f6.0,f5.2,x,4f6.1)
            ...
            do j=1,mp
               if (ssoil%potev(j) .ge. 0.) then
                  ssoil%potev(j) = max(0.001,ssoil%potev(j))
               else
                  ssoil%potev(j) = min(-0.002,ssoil%potev(j))
               endif
               !jhan:Eva has added this block
               if (canopy%fevw_pot(j) .ge. 0.) then
                  canopy%fevw_pot(j) = max(0.001,canopy%fevw_pot(j))
               else
            ...
            enddo
            canopy%wetfac_cs = min(1.0,canopy%fe / rlower_limit )
   ...
   function Humidity_deficit_method(qstss ) result(ssoilpotev)
      implicit none
      real(r_1), dimension(mp)  :: ssoilpotev 
   ...
   subroutine latent_heat_flux() 
      ...
      !real(r_1) :: flower_limit, fupper_limit
      real(r_1), dimension(mp) ::  flower_limit, fupper_limit
      integer :: j
      integer :: idjd1,idjd2,idjd3
...
!         print 10,ssoil%wetfac(idjd1),ssoil%potev(idjd1),ssoil%pudsto(idjd1),pwet(idjd1), &
!         ssoil%pudsmx(idjd1),canopy%fess(idjd1)
!10       format(1x,'idjd1latentheat',f5.2,f7.0,x,3f7.3,x,f7.1)
...
!         print 11,ssoil%wetfac(idjd1),ssoil%potev(idjd1),ssoil%pudsto(idjd1),pwet(idjd1), &
!         ssoil%pudsmx(idjd1),canopy%fess(idjd1),swilt_eff(idjd1),frescale(idjd1),ssoil%evapfbl(idjd1,1)*air%rlam(idjd1)/dels
!11       format(1x,'idjd1latenthea2',f5.2,f7.0,x,3f7.3,x,f7.1,2x,f6.2,f8.0,f8.2)
         ...
         enddo 
!         print 12,ssoil%wetfac(idjd1),ssoil%potev(idjd1),ssoil%pudsto(idjd1),pwet(idjd1), &
!      ssoil%pudsmx(idjd1),canopy%fess(idjd1),swilt_eff(idjd1),frescale(idjd1), &
!      ssoil%evapfbl(idjd1,1)*air%rlam(idjd1)/dels,flower_limit(idjd1),fupper_limit(idjd1),ssoil%cls(idjd1)
!12       format(1x,'idjd1latenthea3',f5.2,f7.0,x,3f7.3,x,f7.1,2x,f6.2,f8.0,f8.2,2x,2f8.2,f5.1)
...
!jhan:these are all NOT contained subrs in mk3l
    ...
    subroutine qsatfjh(var,tair,pmb) 
      ...
      do j=1,mp
         !if (knode_gl == 1 ) then
         !print *,'jhan: j', j 
         !print *,'jhan:rmho2/rmair *tetena', rmh2o/rmair *tetena
         !print *,'jhan:exp_Arg', tetenb*tair/(tetenc+tair) 
         !print *,'jhan:exp_Arg', pmb
         !print *,''
         !endif
...
!         print 10,idjd1,met%precip(idjd1),met%precip_sn(idjd1),canopy%wcint(idjd1), &
!         canopy%through(idjd1),canopy%cansto(idjd1),canopy%fwet(idjd1),ssoil%wetfac(idjd1), &
!         ssoil%owetfac(idjd1),ssoil%wb(idjd1,1),ssoil%wbice(idjd1,1),ssoil%tgg(idjd1,1), &
!         soil%swilt(idjd1),soil%sfc(idjd1),soil%ssat(idjd1),met%tk(idjd1), &
!         veg%canst1(idjd1)*canopy%vlaiw(idjd1)
!10       format(1x,'idjd1wetfac',i5,2f6.3,x,3f7.4,x,3f5.2,x,2f6.3,f6.1,3f5.2,f6.1,f7.4)
!         print 11,idjd2,met%precip(idjd2),met%precip_sn(idjd2),canopy%wcint(idjd2), &
!         canopy%through(idjd2),canopy%cansto(idjd2),canopy%fwet(idjd2),ssoil%wetfac(idjd2), &
!         ssoil%owetfac(idjd2),ssoil%wb(idjd2,1),ssoil%wbice(idjd2,1),ssoil%tgg(idjd2,1), &
!         soil%swilt(idjd2),soil%sfc(idjd2),soil%ssat(idjd2),met%tk(idjd2), &
!         veg%canst1(idjd2)*canopy%vlaiw(idjd2)
!11       format(1x,'idjd2wetfac',i5,2f6.3,x,3f7.4,x,3f5.2,x,2f6.3,f6.1,3f5.2,f6.1,f7.4)
!         print 12,idjd3,met%precip(idjd3),met%precip_sn(idjd3),canopy%wcint(idjd3), &
!         canopy%through(idjd3),canopy%cansto(idjd3),canopy%fwet(idjd3),ssoil%wetfac(idjd3), &
!         ssoil%owetfac(idjd3),ssoil%wb(idjd3,1),ssoil%wbice(idjd3,1),ssoil%tgg(idjd3,1), &
!         soil%swilt(idjd3),soil%sfc(idjd3),soil%ssat(idjd3),met%tk(idjd3), &
!         veg%canst1(idjd3)*canopy%vlaiw(idjd3)
!12       format(1x,'idjd3wetfac',i5,2f6.3,x,3f7.4,x,3f5.2,x,2f6.3,f6.1,3f5.2,f6.1,f7.4)
