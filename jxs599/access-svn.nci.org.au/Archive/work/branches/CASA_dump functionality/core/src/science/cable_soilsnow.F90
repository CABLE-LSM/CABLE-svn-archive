...
MODULE soil_snow_module
  ...
  REAL, PARAMETER :: cp = capp       ! specific heat capacity for air
  REAL, PARAMETER :: rhowat = 1000.0 ! density of water
   !jhan:Eva uses .11 -> 1.
  REAL, PARAMETER :: snmin = 1.    ! for 3-layer;
  REAL, PARAMETER :: max_ssdn = 750.0
  REAL, PARAMETER :: max_sconds = 2.51
   !jhn::r2411 uses .85
  REAL, PARAMETER :: frozen_limit = 0.85 ! EAK Feb2011 (could be 0.95)
...
CONTAINS
  ...
  SUBROUTINE smoisturev (dels,ssoil,soil,veg)
   use cable_common_module
    ...
    INTEGER, PARAMETER                   :: ntest = 0 ! 2 for funny pre-set
    ! nmeth selects the solution method
    INTEGER, PARAMETER                   :: nmeth = -1 ! preferred method
    !                                  Values as follows:
...
! change dimension of at,bt,ct from 3*ms to ms (BP Jun2010)
    ...
    REAL, DIMENSION(mp)      :: wblfmn
    REAL, DIMENSION(mp)      :: wblfmx
    ...
    REAL, DIMENSION(mp)      :: totwba ! diagnostic
    REAL, DIMENSION(mp)      :: totwbb
    REAL, DIMENSION(mp)      :: totwbc
    REAL, DIMENSION(mp)      :: totwblb
    REAL, DIMENSION(mp)      :: totwblc
    REAL, DIMENSION(mp)      :: wbficemx
...
!jhan:default to UM nglacier =0
    ...
    REAL, DIMENSION(mp)     :: sfact
    REAL, DIMENSION(mp)     :: sgamm
    REAL, DIMENSION(mp)     :: smasstot
    REAL, DIMENSION(mp,0:3) :: smelt1
    REAL, DIMENSION(mp)     :: talb ! snow albedo
    REAL, DIMENSION(mp)     :: tmp ! temporary value
    REAL, DIMENSION(mp)     :: xxx
...
!  use arraydiag_m
    ...
    logical :: cable_runtime_coupled = .false.
    integer, save :: ktau =0 
    INTEGER :: idjd1,idjd2,idjd3,idjd4,idjd5
