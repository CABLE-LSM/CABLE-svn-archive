...
CONTAINS
  ...
  SUBROUTINE ruff_resist(veg, rough, ssoil, soil, met, canopy)
    ...
    REAL, DIMENSION(mp) :: xx ! =ccd*LAI; working variable 
    REAL, DIMENSION(mp) :: dh ! d/h where d is zero-plane displacement
    REAL, DIMENSION(mp) :: hmax ! maximum height of canopy from
