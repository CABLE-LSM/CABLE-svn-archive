...
!      si(:,1) = (rwater - 1.) / ( 0. - 1.) * ( rwater  - 2.) / (0. - 2.)
!      si(:,2) = (rwater - 0.) / ( 1. - 0.) * ( rwater  - 2.) / (1. - 2)
!      si(:,3) = (rwater - 0.) / ( 2. - 0.) * ( rwater  - 1.) / (2. - 1.)
      step = 2.4
      s1 = (step - 1.) / ( 0. - 1.) * ( step  - 2.) / (0. - 2.)
      s2 = (step - 0.) / ( 1. - 0.) * ( step  - 2.) / (1. - 2)
      s3 = (step - 0.) / ( 2. - 0.) * ( step  - 1.) / (2. - 1.)
      tssp = max(0., ssoil%otss_0*s1 + ssoil%otss*s2 + ssoil%tss*s3)
    xx1 = max(-1.,min(1.,0.5*ssoil%deltss))
    where ( ssoil%tss - ssoil%otss > 0. .and. ssoil%otss - ssoil%otss_0  > 0. .and. &
              (met%fsd(:,1) + met%fsd(:,2))/ max(0.1,met%ofsd) > 0.9 ) 
          xx1 = max(0.,min(2.,tssp - ssoil%tss ))
    elsewhere ( ssoil%tss - ssoil%otss < 0. .and. ssoil%otss - ssoil%otss_0 <  0. .and. &
              (met%fsd(:,1) + met%fsd(:,2))/ max(0.1,met%ofsd) < 1.1 )
          xx1 = max(-2.,min(0.,tssp - ssoil%tss ))
    elsewhere
          xx1 = 0.
    end where
    print 11, ssoil%deltss,xx1,ssoil%tss,ssoil%tss+xx1,tssp,ssoil%otss_0, ssoil%otss,ssoil%tgg(:,1),ssoil%tggsn(:,1),canopy%vlaiw,met%fsd(:,1)+met%fsd(:,2)
11  format(1x,'tsscor',2f5.2,7f7.2,1x,f5.2,f6.0)
    ! Define longwave from soil/snow surface:
    ssoil%tss_p = ssoil%tss+xx1
    rad%flws = sboltz*emsoil* ssoil%tss **4
    !rad%flws = sboltz*emsoil* (ssoil%tss+xx1) **4
    rad%flws = sboltz*emsoil* (ssoil%tss_p) **4
