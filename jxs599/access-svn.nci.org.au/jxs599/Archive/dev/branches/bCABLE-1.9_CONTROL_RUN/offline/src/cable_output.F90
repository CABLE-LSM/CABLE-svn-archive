...
!   open_output_file -  open the output netcdf file
...
CONTAINS
  ...
  SUBROUTINE open_output_file(dels,soil,veg,bgc,rough)
    ...
    INTEGER :: xID,yID,zID,radID,soilID,soilcarbID,plantcarbID,tID,&
...
!         'rs20',REAL(soil%rs20,4),ranges%rs20,patchout%rs20,'real')
  ...
  !=====================================================================
  SUBROUTINE write_output(dels,met,canopy, &
    ...
    IF(check%mass_bal) CALL mass_balance(dels,ssoil,soil,canopy, &
         met,air,bal)
    ...
    ELSE IF(output%averaging(1:4)=='user'.OR.output%averaging(1:2)=='da') THEN
       ...
       IF(MOD(ktau_gl,output%interval)==0) THEN ! i.e.ktau divisible by interval
    ...
    ELSE IF(output%averaging(1:2)=='mo') THEN ! write monthly averages to file
       ...
       IF(leaps) THEN
          ...
          IF(((ANY(MOD(met%year,4)==0).AND.ANY(MOD(met%year,100)/=0)).OR. & 
             ...
             IF(ANY((lastdayl*24*3600/INT(dels))==ktau_gl)) THEN ! last time step of month
          ...
          ELSE ! not currently a leap year
             IF(ANY((lastday*24*3600/INT(dels))==ktau_gl)) THEN ! last time step of month
                ! increment output month counter
                out_month = MOD(out_month,12) + 1 ! can only be 1 - 12
                ! write to output file this time step 
                writenow = .TRUE.
    ...
    IF(writenow) THEN
       ...
       timetemp(1) = DBLE(REAL(ktau_gl)*dels)
  ...
  !==========================================================================
  SUBROUTINE create_restart(logn, &
...
!    CALL write_ovar (ncid_restart,rpid%rs20,'rs20',REAL(soil%rs20,4),ranges%rs20, &
    CALL write_ovar (ncid_restart,rpid%rs20,'rs20',REAL(veg%rs20,4),ranges%rs20, &
         .TRUE.,'real',.TRUE.)
