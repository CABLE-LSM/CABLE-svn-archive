...
!           endif
            ...
            do j=1,mp
               if ( canopy%vlaiw(j) > LAI_THRESH .and. rough%hruff(j) > rough%z0soilsn(j) ) then
                  ...
                  rad%lwabv(j) = capp * rmair * (tlfy(j)-met%tk(j))* sum_rad_gradis(j) 
                            !+ (canopy%fevw(j) + canopy%fhvw(j))*sum_rad_gradis(j) /max(0.001,ghwet(j))
                            !- (canopy%fevw(j) + canopy%fhvw(j))*sum_rad_gradis(j) /max(0.001,ghwet(j))
            ...
            call qsatfjh(ssoil%qstss,ssoil%tss_p-tfrz,met%pmb)
            ...
            canopy%fhs = air%rho*capp*(ssoil%tss_p - met%tk) /ssoil%rtsoil
...
!            enddo 
   ...
   subroutine latent_heat_flux() 
      ...
      integer :: j
         ...
         !canopy%fess= ssoil%wetfac * ssoil%potev + canopy%fes_cor
         !canopy%fess= ssoil%wetfac * ssoil%potev 
              ...
              ! canopy%fhvw(i) = 0.
               ...
               IF (ecx(i) > 0.0 .AND. canopy%fwet(i) < 1.0) Then
                  ...
                  DO kk = 1,ms
                     ...
                     !swilt factor not consistent with soilsnow
                     ssoil%evapfbl(i,kk) = MIN(evapfb(i)*veg%froot(i,kk), &
