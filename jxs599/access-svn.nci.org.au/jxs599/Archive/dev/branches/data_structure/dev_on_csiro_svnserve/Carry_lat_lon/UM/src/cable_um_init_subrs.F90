...
!========================================================================                                   
   ...
   subroutine initialize_latlon(latitude,longitude)
      ...
      INTEGER :: j
         ...
         if( first_call)  then 
            allocate( cable%lat(mp), cable%lon(mp) )
print *, 'max(latitude)', maxval(asin(latitude) )
print *, 'min(latitude)', minval(asin(latitude) )
print *, 'max(longitude)', maxval(acos(longitude) ) 
print *, 'min(longitude)', minval(acos(longitude) )
stop 
            ...
            call cable_diag( 1, 'lat', mp, 1, ktau_gl,  & 
                  knode_gl, 'lat', cable%lat )
            call cable_diag( 1, 'lon', mp, 1, ktau_gl,  & 
                  knode_gl, 'lon', cable%lon )
         endif
      
      return
   end subroutine initialize_latlon
