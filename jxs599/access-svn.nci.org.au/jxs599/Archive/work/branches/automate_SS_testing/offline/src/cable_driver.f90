...
!jhan: Bernard can you help with this, names are always different e.g. maybe "met file.nc"
...
PROGRAM offline_driver
   ...
   LOGICAL :: &
      ...
      spincasainput, &    ! TRUE: input required to spin casacnp wil be saved;
                        ! FALSE: read input to spin casacnp 1000 years
      spincasa, &       ! TRUE: casacnp will spin mloop times,
                        ! FALSE: no spin up
      l_casacnp, &      ! using casaCNP with CABLE
   ...
   INTEGER(i_d) :: & 
      ...
      mloop, &    ! # spinup loops for casaCNP
      ktauday, &  !  day counter for casaCNP
      nyear, &    ! year counter for casaCNP
   ...
   !jhan : kstart=parameter?
   kstart = 1
   tstep = 0          ! initialise spinup time step
   spinConv = .FALSE. ! initialise spinup convergence variable
 
   ! jhan:BP how does this exit CABLE run
   ! YP introduced option to spinup CASA  - NO CABLE
   if( cable_user%spinCASA_ONLY = .TRUE. ) then
      if(icycle>0) then
         if (spincasa) then
            mloop=500
            call spincasacnp(dels,kstart,kend,mloop,veg,soil,casabiome, &
