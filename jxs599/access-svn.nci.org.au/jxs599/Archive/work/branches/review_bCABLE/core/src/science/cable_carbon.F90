...
!    INTEGER, INTENT(IN)              :: mvtype  ! Number of vegetation types
    REAL, PARAMETER     :: beta = 0.9
    REAL, DIMENSION(mp) :: cfsf     ! fast soil carbon turnover
    REAL, DIMENSION(mp) :: cfrts    ! roots turnover
    REAL, DIMENSION(mp) :: cfwd     ! wood turnover
    REAL, DIMENSION(mp) :: fcl ! fraction of assimilated carbon that
                                ! goes to the construction of leaves  (eq. 5)
    REAL, DIMENSION(mp) :: fr
    REAL, DIMENSION(mp) :: clitt
    REAL, DIMENSION(mp) :: coef_cd ! total stress coeff. for veg (eq. 6)
    REAL, DIMENSION(mp) :: coef_cold  ! coeff. for cold stress (eq. 7)
    REAL, DIMENSION(mp) :: coef_drght ! coeff. for drought stress (eq. 8)
    REAL, DIMENSION(:), ALLOCATABLE :: rw, tfcl, tvclst
!jhan:see earliervn for comment
    REAL, DIMENSION(:), ALLOCATABLE :: trnl, trnr, trnsf, trnw

    REAL, DIMENSION(mp) :: wbav ! water stress index 
...
! rml 17/1/11 change minimum pool size from 0.001 to 0. (since want 0. for vegtype=ice)
  ...
  SUBROUTINE soilcarb( soil, ssoil, veg, bgc, met, canopy)
    ...
    REAL, DIMENSION(mp)  :: den ! sib3  
    INTEGER                             :: k
    REAL, DIMENSION(mp)  :: rswc   
    REAL, DIMENSION(mp)  :: sss    
    REAL, DIMENSION(mp)  :: e0rswc 
    REAL, DIMENSION(mp)  :: ftsoil 
    REAL, DIMENSION(mp)  :: ftsrs  
    REAL, PARAMETER                     :: t0 = -46.0
    REAL, DIMENSION(mp)  :: tref   
    REAL, DIMENSION(mp)  :: tsoil 
    REAL, DIMENSION(mstype)             :: rswch
    REAL, DIMENSION(mstype)             :: soilcf
...
! rml 17/1/11 made plant respiration calculation into a subroutine
  SUBROUTINE plantcarb(veg, bgc, met, canopy)
   use physical_constants
    ...
    REAL, DIMENSION(mp) :: poolcoef1 ! non-leaf carbon turnover rate * non-leaf pool size
    ...
    REAL, DIMENSION(mp) :: tmp1,tmp2,tmp3
