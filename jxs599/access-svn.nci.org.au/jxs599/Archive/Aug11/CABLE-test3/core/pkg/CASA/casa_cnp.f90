...
SUBROUTINE casa_xkN(xkNlimiting,casapool,casaflux,casamet)
...
SUBROUTINE casa_nuptake(veg,casabiome,casapool,casaflux,casamet)
...
!
  ...
  IF(casamet%iveg2(np)/=icewater) THEN 
    ...
    xnuptake(np,leaf) = Nreqmin(np,leaf) + (Nreqmax(np,leaf)-Nreqmin(np,leaf))     &
                      *  casapool%Nsoilmin(np)/(casapool%Nsoilmin(np)+casabiome%kminN(veg%iveg(np)))
    xnuptake(np,wood) = Nreqmin(np,wood) + (Nreqmax(np,wood)-Nreqmin(np,wood))     &
                      *  casapool%Nsoilmin(np)/(casapool%Nsoilmin(np)+casabiome%kminN(veg%iveg(np)))
    xnuptake(np,froot) = Nreqmin(np,froot) + (Nreqmax(np,froot)-Nreqmin(np,froot))     &
                      *  casapool%Nsoilmin(np)/(casapool%Nsoilmin(np)+casabiome%kminN(veg%iveg(np)))
...
SUBROUTINE casa_puptake(veg,casabiome,casapool,casaflux,casamet)
...
!
  ...
  WHERE(casamet%iveg2/=icewater) 
    ...
    xpuptake(:,leaf) = Preqmin(:,leaf) + (Preqmax(:,leaf)-Preqmin(:,leaf))     &
                      *  casapool%Psoillab(:)/(casapool%Psoillab(:)+casabiome%KuplabP(veg%iveg(:)))
    xpuptake(:,wood) = Preqmin(:,wood) + (Preqmax(:,wood)-Preqmin(:,wood))     &
                      *  casapool%Psoillab(:)/(casapool%Psoillab(:)+casabiome%KuplabP(veg%iveg(:)))
    xpuptake(:,froot) = Preqmin(:,froot) + (Preqmax(:,froot)-Preqmin(:,froot))     &
                      *  casapool%Psoillab(:)/(casapool%Psoillab(:)+casabiome%KuplabP(veg%iveg(:)))
...
!      PRINT *, 'np, casamet%glai(np) = ', np, casamet%glai(np)
