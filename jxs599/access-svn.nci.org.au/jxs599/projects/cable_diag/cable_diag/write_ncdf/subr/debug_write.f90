...
module debug_write_mod
   ...
   subroutine write_ncdf_template( ncfile, nlat, nlon, ntile, ntime, nvar, lat, lon, tile, &
         timestep, newvar, varname )
      use netcdf
      ...
      character(len=30), dimension(num_vars) :: var_name
      ...
      integer :: ncok      !ncdf return status
         ...
         ncok = NF90_CREATE(path = ncfile, cmode = nf90_noclobber, ncid = ncid)
            ...
            ncok = NF90_PUT_ATT(ncid, varID(5), "missing_value", "-2000000000.")
                if (ncok /= nf90_noerr ) call stderr_nc('def att ', "missing_value")      
            
            ncok = NF90_PUT_ATT(ncid, varID(5), "_FillValue", -2000000000.)
                if (ncok /= nf90_noerr ) call stderr_nc('def att ', "missing_value")      
            
            ncok = NF90_PUT_ATT(ncid, varID(5), "valid_min", "0.")
                if (ncok /= nf90_noerr ) call stderr_nc('def att ', "missing_value")      
            
            ncok = NF90_PUT_ATT(ncid, varID(5), "valid_max", "400.")
                if (ncok /= nf90_noerr ) call stderr_nc('def att ', "missing_value")      
