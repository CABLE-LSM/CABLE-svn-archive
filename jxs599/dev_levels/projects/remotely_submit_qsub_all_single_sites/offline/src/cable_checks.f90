...
!      END DO
  ...
  !jhan: for testing purposes only
      call cable_diag( 1, 'wbal_tot', mp, kend_gl, ktau_gl,  & 
            knode_gl, 'wbal_tot',  bal%wbal_tot)
  ...
  SUBROUTINE energy_balance(dels,met,rad,canopy,bal,ssoil)
    ...
    bal%ebal_cncheck = SUM(rad%qcan(:,:,1),2)+SUM(rad%qcan(:,:,2),2)+rad%qssabs &
         ...
         !& -canopy%fh                 ! removed bug (EK 1jul08)
  ...
  !jhan: for testing purposes only
      call cable_diag( 1, 'ebal_tot_cncheck', mp, kend_gl, ktau_gl,  & 
            knode_gl, 'ebal_tot_cncheck',  bal%ebal_tot_cncheck )
...
!   
  ...
  !jhan: for testing purposes only
      call cable_diag( 1, 'ebal_tot', mp, kend_gl, ktau_gl,  & 
            knode_gl, 'ebal_tot',  bal%ebal_tot)
