...
module canopy_module
   ...
   subroutine define_canopy(bal,rad,rough,air,met,dels,ssoil,soil,veg, canopy)
      ...
      !jhan: new data structure vars
      real(r_1), dimension(:), pointer :: old_cansto
      ...
      !if (call_number == 1) & 
         call allocate_canopy_types(i, o, io)
  
            
         !jhan:in UM interface. Mk3l to follow
         !this is in cable_driver ?   
         if( .NOT. cable_runtime%um) then
            allocate(old_cansto(mp))
            old_cansto =  io%canopy_cansto
         endif   

            
         ! BATS-type canopy saturation proportional to LAI:
         allocate(cansat(mp))
         cansat = i%canopy_canst1 * canopy%vlaiw
...
!           endif
     ...
     1       elseif (zscl(j) >= rough%zruffs(j) ) then
      ...
      canopy%delwc = canopy%cansto-old_cansto
      ...
      DEALLOCATE(gbhf, csx)
      DEALLOCATE(ghwet, old_cansto)

      
      !jhan: alloc and point to old struture
      call deallocate_canopy_types(i,o,io)
      ...
      integer :: j
         ...
         do j=1,mp
            ...
            if(ssoil%snowd(j) < 0.1 .and. canopy%fess(j) .gt. 0. ) then
               ...
               flower_limit = REAL(ssoil%wb(j,1),r_1)-soil%swilt(j)
               fupper_limit = max(0._r_2,flower_limit) * frescale(j)
               canopy%fess(j) = min(canopy%fess(j), fupper_limit )
               
               fupper_limit = REAL(ssoil%wb(j,1)-ssoil%wbice(j,1),r_1) * frescale(j)
               canopy%fess(j) = min(canopy%fess(j), fupper_limit)
            END if
      ...
      integer :: j
         ...
         ! Temporay fixer for accounting the reduction of soil evap due to freezing
            do j=1,mp
               ...
               if ( veg%iveg(j) == 14 .and. met%tk(j) >= tfrz + 5.) ssoil%wetfac(j) = 1.0
               if( veg%iveg(j) == 14 .and. met%tk(j) < tfrz + 5.) ssoil%wetfac(j) = 0.7
            enddo 
