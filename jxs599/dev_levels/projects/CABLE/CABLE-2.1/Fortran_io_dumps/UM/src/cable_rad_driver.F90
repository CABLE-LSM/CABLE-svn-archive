...
subroutine cable_rad_driver(                   &      
   ...
   use albedo_module, only : surface_albedo
   use cable_um_tech_mod, only : kblum_rad, um1, soil, ssoil, rad, &
                              veg, met, canopy
   use cable_um_init_subrs, only : update_kblum_radiation,  um2cable_met_rad, um2cable_lp 
   ...
   LAND_ALBEDO_CABLE &! Land albedo calculated by Cable
   ...
   real :: & 
   ALB_TILE(um1%LAND_PTS,um1%NTILES,4)          &! Land albedo calculated by Cable
   ,TSOIL_TILE(um1%LAND_PTS,um1%NTILES,um1%SM_LEVELS)&! Mean snow density  (or 1 layer)
   ,SNOW_TMP3L(um1%LAND_PTS,um1%NTILES,3)         ! Snow temperature (3 layer)
   ...
   real :: rad_vis(mp), rad_nir(mp), met_fsd_tot_rel(mp), rad_albedo_tot(mp) 
      ...
      if( cable_user%RUN_DIAG_LEVEL == 'BASIC' ) &    
         call cable_stat('cable_rad_driver')
      ...
      ssoil%tggsn(:,1) = pack(SNOW_TMP3L(:,:,1),um1%L_TILE_PTS)
      ssoil%tgg(:,1) = pack(TSOIL_TILE(:,:,1),um1%L_TILE_PTS)
      ...
      rad_vis = ((1.0-rad%fbeam(:,1))*rad%reffdf(:,1)+rad%fbeam(:,1)*rad%reffbm(:,1)) 
      rad_nir = ((1.0-rad%fbeam(:,2))*rad%reffdf(:,2)+rad%fbeam(:,2)*rad%reffbm(:,2)) 
