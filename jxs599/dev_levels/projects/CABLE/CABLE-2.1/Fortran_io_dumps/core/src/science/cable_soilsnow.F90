  ...
  SUBROUTINE snow_accum (dels,  canopy, met, ssoil, soil)
   use cable_common_module
    ...
    WHERE (canopy%precis > 0.0 .and. ssoil%isflag == 0)
      ...
      ssoil%ssdnn = ssoil%ssdn(:,1)
!jhan:Eva comments this out
!jh:      WHERE (canopy%precis > 0.0 .and. ssoil%tgg(:,1) < tfrz)
!jh:        ssoil%snowd = MAX(ssoil%snowd + canopy%precis, 0.0)
!jh:!        ssoil%tgg(:,1) = ssoil%tgg(:,1) + canopy%precis * hlf &
!jh:!                       & / REAL(ssoil%gammzz(:,1),r_1) + &
!jh:        ssoil%tgg(:,1) = ssoil%tgg(:,1) + canopy%precis *  &
!jh:              & ( hlf + cswat*(tfrz-ssoil%tgg(:,1)) ) /  &
!jh:              & ( REAL(ssoil%gammzz(:,1),r_1) + cswat*canopy%precis )  
!jh:        ! change density due to water being added 
!jh:        ssoil%ssdn(:,1) = MIN(max_ssdn, MAX(120.0, ssoil%ssdn(:,1) &
!jh:                        & * ssoil%osnowd / MAX(0.01, ssoil%snowd) &
!jh:                        & + rhowat * canopy%precis / MAX(0.01, ssoil%snowd)))
!jh:        WHERE (soil%isoilm /= 9) ssoil%ssdn(:,1) = MIN(450.0,ssoil%ssdn(:,1))
!jh:        canopy%precis = 0.0
!jh:        ssoil%ssdnn = ssoil%ssdn(:,1)
!jh:      END WHERE
...
!!jh#else        
!!jh         ssoil%tggsn(:,1) = ssoil%tggsn(:,1) + canopy%precis * hlf &
!!jh               * osm / (sgamm * ssoil%osnowd )
!!jh#endif
...
!                         & / (sgamm + cswat*canopy%precis*osm/ssoil%osnowd)
!!jh          ssoil%tggsn(:,2) = ssoil%tggsn(:,2) + canopy%precis * hlf &
!!jh               * osm / (sgamm * ssoil%osnowd )
!
...
!                        & / (sgamm + cswat*canopy%precis*osm/ssoil%osnowd)
!!jh          ssoil%tggsn(:,3) = ssoil%tggsn(:,3) + canopy%precis * hlf &
!!jh               * osm / (sgamm * ssoil%osnowd )
!
!
...
!    END WHERE
   ...
   !jhan: Eva uses fes -> fess
    WHERE (ssoil%snowd > 0.1)
      ssoil%evapsn = dels * canopy%fess / ( hl + hlf )
      xxx = ssoil%evapsn
      WHERE (ssoil%isflag == 0 .and. canopy%fess.gt.0.0) &
