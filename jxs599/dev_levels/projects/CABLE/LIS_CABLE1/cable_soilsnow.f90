...
MODULE soil_snow_module
  USE cable_dimensions, ONLY: r_1,r_2,i_d,mp_patch,ms,nrb
  USE physical_constants, ONLY: tfrz, sboltz, emsoil
  USE cable_types       
...
!    END IF
 ...
 END SUBROUTINE smoisturev
  ...
  !-------------------------------------------------------------------------
  SUBROUTINE soil_albedo (ssoil, soil)

    USE other_constants, only : albsat, albdry
    ...
    ! Local Variables
    INTEGER   :: ib
    REAL(r_1), DIMENSION(mp_patch)      :: inc
    REAL(r_1), DIMENSION(mp_patch, nrb) :: albsod,          & ! soil albedo (direct)
                                           albsoi             ! soil albedo (indirect)

    if (soil%compalbsoil(1) == 1) then
       DO ib = 1,2 ! Number of wavebands (vis, nir)
          inc = MAX(0.11-0.40*ssoil%wb(:,1), 0.)
          albsod(:,ib) = MIN(albsat(soil%soilcol,ib)+inc, albdry(soil%soilcol,ib))
          albsoi(:,ib) = albsod(:,ib)
       END DO

       ! alb. soil is the mean of direct and indirect albedo
       ssoil%calc_albsoil = 0.5*(albsod + albsoi)
    else
       ssoil%calc_albsoil = SPREAD(soil%albsoil,2,nrb)
    end if
...
! We now calculate albsoil from soil moisture and colour, so use calc_albsoil instead of albsoil
!    sfact = 0.68
...
!    ssoil%albsoilsn(:,1) = sfact * ssoil%albsoilsn(:,2)
    ...
    ! The default option was to compute albedo values for 2 wavebands from reading only 1 value
    ! With new calculation, we already have both values.
    if (soil%compalbsoil(1) == 0) then
       sfact = 0.68
       WHERE (ssoil%calc_albsoil(:,2) <= 0.14)
          sfact = 0.5
       ELSEWHERE (ssoil%calc_albsoil(:,2) > 0.14 .AND. ssoil%calc_albsoil(:,2) <= 0.20)
          sfact = 0.62
       END WHERE
       ssoil%albsoilsn(:,2) = 2.0 * ssoil%calc_albsoil(:,2) / (1.0 + sfact)
       ssoil%albsoilsn(:,1) = sfact * ssoil%albsoilsn(:,2)
    else
       ssoil%albsoilsn(:,2) = ssoil%calc_albsoil(:,2)
       ssoil%albsoilsn(:,1) = ssoil%calc_albsoil(:,1)
    end if
...
!    diff(:,0) = 0.0
  ...
  SUBROUTINE soil_snow(dt, ktau, soil, ssoil, veg, canopy, met, rad, air,prin)
    REAL(r_1), INTENT(IN)                    :: dt ! integration time step (s)
    INTEGER(i_d), INTENT(IN)                 :: ktau ! integration step number
    ...
    LOGICAL, INTENT(IN)                      :: prin ! for debugging prints
    INTEGER(i_d), PARAMETER  :: ntest = 0 !  for prints
    INTEGER(i_d)             :: k
!    REAL(r_2), DIMENSION(mp_patch):: sicefreeze
!    REAL(r_2), DIMENSION(mp_patch):: sicemelt
...
!    END IF
    ...
    ! Update soil albedo with soil moisture and soil colour
    CALL soil_albedo(ssoil, soil)
