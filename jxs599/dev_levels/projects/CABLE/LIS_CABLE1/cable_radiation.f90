...
CONTAINS
  ...
  SUBROUTINE init_radiation(met,rad,veg,canopy)
    ...
    REAL(r_1), DIMENSION(mp_patch,nrb) :: c1    ! sqrt(1. - taul - refl)
    REAL(r_1), DIMENSION(3)   :: cos3  ! cos(15 45 75 degrees)
    REAL(r_1), DIMENSION(mp_patch,nrb) :: rhoch ! canopy reflect'n black horiz leaves
                                       ! (6.19)
   ...
   !c1 = SQRT(1. - taul - refl)
   c1 = SQRT(1. - veg%tauleaf - veg%rholeaf)
   ...
   rad%rhocdf = SPREAD(2.0*gauss_w, 1, mp_patch)*rhoch * xk & 
        & / (xk + SPREAD(rad%extkd, 2, 3))
...
!  SUBROUTINE radiation(ssoil, veg, air, met, rad, canopy)
  SUBROUTINE radiation(ssoil, veg, air, met, rad, canopy, prin)
    ...
    DO b = 1, 2 ! 1 = visible, 2 = nir radiaition
       WHERE (mask) ! i.e. vegetation and sunlight are present
          ...
          rad%qcan(:,1,b) = 0.5 * met%fsd * ( &  ! scale to real sunlit flux
               ...
               + rad%fbeam*(1.0-veg%tauleaf(:,b)-veg%rholeaf(:,b))*rad%extkb &
