...
! July 2012, C. Carouge : Add leaf transmittance and reflectance
!                         with veg. type dependance.
...
MODULE cable_types
  ...
  TYPE soil_parameter_type
     ...
     INTEGER, DIMENSION(:), POINTER :: compalbsoil ! soil reflectance
     ...
     INTEGER(i_d), DIMENSION(:), POINTER :: soilcol   ! Soil colour
  ...
  TYPE soil_snow_type 
     ...
     REAL(r_1), DIMENSION(:,:), POINTER :: calc_albsoil ! soil albedo with dependence 
                                                        ! on soil moisture and colour
  ...
  TYPE veg_parameter_type
     ...
     REAL(r_1), DIMENSION(:,:), POINTER :: rholeaf  !leaf reflectance
     REAL(r_1), DIMENSION(:,:), POINTER :: tauleaf  !leaf transmittance
...
CONTAINS
  ...
  SUBROUTINE alloc_soil_parameter_type(var,landunits,model_structure)
    ...
    ALLOCATE ( var % soilcol(landunits) )
  ...
  SUBROUTINE alloc_soil_snow_type(var,landunits,model_structure)
    ...
    ALLOCATE ( var % calc_albsoil(landunits,nrb) )
  ...
  SUBROUTINE alloc_veg_parameter_type(var,landunits,model_structure)
    ...
    END IF
    ! Add leaf transmittance and reflectance from veg. parameters
    ALLOCATE ( var % rholeaf(landunits,nrb) )
    ALLOCATE ( var % tauleaf(landunits,nrb) )
  ...
  SUBROUTINE dealloc_veg_parameter_type(var,landunits,model_structure)
    ...
    END IF
    ! Deallocate leaf transmittance and reflectance
    NULLIFY ( var % rholeaf )
    NULLIFY ( var % tauleaf )
