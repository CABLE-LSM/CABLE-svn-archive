...
PROGRAM texttonc
  ...
  !======================================================================
  INTEGER, PARAMETER  :: mt = 17520   ! # met forcing timesteps      ! BP
  CHARACTER(LEN=*), PARAMETER :: filein = 'TumbaDataiia.csv'          ! BP
  CHARACTER(LEN=*), PARAMETER :: fileout = 'Tumbarumba2005-06.nc'    ! BP
  CHARACTER(LEN=33), PARAMETER :: timeunits= &
                                  & 'seconds since 2005-01-01 00:00:01' ! BP
  CHARACTER(LEN=33), PARAMETER :: timestpunits= &
                                  & 'timesteps since 2005-01-01 00:00:01'
  ...
  CHARACTER(LEN=*), PARAMETER :: datalength='three of the 6.5 years'
  CHARACTER(LEN=*), PARAMETER :: contact='Bernard.Pak@csiro.au'
  ...
  TYPE par_type
     INTEGER :: isoil(1,1) = 2
     INTEGER :: iveg(1,1) = 2   ! IGBP #2 or CASA #1
  ...
  ! skipping 2002 to 2004 data
  DO j=1, 26280
     READ (unitin, *, IOSTAT = ios)
  END DO
  IF (ios /= 0) WRITE(*,*) 'Error reading 2002-4 data in ', filein
...
!     END IF
     IF (flst%Rnet(1,1,i) < -3500.0) THEN ! BP to fix very negative data
       ...
       flst%Rnet(1,1,i) = 0.0
     END IF
     time(i)=timestp(i)*REAL(timestepsize)
     eof = i==mt.AND.ios==0
     IF (eof) THEN
        CLOSE(unitin)
     ELSE
...
!       & print *, '1-3-04, i = ', i
  ...
  print *, flst%Qle(1,1,1:20)
  print *, ' '

  ! BP added this section to read in Tumbarumba daily LAI and interpolate to hourly
  OPEN (UNIT=14, FILE='tumb_lai.txt', FORM='formatted')
!  OPEN (UNIT=14, FILE='tumb02_std_input', FORM='formatted')
!  DO j=1, 110            ! this loop is for file tumb02_std_input
!     READ (14, *)
!  END DO
  DO j = 1, 365
     READ (14, *)  tempLAI(j)   ! read daily value from tumb_lai.txt
!     READ (14, *) i, tempLAI(j)   ! read daily value
  END DO
...
!  print *, 'i value after first year assignment: ', i
  DO j = 1, 365
    DO i = (j-1)*24+1, j*24
      DO bp1 = 0, 1
        met%LAI(1,1,bp1*8760+i) = tempLAI(j)
...
!     END IF
  ...
  status = NF90_DEF_DIM(ncid, 'x', 10, xID)
  IF (status /= NF90_NOERR) CALL handle_err(status)
  status = NF90_DEF_DIM(ncid, 'y', 10, yID) 
  IF (status /= NF90_NOERR) CALL handle_err(status)
  ...
  status = NF90_DEF_VAR(ncid,'nav_lon',NF90_FLOAT,(/xID/),nav_lonID)
  IF (status /= NF90_NOERR) CALL handle_err(status)
  status = NF90_PUT_ATT(ncid,nav_lonID,'long_name','Longitude')
  status = NF90_PUT_ATT(ncid,nav_lonID,'units','degrees_east')
  status = NF90_DEF_VAR(ncid,'nav_lat' ,NF90_float,(/yID/),nav_latID)
