...
module cable_diag_module
   ...
   interface cable_diag
      module procedure cable_diag1, cable_diag2, cable_diag3
...
!      integer(i_d) :: frecl
  ...
  11 format(i2)   
   ...
   subroutine put_var(ncfile_in, var_name, var, n_call, inst)
      use netcdf
      implicit none
      character(len=*), intent(in) :: ncfile_in, var_name
      real, dimension(:),intent(in) :: var
      integer, intent(in) :: n_call, inst
      ...
      ncok = NF90_OPEN(ncfile_in, nf90_write, ncid)           
         ...
         ncok = NF90_PUT_VAR(ncid, varId, var, start=(/1,1,n_call, inst/) )
