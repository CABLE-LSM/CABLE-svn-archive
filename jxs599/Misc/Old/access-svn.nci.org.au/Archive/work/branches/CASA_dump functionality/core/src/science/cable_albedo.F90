...
!#endif                  
   ...
   SUBROUTINE surface_albedosn(ssoil, veg, met, soil)
      ...
      TYPE (met_type),INTENT(INOUT)       :: met
      TYPE(soil_parameter_type), INTENT(INOUT) :: soil   
      REAL, DIMENSION(mp)    :: alv ! Snow albedo for visible
      REAL, DIMENSION(mp)    :: alir ! Snow albedo for near infra-red
      REAL, PARAMETER        :: alvo  = 0.95 ! albedo for vis. on a new snow
      REAL, PARAMETER        :: aliro = 0.70 ! albedo for near-infr. on a new snow
      REAL, DIMENSION(mp)    :: ar1 ! crystal growth  (-ve)
      REAL, DIMENSION(mp)    :: ar2 ! freezing of melt water
      REAL, DIMENSION(mp)    :: ar3
      REAL, DIMENSION(mp)    :: dnsnow ! new snow albedo
      REAL, DIMENSION(mp)    :: dtau
      REAL, DIMENSION(mp)    :: fage !age factor
      REAL, DIMENSION(mp)    :: fzenm
      REAL, DIMENSION(mp)    :: sfact
      REAL, DIMENSION(mp)    :: snr
      REAL, DIMENSION(mp)    :: snrat
