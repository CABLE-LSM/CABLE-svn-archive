...
MODULE define_types
  ...
  USE define_dimensions, only : i_d, r_1, r_2, ms, msn, mf, nrb, ncp, ncs, swb
  implicit none
...
!jhan:}
 ...
 !jhan:Eva added these 4 vars. ome are for lkaes?
...
!jhan:prev. def. in UM only
  ...
  TYPE canopy_type
     REAL(r_1), DIMENSION(:), POINTER :: fess   ! latent heatfl from soil (W/m2)
...
CONTAINS
   ...
   SUBROUTINE alloc_soil_snow_type(var, mp)
    ...
    ALLOCATE ( var % dtmlt(mp,3) )
         ...
         allocate( var%tilefrac(mp,JHNTILES) ) !#def JHNTILES in cable_directives.h    
  ...
  SUBROUTINE dealloc_soil_snow_type(var, mp)
    ...
    deALLOCATE ( var % dtmlt )
         ...
         deallocate( var%tilefrac ) !#def JHNTILES in cable_directives.h    
