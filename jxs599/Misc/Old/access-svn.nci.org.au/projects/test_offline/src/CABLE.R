...
for(site in 1:length(obsfiles)) { # for each site
   ...
   cnlist[5] = '  filename%restart_in  = \'/home/srb001/CABLE-AUX/data/restart_out.nc \''
   ...
   cnlist[34] = '  l_casacnp     = .TRUE.  ! using casaCNP with CABLE '
   ...
   #Method for saturation heat calc 
   cnlist[69] = '  cable_user%SSNOW_POTEV= \'HDM\'      ! Humidity Deficit Method' 
   #cnlist[69] = '  cable_user%SSOIL_POTEV= \'P-M\'      ! Penman-Monteith Method' 
   cnlist[70] = '&end'
...
#jhan:hardwired hack      
      ...
      file.rename('../../log_cable.txt','log_cable.txt')
      # restart file generated from run
      file.rename('../../restart_out.nc','restart_out.nc')
      # make a copy of the namelist file
      file.rename('../../cable.nml','cable.nml')
      # copy the files from CASA-CNP, if generated
      file.rename('../../poolcnpOut.csv','poolcnpOut.csv')
      file.rename('../../cnpfluxOut.csv','cnpfluxOut.csv')
