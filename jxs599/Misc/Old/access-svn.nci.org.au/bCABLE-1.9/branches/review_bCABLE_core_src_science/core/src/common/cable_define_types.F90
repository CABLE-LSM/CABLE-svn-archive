MODULE define_types
   ...
   USE define_dimensions, ONLY : r_2, ms, msn,                       &
                                 mf, nrb, ncp, ncs, swb, n_tiles
   ...
   TYPE soil_snow_type 
     ...
     INTEGER, DIMENSION(:), POINTER :: isflag ! 0 => no snow 1 => snow
      ...
      REAL, DIMENSION(:), POINTER ::                                           &
         ...
         dfn_dtg, & ! d(canopy%fns)/d(ssoil%tgg)
         dfh_dtg, & ! d(canopy%fhs)/d(ssoil%tgg)
         dfe_ddq, & ! d(canopy%fes)/d(dq)
         ddq_dtg, & ! d(dq)/d(ssoil%tgg)
