...
PROGRAM texttonc
  ...
  !======================================================================
  INTEGER, PARAMETER  :: mt = 17520   ! # met forcing timesteps
  CHARACTER(LEN=*), PARAMETER :: filein = '2002_2003.txt'            ! BP
  CHARACTER(LEN=*), PARAMETER :: fileout = 'Tumbarumba_met_lai.nc'   ! BP
  CHARACTER(LEN=33), PARAMETER :: timeunits='seconds since 2002-01-01 00:01:00'
  CHARACTER(LEN=33), PARAMETER :: timestpunits= &
                                  & 'timesteps since 2001-01-01 00:01:00'
  ...
  CHARACTER(LEN=*), PARAMETER :: datalength='Two years'
  ...
  TYPE units_type
     ...
     CHARACTER(LEN=12):: NEE='umol/m^2/s^2'
  ...
  INTEGER :: lineskip=0 ! Skip how many lines in text file
...
!     READ (unitin, *) i, tempLAI(j)   ! read daily value
     DO i = (j-1)*24+1, j*24          ! replicate for every hour in that day
        met%LAI(1,1,i) = tempLAI(j)
        met%LAI(1,1,8760+i) = tempLAI(j)
     END DO
  END DO
  CLOSE(unitin)
  ! BP end of addition
