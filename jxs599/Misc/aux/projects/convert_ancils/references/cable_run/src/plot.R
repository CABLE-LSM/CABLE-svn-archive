...
for(i in 1:length(sitenames) ) { 
   ...
   cday <- cdaily_data( rdata ) 
   cday_patch <- cdaily_data_patches( rdata, cday$ndays ) 
   oday <- odaily_data( rdata ) 
   if( oldFLAG) {
      cdayB <- cdaily_dataB( rdata ) 
      cday_patchB <- cdaily_dataB_patches( rdata, cday$ndays ) 
   }
   ###############################################
   ...
   for( ii in 1:length(plottypes) ) {
      ...
      if( plottypes[ii]=='diurnal')
         digof[i,,] <- diurnal( sitenames[i], rdata, cday, oday,cdayB, cday_patch,
                           cday_patchB,outtype, flux )
      #####################################     
      ###    mean monthly fluxes       ####     
      #####################################     
      if( plottypes[ii]=='seasonal')
         seasgof[i,,] <- seasonal( sitenames[i], rdata, cday, oday, cdayB, cday_patch,
                           cday_patchB,outtype, flux )
      #####################################     
      ###     fluxes windowed on        ###
      ###      different scales         ###     
      ...
      if( plottypes[ii]=='timeseries')
         timeseries(sitenames[i],cablefile,outtype,-1,-1,rdata$Lcdata$npatch,rdata$Lcdata$patchfrac) 
   }
   if( oldFLAG ) rm ( rdata, cday, oday, cdayB, cday_patch, cday_patchB )
   else rm ( rdata, cday, oday, cday_patch )
