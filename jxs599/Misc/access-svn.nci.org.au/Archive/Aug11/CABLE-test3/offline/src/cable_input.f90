...
CONTAINS
  ...
  ! no more interpolation or translation from CCAM grid (BP apr2010)
    ...
    IF(ok/=NF90_NOERR)  ok = NF90_INQ_DIMID(ncid,'longitude',xID)
    ...
    IF(ok/=NF90_NOERR)  ok = NF90_INQ_DIMID(ncid,'latitude',yID)
    ...
    IF(ok/=NF90_NOERR)  ok = NF90_INQ_DIMID(ncid,'month',tID)
