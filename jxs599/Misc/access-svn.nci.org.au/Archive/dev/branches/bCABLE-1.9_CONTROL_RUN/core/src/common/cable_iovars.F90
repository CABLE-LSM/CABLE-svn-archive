...
MODULE io_variables
  ...
  INTEGER,DIMENSION(12) :: daysm = &
       (/31,28,31,30,31,30,31,31,30,31,30,31/)
  INTEGER,DIMENSION(12) :: daysml = &
       (/31,29,31,30,31,30,31,31,30,31,30,31/)
  INTEGER,DIMENSION(12) :: lastday = &
       (/31,59,90,120,151,181,212,243,273,304,334,365/)
  INTEGER,DIMENSION(12) :: lastdayl = &
  ...
  INTEGER :: xdimsize,ydimsize ! sizes of x and y dimensions
  INTEGER :: ngridcells ! number of gridcells in simulation
  TYPE patch_type ! For vegetated surface type
     REAL :: frac ! fractional cover of each veg patch
     REAL :: latitude
     REAL :: longitude
  END TYPE patch_type
  TYPE land_type  
     INTEGER :: nap ! number of active (>0%) patches (<=max_vegpatches)
     INTEGER :: cstart ! pos of 1st gridcell veg patch in main arrays
     INTEGER :: cend ! pos of last gridcell veg patch in main arrays
     INTEGER :: ilat ! replacing land_y  ! ??
     INTEGER :: ilon ! replacing land_x  ! ??
  END TYPE land_type
  ...
  INTEGER :: nmetpatches ! size of patch dimension in met file, if exists
  ...
  TYPE gswp_type
     CHARACTER(LEN=99) :: rainf
     CHARACTER(LEN=99) :: snowf
     CHARACTER(LEN=99) :: LWdown
     CHARACTER(LEN=99) :: SWdown
     CHARACTER(LEN=99) :: PSurf
     CHARACTER(LEN=99) :: Qair
     CHARACTER(LEN=99) :: Tair
     CHARACTER(LEN=99) :: wind
  END TYPE gswp_type
  TYPE(gswp_type)      :: gswpfile
  INTEGER :: ncciy    ! year number (& switch) for gswp run
  INTEGER :: ncid_rin ! input netcdf restart file ID
  ...
  LOGICAL :: redistrb    ! Turn on/off the hydraulic redistribution
  REAL :: wiltParam
  REAL :: satuParam

  ! ================ Veg and soil type variables ====================================
  INTEGER,POINTER :: soiltype_metfile(:,:) ! user defined soil type (from met file)
  INTEGER,POINTER :: vegtype_metfile(:,:) ! user-def veg type (from met file)
! added mvtype and mstype to define_dimensions (BP sep2010)
!  INTEGER :: mvtype ! # of vegetation types in veg classification scheme
!  INTEGER :: mstype ! # of soil types in soil classification scheme
  ...
  TYPE output_inclusion_type
    ...
    INTEGER :: interval ! in case of 'user6' above, interval will be 6
...
!    LOGICAL :: rootbeta = .FALSE.  !
  ...
  REAL,POINTER,DIMENSION(:,:)  :: defaultLAI ! in case met file/host model has no LAI
