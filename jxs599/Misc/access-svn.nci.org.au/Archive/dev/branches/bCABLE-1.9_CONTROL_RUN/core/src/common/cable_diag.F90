module cable_diag_module
  ...
  11 format(i2)   
   ...
   subroutine put_var_ncr1(ncid, var_name, var, mp, n_call )
      use netcdf
      implicit none
      character(len=*), intent(in) ::  var_name
      real(r_2), dimension(:),intent(in) :: var
      integer, intent(in) :: ncid, n_call, mp
      integer :: ncok, varID,j
         ...
         do j=1,mp           
            ncok = NF90_PUT_VAR(ncid, varId, var(j), start=(/1,1,n_call /), &
                                 count=(/1,1,1,1/) )
   ...
   subroutine put_var_ncr2(ncid, var_name, var, mp, n_call)
      use netcdf
      implicit none
      character(len=*), intent(in) :: var_name
      real(r_2), dimension(:,:),intent(in) :: var
      ...
      integer :: ncok, varID ,j 
         ...
         do j=1,mp           
            ncok = NF90_PUT_VAR(ncid, varId, var(j), start=(/1,1,1,n_call /), & 
                                 count=(/1,1,1,6,1/))
               if (ncok /= nf90_noerr ) call stderr_nc('putting var ', var_name)      
         enddo
