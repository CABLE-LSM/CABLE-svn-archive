...
PROGRAM offline_driver
  ...
  LOGICAL    :: CASA_dump_read = .false. 
  LOGICAL    :: CASA_dump_write = .false. 
...
!**YP - CASA switch here **
! if CASA_dump_read is false  then  the code expects just to run CABLE. If you
! want to write CASA_dump.nc in this case then you have to re-set 
! CASA_dump_write=.true. 
! by uncommenting above

! if you re-set 
! CASA_dump_read=.true. 
! by uncommenting above

!then the code expects just to run CASA from forcing. 
! you cant write CASA_dump.nc in this case 

!i will set these from namelist variables soon

! search code for CASA_dump_read(write) to see where i've drawn lines. 
! NB - these may not be correct!!



if ( .NOT. CASA_dump_read ) then 
...
if ( CASA_dump_read ) then 
   if(mod((ktau-kstart+1),ktauday)==0) then
      ...
      IF (l_vcmaxFeedbk) CALL casa_feedback(ktau_gl,veg,casabiome,casapool, &
                                            casamet)
      ...
      if(icycle >0) then
        call bgcdriver(ktau_gl,kstart,kend,dels,met,ssoil,canopy,veg,soil, &
                  casabiome,casapool,casaflux,casamet,casabal,phen, spinConv, &
                  ktauday, idoy, CASA_dump_read, CASA_dump_write )
         !spinConv = .true.
      endif 
