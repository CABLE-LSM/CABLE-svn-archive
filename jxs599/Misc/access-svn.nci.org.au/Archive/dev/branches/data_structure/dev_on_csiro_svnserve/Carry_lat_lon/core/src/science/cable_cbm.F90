...
!jh! commented as vars not around in this version - reconcile with casaCnp vn 
!jh      if (icycle==0) then
!jh!      calculate canopy%frp
!jh       CALL plantcarb(veg,bgc,met,canopy)
!jh!      calculate canopy%frs
!jh       CALL soilcarb(soil, ssoil, veg, bgc, met, canopy)
!jh
!jh! rml - why was this here, is it needed??
!jh!        rad%flws = sboltz*emsoil* ssoil%tss **4
!jh       CALL carbon_pl(dels, soil, ssoil, veg, canopy, bgc)
!jh
!jh       canopy%fnpp = -1.0* canopy%fpn - canopy%frp
!jh       canopy%fnee = canopy%fpn + canopy%frs + canopy%frp
!jh
!jh
!jh   endif
   endif
