...
!++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
   ...
   SUBROUTINE define_air_cable(met,air)
      ...
      !call dump_air_in_ncdf(i(1), 17520, n_call)
      !call read_air_in(i(1))
      call read_air_in_ncdf(i(1), n_call)
...
!++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
   ...
   subroutine dump_air_in_ncdf(i, kend,n_call)
      use netcdf
      use cable_data_module, only : air_in
      implicit none   
      type (air_in), intent(in) :: i
      integer, intent(in) :: kend, n_call

      integer:: ncid       ! netcdf file ID
      integer :: ncok      !ncdf return status
      integer :: l1, l2    !length of dimension 
      integer, parameter :: num_vars=2 
      integer, parameter :: num_dims=3 
      integer :: xID, yID, tID 
      !var IDs 
      integer, dimension(num_vars), :: varID ! (1) ~ tvair, (2) ~ pmb 
      integer, dimension(num_dims)  :: dim_len = (/1,1,kend/)  ! (1) ~ x, (2) ~ y, (3) time 
      integer, dimension(num_dims)  :: dimID   ! (1) ~ x, (2) ~ y, (3) time 
      !var names 
      character(len=*), dimension(num_vars), parameter :: var_name =  (/   "met_tvair", &
                                                                           "met_pmb  " /)
      !dim names 
      character(len=*), dimension(num_dims), parameter :: dim_name =  (/   "lat ", &
                                                                           "lon ", &
                                                                            "time" /)
      
      if (n_call == 1) then
         ! create netCDF dataset: enter define mode
         ncok = nf90_create(path = "air_in.nc", cmode = nf90_noclobber, ncid = ncid)
         if (ncok /= nf90_noerr) stop 'ERROR: ncdf creating air_in.nc' 
            ...
            call def_dims(nd, ncid, dimID, dim_name )
            !ncok = nf90_def_dim(ncid, "Lat", l1, xID)
            !   if (ncok /= nf90_noerr ) stop 'ERROR: ncdf deining air_out dimensions' 
            !ncok = nf90_def_dim(ncid, "Lon", l2, yID)
            !   if (ncok /= nf90_noerr ) stop 'ERROR: ncdf deining air_out dimensions' 
            !ncok  = nf90_def_dim(ncid, "time", kend, tId)
            !   if (ncok /= nf90_noerr ) stop 'ERROR: ncdf deining air_out dimensions' 
      
            ! define variables: from name, type, dims
            call def_vars(num_vars, ncid,  nf90_float, (/xID,yID,tID/), var_name, varID )
       
            ! define variable attributes
            call def_var_atts(ncid, varID )
               
            ncok = nf90_enddef(ncid) 
         ncok = nf90_close(ncid)            ! close: save new netCDF dataset
      endif 

      ncok = nf90_open("air_in.nc",nf90_write, ncid)           
         ncok = nf90_inq_varid(ncid, "met_tvair", varId )
         ncok = nf90_put_var(ncid, varId, i%met_tvair, start=(/1,1,n_call/) )
         ncok = nf90_inq_varid(ncid, "met_pmb", varId )
         ncok = nf90_put_var(ncid, varId, i%met_pmb, start=(/1,1,n_call/) )
      ncok = nf90_close(ncid)            ! close: save new netCDF dataset
   
      return 
   end subroutine dump_air_in_ncdf
   ...
   subroutine read_air_in_ncdf(i, n_call)
      use netcdf
      use cable_data_module, only : air_in
      implicit none   
      type (air_in), intent(out) :: i
      integer, intent(in) :: n_call
      integer:: ncid, grp_ncid  ! netcdf file ID
      integer :: ncok, ncok1, ncok2 !ncdf return status
      integer :: lend1, lend2 ! 
      integer :: xID,yID,tID, tvairID, pmbID    ! netcdf dimension IDs 
      integer :: j
      
      ncok = nf90_open("air_in.nc",nf90_write, ncid)           
         ncok = nf90_inq_varid(ncid, "met_tvair", tvairId )
         ncok = nf90_get_var(ncid, tvairId, i%met_tvair, start=(/1,1,n_call/) )
         ncok = nf90_inq_varid(ncid, "met_pmb", pmbId )
         ncok = nf90_get_var(ncid, pmbId, i%met_pmb, start=(/1,1,n_call/) )
      ncok = nf90_close(ncid)            ! close: save new netCDF dataset
   
      return 
   end subroutine read_air_in_ncdf
   ...
   subroutine def_dims(nd, ncid, dimID, dim_name )
      use netcdf
      implicit none 
      ...
      do j=1, nd
         ncok = nf90_def_dim(ncid, trim(dim_names(j)), dimID(j) )
         if (ncok /= nf90_noerr ) stop 'ERROR: ncdf deining air_in dimensions' 
      enddo
   ...
   subroutine def_var_atts(ncid, varID )
     use netcdf
     implicit none   
     integer:: ncid       ! netcdf file ID
     integer, dimension(num_vars), :: varID ! (1) ~ tvair, (2) ~ pmb 
