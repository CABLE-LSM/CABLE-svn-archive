...
!===============================================================================
!jh!subroutine cable_explicit_driver(  row_length, rows, land_pts, ntiles,npft,  & 
!jh!           sm_levels, timestep, latitude, longitude, land_index, tile_frac,  &
!jh!           tile_pts, tile_index, bexp, hcon, satcon, sathh, smvcst, smvcwt,  &
!jh!           smvccl, albsoil, snow_tile, snow_rho1l, snage_tile, isnow_flg3l,  &
!jh!           snow_rho3l, snow_cond, snow_depth3l, snow_tmp3l, snow_mass3l,     &
!jh!           sw_down, lw_down, cos_zenith_angle, surf_down_sw, ls_rain,        &
!jh!           ls_snow, tl_1, qw_1, vshr_land, pstar, z1_tq, z1_uv, rho_water,   &
!jh!           L_tile_pts, canopy_tile, Fland, CO2_MMR, sthu_tile, smcl_tile,    &
!jh!           sthf_tile, sthu, tsoil_tile, canht_ft, lai_ft, sin_theta_latitude,&
!jh!           dzsoil, LAND_MASK, FTL_TILE_CAB, FTL_CAB, FTL_TILE, FQW_TILE,     &
!jh!           LE_TILE_CAB, LE_CAB, TSTAR_TILE, TSTAR_TILE_CAB, TSTAR_CAB, U_S,  &
!jh!           U_S_STD_TILE, U_S_CAB, CH_CAB, CD_CAB, CD_TILE, CH_TILE,          &
!jh!           RADNET_TILE, FRACA, rESFS, RESFT, Z0H_TILE, Z0M_TILE,             &
!jh!           RECIP_L_MO_TILE, EPOT_TILE, endstep, timestep_number, mype )    
...
subroutine cable_explicit_driver(  row_length, rows, land_pts, ntiles,npft,  & 
                     sm_levels, timestep, endstep, timestep_number, mype, &     
                     latitude, longitude, land_index,       &
...
!      EPOT_TILE
   ...
   character(len=200), parameter ::   & 
         runtime_vars_file = '/home/599/jxs599/CABLE-UM/cable.nml'
...
!shape(  row_length, rows, land_pts, ntiles,npft,  & 
...
!           RECIP_L_MO_TILE, EPOT_TILE, endstep, timestep_number, mype )    
            ...
            open(unit=71,file='latlon.txt', status="replace",action="write" )
               write(71,*) shape(latitude)
               write(71,*) '' 
               write(71,*) shape(longitude)
               write(71,*) '' 
               write(71,*) shape(land_index)
               write(71,*) '' 
               write(71,*) shape(tile_frac)
               write(71,*) '' 
               write(71,*) shape(tile_pts)
               write(71,*) '' 
               write(71,*) shape(tile_index) 
               write(71,*) shape(bexp)
               write(71,*) shape(hcon)
               write(71,*) shape(satcon)
               write(71,*) shape(sathh)
               write(71,*) shape(smvcst)
               write(71,*) shape(smvcwt)
            close(71)
...
!         if smvccl,( first_cable_call)  then 
!            albsoil print *,'jhan:rowlength', row_length
!            print *,'jhan:rows', rows
!            open(unit=71,file='latlon.txt', status="replace",action="write" )
!            do j=1, row_length
!               do k=1, rows
!                  write(71,*), latitude(j,k), longitude(j,k)
!               enddo
!            enddo
!            close(71)
!         endif
