...
module canopy_module
   ...
   subroutine define_canopy(bal,rad,rough,air,met,dels,ssoil,soil,veg, canopy)
      ...
      REAL, DIMENSION(mp)            :: ortsoil ! turb. resist. prev t-step
      ...
      REAL, DIMENSION(mp) :: qstvair ! sat spec humidity at leaf temperature
      INTEGER :: iter ! iteration #
      INTEGER :: iterplus !

      REAL, DIMENSION(mp) :: xx ! delta-type func for sparse canopy limit, p20 SCAM manual
      REAL, DIMENSION(mp)  :: term1, term2, term3, term5
      REAL, DIMENSION(mp)  :: r_sc
      REAL, DIMENSION(mp)  :: zscl

      real, dimension(:), pointer :: cansat ! max canopy intercept. (mm)
      real, dimension(:), pointer :: dsx ! leaf surface vpd
      real, dimension(:), pointer :: fwsoil ! soil water modifier of stom. cond
      real, dimension(:), pointer :: tlfx ! leaf temp prev. iter (K)
      real, dimension(:), pointer :: tlfy ! leaf temp (K)
      ...
      REAL, DIMENSION(mp)            :: pwet
      REAL, DIMENSION(mp)            :: dq ! sat sp
      REAL, DIMENSION(mp)            :: xx1
      REAL, DIMENSION(mp)  :: sum_rad_rniso 

      !jhan:local temporary vars
      real, dimension(mp)  :: ftemp,z_eff,psim_arg, psim_1, psim_2, rlower_limit  
      real  :: rt_min
      real, dimension(mp)  :: sum_rad_gradis 
...
!            enddo 
   ...
   function Penman_Monteith( ground_H_flux ) result(ssoilpotev)
      ...
      real, dimension(mp)  :: ssoilpotev 
      REAL, DIMENSION(mp) :: sss ! var for Penman-Monteith soil evap
      REAL, DIMENSION(mp) :: cc1 ! var for Penman-Monteith soil evap
      REAL, DIMENSION(mp) :: cc2 ! var for Penman-Monteith soil evap
      REAL, DIMENSION(mp) :: qsatfvar
      integer :: j
   ...
   function Humidity_deficit_method(dq,qstss ) result(ssoilpotev)
      ...
      REAL, DIMENSION(mp)    :: qstss !dummy var for compilation 
      integer :: j
   ...
   subroutine latent_heat_flux() 
      use cable_common_module
      implicit none
      real, dimension(mp) ::  frescale
      real, dimension(mp) ::  flower_limit, fupper_limit
      integer :: j
...
subroutine within_canopy( gbhu, gbhf )
  implicit none
   ...
   real(r_2), intent(in), dimension(:,:):: gbhf ! freeConvectionBndryLayerCond
      ...
      !real, dimension(mp)  :: lower_limit, upper_limit
      integer :: j
...
!jhan:these are all NOT contained subrs in mk3l
    ...
    subroutine qsatfjh(var,tair,pmb) 
      REAL, INTENT(IN), dimension(mp) :: tair ! air temperature (C)
      REAL, INTENT(IN), dimension(mp) :: pmb  ! pressure PMB (mb)
    ...
    END subroutine qsatfjh2
     ...
     REAL, dimension(mp)                 :: z
     REAL, dimension(mp)                 :: stable
     REAL, dimension(mp)                 :: unstable
...
!      REAL                 :: ustzeta
  ...
  SUBROUTINE wetLeaf(dels,rad,rough,air,met,veg,canopy,cansat, tlfy, gbhu, gbhf,ghwet)
    ...
    TYPE (veg_parameter_type), INTENT(INOUT)    :: veg
    TYPE (canopy_type), INTENT(INOUT)   :: canopy
   real,intent(in), dimension(:) :: tlfy ! leaf temp (K)
   real(r_2), intent(in), dimension(:,:) :: gbhu ! forcedConvectionBndryLayerCond
   real(r_2), intent(in), dimension(:,:) :: gbhf ! freeConvectionBndryLayerCond
   ...
   ! assuming the temperature of wet leaf is equal that of dry leaf ="tlfy"
    ...
    REAL, DIMENSION(mp)  :: ccfevw ! limitation term for
                                        ! wet canopy evaporation rate
    REAL, DIMENSION(mp)  :: gwwet  ! cond for water for a wet canopy
    REAL, DIMENSION(mp)  :: ghrwet ! wet canopy cond: heat & thermal rad
    REAL, DIMENSION(mp)  :: sum_gbh, sum_rad_rniso, sum_rad_gradis 
    REAL, DIMENSION(mp)  :: xx1,xx2,xx3
    integer :: j
  ...
  END SUBROUTINE define_canopy
   ...
   SUBROUTINE dryLeaf(dels,rad,rough,air,met,veg,canopy, soil,ssoil, dsx,fwsoil, tlfx,  &
      ...
      type (soil_parameter_type), intent(inout)   :: soil
      type (soil_snow_type), intent(inout):: ssoil
      real, intent(inout), dimension(:) :: dsx ! leaf surface vpd
      real, intent(inout), dimension(:):: fwsoil ! soil water modifier of stom. cond
      real,intent(inout), dimension(:) :: tlfx ! leaf temp prev. iter (K)
      real,intent(inout), dimension(:) :: tlfy ! leaf temp (K)
      ...
      REAL, INTENT(IN)     :: dels ! integration time step (s)
      REAL, PARAMETER  :: co2cp3 = 0.0 ! CO2 compensation pt C3
      REAL, PARAMETER  :: jtomol = 4.6e-6 ! Convert from J to Mol for light
      REAL, DIMENSION(mp)  :: conkct ! Michaelis Menton const.
      REAL, DIMENSION(mp)  :: conkot ! Michaelis Menton const.
      REAL, DIMENSION(mp)  :: cx1  ! "d_{3}" in Wang and Leuning,
      REAL, DIMENSION(mp)  :: cx2  !     1998, appendix E
      REAL, DIMENSION(mp)  :: tdiff ! leaf air temp diff.
      REAL, DIMENSION(mp)  :: tlfxx ! leaf temp of current iteration (K)
      REAL, DIMENSION(mp)  :: abs_deltlf ! ABS(deltlf)
      REAL, DIMENSION(mp)  :: deltlf ! deltlfy of prev iter.
      REAL, DIMENSION(mp)  :: deltlfy ! del temp successive iter.
      REAL, DIMENSION(mp)  :: gras ! Grashof coeff
      REAL, DIMENSION(mp)  :: evapfb !
      ...
      REAL, DIMENSION(mp,mf)  :: gw  ! cond for water for a dry canopy
      REAL, DIMENSION(mp,mf)  :: gh  ! cond for heat for a dry canopy
      REAL, DIMENSION(mp,mf)  :: ghr ! dry canopy cond for heat & thermal rad
      REAL, DIMENSION(mp,mf)  :: anx ! net photos. prev iteration
      REAL, DIMENSION(mp,mf)  :: an_y ! net photosynthesis soln
      REAL, DIMENSION(mp,mf)  :: rdx ! daytime leaf resp rate, prev iteration
      REAL, DIMENSION(mp,mf)  :: rdy ! daytime leaf resp rate
      REAL, DIMENSION(mp,mf)  :: ejmax2 ! jmax of big leaf
      REAL, DIMENSION(mp,mf)  :: ejmxt3 ! jmax big leaf C3 plants
      REAL, DIMENSION(mp,mf)  :: vcmxt3 ! vcmax big leaf C3
      REAL, DIMENSION(mp,mf)  :: vcmxt4 ! vcmax big leaf C4
      REAL, DIMENSION(mp,mf)  :: vx3 ! carboxylation C3 plants
      REAL, DIMENSION(mp,mf)  :: vx4 ! carboxylation C4 plants
      REAL, DIMENSION(mp,mf)  :: xleuning ! leuning stomatal coeff
      REAL, DIMENSION(mp,mf)  :: psycst ! modified pych. constant
      REAL, DIMENSION(mp,mf)  :: temp2
      INTEGER   :: k  ! iteration count
      INTEGER   :: kk  ! iteration count
  ...
  SUBROUTINE photosynthesis(csxz,cx1z,cx2z,gswminz,rdxz,vcmxt3z,vcmxt4z, &
    ...
    REAL, DIMENSION(mp,mf), INTENT(IN) :: cx1z
    REAL, DIMENSION(mp,mf), INTENT(IN) :: cx2z
    REAL, DIMENSION(mp,mf), INTENT(IN) :: gswminz
    REAL, DIMENSION(mp,mf), INTENT(IN) :: rdxz
    REAL, DIMENSION(mp,mf), INTENT(IN) :: vcmxt3z
    REAL, DIMENSION(mp,mf), INTENT(IN) :: vcmxt4z
    REAL, DIMENSION(mp,mf), INTENT(IN) :: vx4z
    REAL, DIMENSION(mp,mf), INTENT(IN) :: vx3z
    REAL, DIMENSION(mp,mf), INTENT(IN) :: xleuningz
    REAL, DIMENSION(mp,mf), INTENT(IN) :: vlaiz
    REAL, DIMENSION(mp,mf), INTENT(IN) :: deltlfz
  ...
  END SUBROUTINE photosynthesis
    ...
    FUNCTION xvcmxt3(x) result(z)
      ...
      ! modifying input to single real - kdcorbin, 09/10
      !REAL, DIMENSION(mp), INTENT(IN)   :: x
      !REAL, DIMENSION(mp)               :: xvcnum
      !REAL, DIMENSION(mp)               :: xvcden
      !REAL, DIMENSION(mp)               :: z
