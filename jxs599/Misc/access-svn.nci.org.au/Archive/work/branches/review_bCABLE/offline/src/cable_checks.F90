...
!
  ...
  TYPE ranges_type 
    REAL, DIMENSION(2) :: nav_lon = (/-360.0,360.0/)   
    REAL, DIMENSION(2) :: nav_lat = (/-90.0,90.0/)   
    REAL, DIMENSION(2) :: time     
    REAL, DIMENSION(2) :: timestp      
    ! possible forcing variables for CABLE
    REAL, DIMENSION(2) :: SWdown = (/0.0,1360.0/)  ! W/m^2
    REAL, DIMENSION(2) :: LWdown = (/0.0,750.0/)   ! W/m^2
    REAL, DIMENSION(2) :: Rainf = (/0.0,0.03/)     ! mm/s
    REAL, DIMENSION(2) :: Snowf = (/0.0,0.0085/)   ! mm/s
    REAL, DIMENSION(2) :: PSurf = (/500.0,1100.0/) ! mbar/hPa
    REAL, DIMENSION(2) :: Tair = (/200.0,333.0/)   ! K
    REAL, DIMENSION(2) :: Qair = (/0.0,0.04/)      ! g/g
    REAL, DIMENSION(2) :: CO2air = (/160.0,2000.0/)! ppmv   
    REAL, DIMENSION(2) :: Wind = (/0.0,75.0/)      ! m/s
    REAL, DIMENSION(2) :: Wind_N = (/-75.0,75.0/)  ! m/s
    REAL, DIMENSION(2) :: Wind_E = (/-75.0,75.0/)  ! m/s
    ! possible output variables
    REAL, DIMENSION(2) :: Qh = (/-1000.0,1000.0/)    ! W/m^2
    REAL, DIMENSION(2) :: Qle = (/-1000.0,1000.0/)   ! W/m^2
    REAL, DIMENSION(2) :: Qg = (/-1000.0,1000.0/)    ! W/m^2   
    REAL, DIMENSION(2) :: SWnet = (/0.0,1350.0/)     ! W/m^2 (YP oct07)
!    REAL, DIMENSION(2) :: SWnet = (/0.0,1250.0/)     ! W/m^2
    REAL, DIMENSION(2) :: LWnet = (/-500.0,510.0/)   ! W/m^2 
    REAL, DIMENSION(2) :: Rnet = (/-500.0,1250.0/)   ! W/m^2 
    REAL, DIMENSION(2) :: Evap = (/-0.0003,0.00035/)      
    REAL, DIMENSION(2) :: Ewater = (/-0.0003,0.0003/)
    REAL, DIMENSION(2) :: ESoil = (/-0.0003,0.0003/)     
    REAL, DIMENSION(2) :: TVeg = (/-0.0003,0.0003/)    
    REAL, DIMENSION(2) :: ECanop = (/-0.0003,0.0003/)   
    REAL, DIMENSION(2) :: PotEvap = (/-0.0006,0.0006/)     
    REAL, DIMENSION(2) :: ACond = (/0.0,1.0/)    
    REAL, DIMENSION(2) :: SoilWet = (/-0.4,1.2/) 
    REAL, DIMENSION(2) :: Albedo = (/0.0,1.0/)    
    REAL, DIMENSION(2) :: VegT = (/213.0,333.0/)     
    REAL, DIMENSION(2) :: SoilTemp = (/213.0,343.0/)   
    REAL, DIMENSION(2) :: SoilMoist = (/0.0,2000.0/) 
    REAL, DIMENSION(2) :: Qs = (/0.0,5.0/)
    REAL, DIMENSION(2) :: Qsb = (/0.0,5.0/)
    REAL, DIMENSION(2) :: DelSoilMoist  = (/-2000.0,2000.0/) 
    REAL, DIMENSION(2) :: DelSWE  = (/-2000.0,2000.0/)       
    REAL, DIMENSION(2) :: DelIntercept = (/-100.0,100.0/)  
    REAL, DIMENSION(2) :: SnowT  = (/213.0,280.0/)        
    REAL, DIMENSION(2) :: BaresoilT = (/213.0,343.0/)     
    REAL, DIMENSION(2) :: AvgSurfT = (/213.0,333.0/)      
    REAL, DIMENSION(2) :: RadT = (/200.0,373.0/)         
    REAL, DIMENSION(2) :: SWE = (/0.0,2000.0/)           
    REAL, DIMENSION(2) :: RootMoist = (/0.0,2000.0/)     
    REAL, DIMENSION(2) :: CanopInt = (/0.0,100.0/)  
    REAL, DIMENSION(2) :: NEE = (/-70.0,50.0/) ! umol/m2/s
    REAL, DIMENSION(2) :: NPP = (/-20.0,75.0/) ! umol/m2/s 
    REAL, DIMENSION(2) :: GPP = (/-20.0,100.0/) ! umol/m2/s 
    REAL, DIMENSION(2) :: AutoResp = (/-50.0,20.0/) ! umol/m2/s
    REAL, DIMENSION(2) :: LeafResp = (/-50.0,20.0/) ! umol/m2/s
    REAL, DIMENSION(2) :: HeteroResp = (/-50.0,20.0/) ! umol/m2/s
    REAL, DIMENSION(2) :: HSoil = (/-1000.0,1000.0/) 
    REAL, DIMENSION(2) :: HVeg = (/-1000.0,1000.0/)
    REAL, DIMENSION(2) :: SnowDepth = (/0.0,50.0/) ! EK nov07
    REAL, DIMENSION(2) :: Wbal = (/-999999.0,999999.0/)
    REAL, DIMENSION(2) :: Ebal = (/-999999.0,999999.0/)
    ! parameters:
    REAL, DIMENSION(2) :: albsoil = (/0.0,0.9/)
    REAL, DIMENSION(2) :: isoil = (/1.0,30.0/)
    REAL, DIMENSION(2) :: iveg = (/1.0,30.0/)
    REAL, DIMENSION(2) :: bch = (/2.0,15.0/)  
    REAL, DIMENSION(2) :: latitude = (/-90.0,90.0/)
    REAL, DIMENSION(2) :: c3 = (/0.0,1.0/)  ! EK nov07   
    REAL, DIMENSION(2) :: clay = (/0.0,1.0/)  
    REAL, DIMENSION(2) :: css = (/700.0,2200.0/)         
    REAL, DIMENSION(2) :: rhosoil = (/300.0,3000.0/)    
    REAL, DIMENSION(2) :: hyds = (/5.0E-7,8.5E-4/)
    REAL, DIMENSION(2) :: rs20 = (/0.0,10.0/)
    REAL, DIMENSION(2) :: sand = (/0.0,1.0/)      
    REAL, DIMENSION(2) :: sfc = (/0.1,0.5/)        
    REAL, DIMENSION(2) :: silt = (/0.0,1.0/)
    REAL, DIMENSION(2) :: ssat = (/0.35,0.5/)      
    REAL, DIMENSION(2) :: sucs = (/-0.8,-0.03/)       
    REAL, DIMENSION(2) :: swilt = (/0.05,0.4/)
    REAL, DIMENSION(2) :: froot = (/0.0,1.0/) 
    REAL, DIMENSION(2) :: zse = (/0.0,5.0/)  
    REAL, DIMENSION(2) :: canst1 = (/0.05,0.15/)     
    REAL, DIMENSION(2) :: dleaf = (/0.005,0.4/)      
    REAL, DIMENSION(2) :: ejmax = (/1.0E-5,3.0E-4/) 
    REAL, DIMENSION(2) :: frac4 = (/0.0,1.0/)
    REAL, DIMENSION(2) :: hc = (/0.0,100.0/)         
    REAL, DIMENSION(2) :: lai = (/0.0,8.0/)
    REAL, DIMENSION(2) :: rp20 = (/0.0,10.0/)   
    REAL, DIMENSION(2) :: vbeta =(/-999999.0,999999.0/)
    REAL, DIMENSION(2) :: xalbnir = (/0.0,1.5/)
    REAL, DIMENSION(2) :: meth = (/0.0,1.0/)
    REAL, DIMENSION(2) :: za =(/0.0,150.0/)
!<<<<<<< .working
!    REAL, DIMENSION(2) :: rpcoef = (/0.05,0.15/)
!=======
    REAL, DIMENSION(2) :: rpcoef = (/0.05,1.5/)
!>>>>>>> .merge-right.r387
    REAL, DIMENSION(2) :: shelrb = (/1.0,3.0/)     
    REAL, DIMENSION(2) :: vcmax = (/5.0E-6,1.5E-4/)      
    REAL, DIMENSION(2) :: xfang = (/-1.0,0.5/)   
    REAL, DIMENSION(2) :: ratecp = (/0.01,3.0/)    
    REAL, DIMENSION(2) :: ratecs = (/0.01,3.0/)   
    REAL, DIMENSION(2) :: refsbare = (/0.0,0.5/) 
    REAL, DIMENSION(2) :: taul = (/0.0,0.3/)     
    REAL, DIMENSION(2) :: refl = (/0.0,0.5/)    
    REAL, DIMENSION(2) :: tauw = (/0.0,0.1/)     
    REAL, DIMENSION(2) :: refw = (/0.0,0.5/)
    REAL, DIMENSION(2) :: extkn = (/0.0,10.0/)    ! YP oct07
    REAL, DIMENSION(2) :: wai = (/0.0,5.0/)       ! YP oct07
    REAL, DIMENSION(2) :: vegcf = (/0.0,100.0/) ! YP oct07
    REAL, DIMENSION(2) :: tminvj = (/-20.0,15.0/)   
    REAL, DIMENSION(2) :: tmaxvj = (/-15.0,30.0/)
    REAL, DIMENSION(2) :: rootbeta = (/0.7,1.0/)  ! YP oct07
    REAL, DIMENSION(2) :: veg_class = (/1.0,20.0/)  
    REAL, DIMENSION(2) :: soil_class = (/1.0,20.0/)  
  END TYPE ranges_type
  TYPE(ranges_type),SAVE :: ranges
CONTAINS
  SUBROUTINE mass_balance(dels,ssoil,soil,canopy,met,air,bal)
...
! rml 28/2/11 ! BP changed rnof1+rnof2 to ssoil%runoff which also included rnof5
...
!          WRITE(*,*) 'Timestep:',ktau_gl, 'land point #:',j,'In mm:'
...
!      END DO
  ...
  SUBROUTINE energy_balance(dels,met,rad,canopy,bal,ssoil)
    USE physical_constants
    use cable_diag_module
    REAL,INTENT(IN)         :: dels ! time step size
    TYPE (canopy_type),INTENT(IN):: canopy ! canopy variable data
    ...
    bal%ebal_cncheck = SUM(rad%qcan(:,:,1),2)+SUM(rad%qcan(:,:,2),2)+rad%qssabs &
         ...
         & -sboltz*emsoil*ssoil%tss**4*rad%transd -canopy%fev-canopy%fes * ssoil%cls &
...
!   
    ...
    bal%ebal = (1.0-rad%albedo(:,1))*met%fsd(:,1) + (1.0-rad%albedo(:,2))*met%fsd(:,2)  &
         & +met%fld-sboltz*emleaf*canopy%tv**4*(1-rad%transd) &
         & -sboltz*emsoil*ssoil%tss**4*rad%transd -canopy%fev-canopy%fes * ssoil%cls &
         & -canopy%fh                 
    bal%ebal_tot = bal%ebal_tot + bal%ebal
...
!11  format(1x,'Ebal_cncheck, Ebal',f6.1,f8.0,2x,f6.1,f8.0)
  ...
  SUBROUTINE units_in(met,rad,dels)
    ...
    REAL,INTENT(IN) :: dels
    REAL :: temp_hum ! temporary humidity variable
