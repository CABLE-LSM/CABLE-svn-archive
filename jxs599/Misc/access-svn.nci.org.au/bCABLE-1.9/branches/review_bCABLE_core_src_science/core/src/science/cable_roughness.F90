...
SUBROUTINE ruff_resist(veg, rough, ssoil, soil, met, canopy)
   ...
   rough%hruff = MAX( 0.01, veg%hc - 1.2 * ssoil%snowd /                       &
                 MAX( ssoil%ssdnn, 100. ) ) 
   ...
   rough%z0soil = 1.e-6
   rough%z0soilsn = MAX( rough%z0soil - 0.5e-7 * MIN( ssoil%snowd, 20. ),      &
                    0.1e-7 )
   ...
   ELSEWHERE ! VEGETATED SURFACE
      ...
      rough%rt0us  = rough%term5 * ( zdlin * LOG(                              &
                     zdlin * rough%disp / rough%z0soilsn ) + ( 1 - zdlin ) )   &
                     * ( EXP( 2 * csw * canopy%rghlai )  -  rough%term2 )      &
                     / rough%term3  
