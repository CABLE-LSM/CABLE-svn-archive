...
!jhan: some points are legitimately 0 as in UM tile dim had a 0 there. Now arbitrarily activating that tile
         ...
         IF( um1%TILE_FRAC(i,N) .gt. 0.0 ) THEN
            ...
            ELSE IF(N > 4 .AND. N < 14 ) THEN !rml changed 3 to 4
             ...
             ELSE IF(N > 13 ) THEN
               ! non-vegetated
               kblum_veg%IVEGT(i,N) = N
! jhan: reverted. divide by zero still occurs later with doctored tile_frac 
! ancillary, AND can be avoided by setting LAI here >0, then rad%transd can
! enter WHERE loop where it is set away from default=1 (which leads to /0 error)
!for in canopy (/ (1-1) ). try resetting default of transddirectly in _radiation
...
SUBROUTINE um2cable_lp(umvar, defaultin, cablevar, soiltype, skip )
   ...
   INTEGER :: n,k,l,i
      ...
      fvar = 0.0
!jhan: incoming UM var has dims land_pts. 
!jhan: UM created tile_pts(N) says how many, N, (sub)cells of each type
!jhan: UM created tile_index gives index of Nth type into land_pts array
!jhan: e.g. land_pts(1)= first land point has 2 active tiles, 1 AND 2
!jhan: at this juncture then
!jhan: e.g. tile_pts(1)=1, tile_pts(2)=1, tile_pts(3:17)=0
!jhan: e.g. tile_index(tile_pts point=1,tile=1)=1 = index of land_pts array
!jhan: e.g. tile_index(tile_pts point=2,tile=2)=1 = index of land_pts array
!jhan: e.g. tile_index(tile_pts point=3,tile=1) = doesnt exist
      DO N=1,um1%NTILES
         DO K=1,um1%TILE_PTS(N)
            L = um1%TILE_INDEX(K,N)
            fvar(L,N) = umvar(L)
