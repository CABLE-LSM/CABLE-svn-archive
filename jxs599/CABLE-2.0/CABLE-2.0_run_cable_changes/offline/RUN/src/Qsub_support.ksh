...
cd src/
#module add R
# call script to create per node site details
R CMD BATCH --slave Qsub.R
cd ../

#nch refers to single/double/triple digit
if [[ $nproc -lt 10 ]]; then
   nch=1
...
# launches job per node by calling RUN_CABLE with no arguments.
# script hangs in here until you are allowed to launch a new job
