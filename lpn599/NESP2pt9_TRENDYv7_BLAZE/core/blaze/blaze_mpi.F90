...
SUBROUTINE worker_blaze_types(comm, mp, BLAZE, blaze_restart_t, blaze_out_t)
  ...
  CALL MPI_Type_create_struct (bidx, blocks, displs, types, restart_blaze_t, ierr)
  CALL MPI_Type_commit (restart_blaze_t, ierr)
  
  CALL MPI_Type_size (restart_blaze_t, tsize, ierr)
  CALL MPI_Type_get_extent (restart_blaze_t, tmplb, text, ierr)
  ...
  CALL MPI_Type_create_struct (bidx, blocks, displs, types, _t, ierr)
...
SUBROUTINE master_simfire_types(comm, wland, mp, SF, simfire_restart_ts, simfire_out_ts)
  ...
  INTEGER, DIMENSION(:), INTENT(OUT) :: simfire_restart_ts,simfire_out_ts 
...
!   REAL,     DIMENSION(:),  ALLOCATABLE :: RAINF, KBDI, LR, U10,RH
...
!  REAL,    DIMENSION(:), ALLOCATABLE  :: POPD, MAX_NESTEROV, CNEST, FAPAR, LAT, LON, FLI
   ...
   IF ( TRIM(SIMFIRE%OUTMODE) == "full" ) THEN 
...
SUBROUTINE worker_simfire_types(comm, mp, SF, simfire_restart_t, simfire_inp_t, simfire_out_t)
  ...
  INTEGER              , INTENT(IN)  :: mp   ! Number of gridcells
  TYPE(TYPE_SIMFIRE)   , INTENT(IN)  :: SF
  INTEGER, DIMENSION(:), INTENT(OUT) :: simfire_restart_t,simfire_inp_t,simfire_out_t
