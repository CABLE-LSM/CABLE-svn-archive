...
USE prognostics, ONLY: nsnow,sice,sliq,snowdepth,tsnow, ds
...
IF (lhook) CALL dr_hook('SF_EXPL_L_CABLE',zhook_in,zhook_handle)
...
! DEPENDS ON: generate_anthropogenic_heat
    CALL generate_anthropogenic_heat(                                         &
      curr_year, curr_day_number, curr_hour, curr_minute, curr_second,        &
      ntiles, land_pts, frac, l_anthrop_heat_src)
...
IF (lhook) CALL dr_hook('SF_EXPL_L_CABLE',zhook_out,zhook_handle)
