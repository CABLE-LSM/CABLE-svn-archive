...
IF (lhook) CALL dr_hook('SF_EXCH_CABLE',zhook_in,zhook_handle)
...
!CABLE{  Replaces land call to sf_flux
  DO k=1,tile_pts(n)
    l = tile_index(k,n)
    j = (land_index(l) - 1) / t_i_length + 1
    i = land_index(l) - (j - 1) * t_i_length
    ...
    ELSE IF( tl_1(i,j) > tm ) THEN
      alpha1(l,n) = cable% tmp% epsilon * lc * qs1(i,j) *                     &
         (1.0 + cable% tmp% c_virtual * qs1(i,j)) / (r * tl_1(i,j) * tl_1(i,j))
    ELSE
   ...
   ! this needs to be look at:
    ...
    IF( snow_tile(l,n) > 0.0 .AND. smvcst(l) /= 0.0 ) THEN
      ...
      IF( cable% tmp% ds_ratio <= 1.0 ) THEN
        ashtf_prime_tile(l,n) =  ashtf_prime_tile(l,n) /                      &
                    (1.0 + cable% tmp% ds_ratio * (hcons(l) / snow_hcon - 1.0))
      ELSE
    ...
    cable% tmp% lh = lc
    IF( snow_tile(l,n) > 0.0 ) cable% tmp% lh = ls
...
IF (iseasurfalg == ip_ss_surf_div) THEN
...
ELSE IF (iseasurfalg == ip_ss_fixed) THEN
...
IF (lhook) CALL dr_hook('SF_EXCH_CABLE',zhook_out,zhook_handle)
RETURN
