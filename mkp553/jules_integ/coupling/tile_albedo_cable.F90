...
SUBROUTINE tile_albedo_cable (                                    &
 ...
 z0_tile,alb_tile,land_albedo,albobs_sc                           &
 )
...
IF (lhook) CALL dr_hook('TILE_ALBEDO_CABLE',zhook_in,zhook_handle)
DO n=1,ntiles
  DO band=1,4
    DO l=1,land_field
      alb_tile(l,n,band) = 0.
        ...
        DO j=1,tile_pts(n)
          ...
          IF ( (alb_type(l,n,band) < 0.) .OR. (alb_type(l,n,band) > 1.) ) THEN
            WRITE(jules_message,'(a,F24.12)') &
                'ERROR: Albedo < 0.0 or > 1.0: ',alb_type(l,n,band)
            ...
            CALL ereport ('tile_albedo_cable',errcode,errmsg)
         END IF
        END DO
      END DO
    END DO
    ...
    !DO l=1,land_field
      DO j=1,tile_pts(n)
        ...
        IF ( (albsnf(l,n) < 0.) .OR. (albsnf(l,n) > 1.) ) THEN
          WRITE(jules_message,'(a,F24.12)') 'ERROR: Albedo < 0.0 or > 1.0: ', &
              albsnf(l,n)
          CALL jules_print('tile_albeno_jls',jules_message)
          ...
          CALL jules_print('tile_albeno_jls',jules_message)
          WRITE(jules_message,'(a,F24.12,F24.12,F24.12)') 'obs, model, soil:', &
