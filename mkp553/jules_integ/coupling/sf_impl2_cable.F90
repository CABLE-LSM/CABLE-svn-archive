...
USE jules_snow_mod,           ONLY: &
  nsmax, rho_snow_const, cansnowtile, l_snowdep_surf
...
USE fluxes,                   ONLY: &
  anthrop_heat, surf_ht_store, sw_sice
...
IF (lhook) CALL dr_hook('SF_IMPL2_CABLE',zhook_in,zhook_handle)
  ...
  IF (l_neg_tstar) THEN
    DO l=1,land_pts
      IF (t_soil(l,1) < 0) THEN
        error = 1
        WRITE(jules_message,*) '*** ERROR DETECTED BY ROUTINE SF_IMPL2_CABLE ***'
        CALL jules_print('sf_impl2_cable',jules_message)
...
IF (lhook) CALL dr_hook('SF_IMPL2_CABLE',zhook_out,zhook_handle)
RETURN
