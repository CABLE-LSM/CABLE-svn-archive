...
contains
  ...
  recursive function my_gamma(a) result(g)
    ...
    if ( x < 0.5_r_2 ) then 
       g = pi / ( sin(pi*x) * my_gamma(1._r_2-x) )
    else 
       x = x - 1._r_2
       t = p(0) 
       do i=1, cg+2 
       ...
       w = x + seven + 0.5_r_2
       g = sqrt(2._r_2*pi) * w**(x+0.5_r_2) * exp(-w) * t
    end if
...
SUBROUTINE or_soil_evap_resistance(soil,air,met,canopy,ssnow,veg,rough,snow_covered,dz_litter)
   ...
   integer :: i,j,k 
   logical, save ::  myfirst_call = .true.
   ...
   pore_size(:) = pore_radius(:)*sqrt(pi)
      ...
      eddy_shape = 0.3*real(met%ua,r_2)/ max(0.0001_r_2,real(canopy%us,r_2) *&
                exp(-real(rough%coexp,r_2)*(1._r_2-canopy%sublayer_dz/max(0.01_r_2,real(rough%hruff,r_2)))))
