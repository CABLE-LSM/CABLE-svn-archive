...
if [ -z $restart_infile ]; then
  if [ $year -gt 1901 ]; then              #gswp run after first year
    ...
    #check if file exists.  if not do not try to use it
...
&cable
   ...
   filename%path    = "./"
   filename%type    = "./surface_data/CABLE_GSWP3_HGSD_DRT_Surface_Data_veg_fix_dtb_GW.nc"
   filename%veg    = "./surface_data/def_veg_params_zr_clitt_correct.txt"
   filename%soil    = "./surface_data/def_soil_params.txt"
   filename%gw_elev = "./surface_data/GSWP3_elevation_slope_mean_stddev_dtb.nc"
   ...
   gswpfile%mask  = "./gswp/landmask.nc"
   ...
   gw_params%MaxSatFraction     = 1500.0
   gw_params%MaxHorzDrainRate   = 2e-4  !qh = qhmax*hksat*tan(slope), tan(slope) -> 1e-5 ->0.02
   gw_params%EfoldHorzDrainRate = 1.5
