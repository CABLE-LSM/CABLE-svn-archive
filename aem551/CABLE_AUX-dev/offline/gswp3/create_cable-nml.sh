...
if [ -f "$(pwd)/cable.nml" ]; then
   mv $(pwd)/cable.nml $(pwd)/old.cable.nml
fi
...
&cable
   ...
   filename%type    = "./surface_data/CABLE_GSWP3_HGSD_Surface_Data.nc"
   filename%veg    = "./surface_data/$vegfile"
   filename%soil    = "./surface_data/$soilfile"
   filename%gw_elev = "./surface_data/GSWP3_elevation_slope_mean_stddev.nc"
   ...
   gw_params%MaxHorzDrainRate   = 10.0  !qh = qhmax*hksat*tan(slope), tan(slope) -> 1e-5 ->0.02
