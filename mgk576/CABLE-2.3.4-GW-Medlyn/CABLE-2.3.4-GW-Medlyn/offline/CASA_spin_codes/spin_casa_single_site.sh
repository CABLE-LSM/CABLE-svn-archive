...
EOF
    ...
    ./create_cable-nml_casaspin_Step4_final_run.sh -m $met_file -d $LAIflag -c $icycle -p $casa_outpath -j $cable_finalpath -g $GWflag -e false -u true -i $restart_in -r $restart_out -k $casa_biome #-n $CO2_con
...
#using fast spin-up
if [[ "$step2" = true ]]; then
     restart_init="${cable_outpath}/CABLE_restart_final_Step3.nc"
else
...
EOF
    ...
    ./create_cable-nml_casaspin_Step1-3.sh -m $met_file -d $LAIflag -c $icycle -p $casa_outpath -j $cable_finalpath -g $GWflag -e true -i $restart_in -b true -z true -k $casa_biome #-n $CO2_con
