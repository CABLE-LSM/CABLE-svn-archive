...
REAL :: totdepth
  ...
  veg_cbl%ejmax    = 2.*veg_cbl%vcmax
  veg_cbl%gamma = 3.e-2 !for Haverd2013 switch 
  veg_cbl%F10 = 0.85 
...
DO J=1,um1%TILE_PTS(N)
  ...
  IF( um1%TILE_FRAC(i,N) .gt. 0.0 ) THEN
    ...
    IF(N < 5 ) THEN ! trees
      ...
      clobbered_htveg(i,N) = max(1.,um_htveg(i,N)) 
...
!      canopy%_cblfrp = PACK(tempvar2, um1%L_TILE_PTS)
      ...
      canopy_cbl%frs = canopy_cbl%frs * 1000.
      canopy_cbl%frp = canopy_cbl%frp * 1000.
...
!H!enddo
      ...
      !ELSE
        met_cbl%ca = CO2_MMR
      !ENDIF
      met_cbl%ca = met_cbl%ca * 28.966/44.
...
TYPE(canopy_type),     INTENT(inout)  :: canopy_cbl
   ...
   LOGICAL, SAVE :: first_call= .TRUE.
      ...
      IF( first_call ) THEN
         canopy_cbl%ga = 0.
         canopy_cbl%us = 0.01
         canopy_cbl%fes_cor = 0.
         canopy_cbl%fhs_cor = 0.
         canopy_cbl%fwsoil = 1.
         first_call = .FALSE.
      ENDIF
     ...
     !H!this is not used in HAC, not unpacked anyway AND why it didnt show up
      ...
      IF( first_call) THEN 
         ...
         ELSEWHERE (soil_cbl%albsoil(:,1) > 0.14 .and. soil_cbl%albsoil(:,1) <= 0.20)
           sfact = 0.62
         END WHERE
         ssnow_cbl%albsoilsn(:,2) = 2. * soil_cbl%albsoil(:,1) / (1. + sfact)
         ...
         DO J = 1,um1%SM_LEVELS
            ...
            ssnow_cbl%wbice(:,J) = max(0.,ssnow_cbl%wbice(:,J))  !should not be needed -- mrd561
         ...
         ssnow_cbl%owetfac = MAX( 0., MIN( 1.0,                                    &
      ...
      IF (cable_user%gw_model) THEN
         ...
         WHERE( veg_cbl%iveg == 16 .and. &
                ...
                ssnow_cbl%GWwb(:) .gt. ssnow_cbl%wbtot1)
...
!     DO J=1, msn
      ...
      ENDDO
      ssnow_cbl%wb_lake = MAX( ssnow_cbl%wbtot2 - ssnow_cbl%wbtot1, 0.)
      END IF 
...
TYPE(sum_flux_type),  INTENT(inout)  :: sum_flux_cbl
      sum_flux_cbl%sumpn = 0.; sum_flux_cbl%sumrp = 0.; sum_flux_cbl%sumrpw = 0.
      sum_flux_cbl%sumrpr = 0.; sum_flux_cbl%sumrs = 0.; sum_flux_cbl%sumrd = 0.
      sum_flux_cbl%dsumpn = 0.; sum_flux_cbl%dsumrp = 0.; sum_flux_cbl%dsumrs = 0.
      sum_flux_cbl%dsumrd = 0.; sum_flux_cbl%sumxrp = 0.;  sum_flux_cbl%sumxrs = 0.
