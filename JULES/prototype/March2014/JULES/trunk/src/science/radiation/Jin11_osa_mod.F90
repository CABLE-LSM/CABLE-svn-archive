...
!
    ...
    DO ibd = 1, nd_bands
      ...
      IF (band_top(ibd) < 0.OR.band_bot(ibd) < 0) THEN
        ...
        WRITE(jules_message,'(/,a)')                                           &
          'Error in Jin11_osa: Attempting to mix broadband and spectral bands'
        CALL jules_print('Jin11_osa',jules_message)
        STOP
#endif
      END IF
  ...
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ! Seciton 1: parameters that can be done outside the wavelength loop:
  ...
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ! Seciton 2: main wavelength loop:
  DO iwl = 1, nd_wl 
    ...
    IF (.NOT.l_broadband) THEN
      ...
      DO ipt = 1, nd_points
        alb_w_dr(ipt) = r00(ipt) * (1.-r22(ipt)) * (1. - alb_s_dr(ipt)) /      &
                                                   (1. - r00(ipt)*r22(ipt)) 
      END DO
      ...
      DO ipt = 1, nd_points
        alb_w_df(ipt) = r00_cosz_e(ipt) * (1.-r22(ipt)) *                      &
                       (1. - r11_cosz_e(ipt)) / (1. - r00_cosz_e(ipt)*r22(ipt)) 
      END DO
  ...
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ! Seciton 3: combine the subsampled wavelengths back onto bands
...
!          to sensible numbers:
...
#else
    WRITE(jules_message,'(/,a)')                                           &
          'ERROR: Cos(sza) out of range 0-1'
    CALL jules_print('Jin11_check_inputs',jules_message)
    STOP
#endif
  END IF
  ...
  IF (l_ws_10m_capped) THEN
    WRITE(jules_message,'(/,a)')                                           &
          'WARNING: 10m wind speed out of range 0.0 - 100.0 ms-1'
    CALL jules_print('Jin11_check_inputs',jules_message)
  END IF
  ...
  IF (l_chloro_capped) THEN
    WRITE(jules_message,'(/,a)')                                           &
          'WARNING: chlorophyll content out of range 0.0 - 100.0 gm-3'
    CALL jules_print('Jin11_check_inputs',jules_message)
  END IF
...
#else
      WRITE(jules_message,'(/,a)') 'Attempting to interpolate out of range'
      CALL jules_print('Jin11_lin_interp_real',jules_message)
      STOP
#endif
    ELSE
...
#else
      WRITE(jules_message,'(/,a)')                                             &
                  'Array length mismatch for wind and solar zenith angle'
      CALL jules_print('Jin11_regr_eq4',jules_message)
      STOP
#endif
    END IF
...
!          (with an extraplolated value at 5001nm added)
  ...
  ! the relative refractive index of water and air, for broadband VIS:
  REAL, PARAMETER :: bb_refm = 1.34
  ! start point, on the lookup table, for an input wavelength
  INTEGER :: st
  ...
  DO ipt = 1, nd_points
    IF (chloro_gm3(ipt) > 0.02) THEN
      bbp(ipt) = (0.002 + 0.01*(0.5 - 0.25*LOG10(chloro_gm3(ipt))) *           &
                          (wl_nm(1)/550.)**chl_v(ipt) )*bp550(ipt)
    ELSE
