...
!$The three lines above appear as below in MMY code with different factors -- rk4417 ! MMY@23Apr2023 I have no idea which lines should be used, need to ask Claire
...
!$       LUC_EXPT%secdf =  LUC_EXPT%secdf * 1.0/3.0
    ELSEWHERE (LUC_EXPT%biome .EQ. 7 .OR. LUC_EXPT%biome .EQ. 8 &  ! boreal ! the remainder of the where construct from here
         .OR. LUC_EXPT%biome .EQ. 9 .OR. LUC_EXPT%biome .EQ. 10  )          ! is missing from MMY code -- rk4417 ! MMY@23Apr2023 I have no idea which lines should be used, need to ask Claire
    ...
    WHERE (LUC_EXPT%prim_only .EQV. .TRUE.)
       ...
       ELSEWHERE (LUC_EXPT%biome .EQ. 12 .OR. LUC_EXPT%biome .EQ. 13 &
          ...
          LUC_EXPT%primaryf =  LUC_EXPT%primaryf * 1.0/5.0
!$The two lines above appear as below in MMY code with different factors -- rk4417 ! MMY@23Apr2023 I have no idea which lines should be used, need to ask Claire
...
!$          LUC_EXPT%primaryf =  LUC_EXPT%primaryf * 1.0/3.0
       ELSEWHERE (LUC_EXPT%biome .EQ. 7 .OR. LUC_EXPT%biome .EQ. 8 &            ! the remainder of the where construct
            .OR. LUC_EXPT%biome .EQ. 9 .OR. LUC_EXPT%biome .EQ. 10) ! boreal    ! is missing from MMY code -- rk4417    ! MMY@23Apr2023 I have no idea which lines should be used, need to ask Claire
  ...
  SUBROUTINE LUC_EXPT_SET_TILES(inVeg, inPfrac, LUC_EXPT )
    ...
    DO k=1,mland
       ...
       IF (inVeg(m,n,1).LT.11) THEN ! vegetated
          ...
          ENDIF
       ELSE
          LUC_EXPT%prim_only(k)=.TRUE.     ! this else is absent in MMY code -- rk4417 ! MMY@23Apr2023 I have no idea which lines should be used, need to ask Claire
       ...
       IF (inveg(m,n,1)==14 .OR.  inveg(m,n,1)==8 ) THEN
          ...
          inPFrac(m,n,1) = 1.0
          inPFrac(m,n,2:3) = 0.0
          inVeg(m,n,2:3) = 0              ! this line is absent in MMY code -- rk4417 ! MMY@23Apr2023 I have no idea which lines should be used, need to ask Claire
       ENDIF
...
!$  ! Adjust transition areas based on native tree fraction for savanna grid-cells - comment line in MMY code -- rk4417 ! MMY@23Apr2023 delete the comment
...
!$The four lines above appear as below in MMY code with different factors -- rk4417 ! MMY@23Apr2023 I have no idea need to ask Claire
...
!$    LUC_EXPT%INPUT(stog)%VAL =  LUC_EXPT%INPUT(stog)%VAL * 1.0/3.0
    ELSEWHERE (LUC_EXPT%biome .EQ. 7 .OR. LUC_EXPT%biome .EQ. 8 &               ! the remainder of the where construct 
         .OR. LUC_EXPT%biome .EQ. 9 .OR. LUC_EXPT%biome .EQ. 10) ! boreal       ! is missing from MMY code -- rk4417   ! MMY@23Apr2023 I have no idea need to ask Claire
