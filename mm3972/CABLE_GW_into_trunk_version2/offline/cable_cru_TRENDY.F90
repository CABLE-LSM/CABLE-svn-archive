...
!$          ALLOCATE( CRU%CO2VALS( 1860:2015 ) )   ! the two lines above appear as below in MMY code -- rk4417 ! MMY@23Apr2023 delete this line and the line below 
...
!$       ! read Ndep at current year (noting that file starts at 1850) ! 2 lines replace ones above in MMY code -- rk4417 ! MMY@23Apr2023 delete this line the and line below since the lines above look more reasonable
...
!$    IF (CALL1) RunStartYear = 1800 ! edit vh !  ! line above appears this way in MMY code -- rk4417 ! MMY@23Apr2023 delete the comment
...
!$    ENDIF
  ...
  SUBROUTINE CRU_GET_DAILY_MET( CRU, LastDayOfYear, LastYearOfMet )
    ...
    ! Keep the initial value of CYEAR for calculation of different MetYear if required.
    IF (CALL1) THEN
       !RunStartYear = CRU%CYEAR    ! this line replaces the one below in MMY code -- rk4417 ! MMY@23Apr2023 delete the comment
       RunStartYear = 1691
