...
#ifdef UM_BUILD    
             ! note that this if block is not found in MMY code -- rk4417 ! MMY@23Apr2023 need to ask Claire or Jhan
             mstype=9,& ! total # soil types, needs to de defined atCompile TimeForNow
#else       
             mstype,& ! total # soil types,         from input
#endif
!$             mland                           ! # land grid cells   ! replaced by rk4417 as per MMY ! MMY@23Apr2023 delete this line as duplication
  ...
  TYPE soil_parameter_type
     ...
     REAL(r_2), DIMENSION(:,:), POINTER ::                                    &
          ...
          smpc_vec, &  ! Hutson Cass SWC potential cutoff    ! 2 lines inserted by rk4417 as per MMY ! MMY@23Apr2023 accept as it's new from CABLE-GW
          wbc_vec,  &  ! Hutson Cass SWC volumetric water cutoff
          sfc_vec, & !field capcacity (hk = 1 mm/day)
          swilt_vec     ! wilting point (hk = 0.02 mm/day)

     REAL(r_2), DIMENSION(:), POINTER ::                                      &
          hkrz,&! rate hyds changes with depth                  ! 6 lines added by rk4417 as per MMY ! MMY@23Apr2023 accept as it's new from CABLE-GW
     ...
     REAL(r_2), DIMENSION(:), POINTER ::                                       &
          ...
          smpc_GW, &  ! Hutson Cass SWC potential cutoff  ! 2 lines inserted by rk4417 as per MMY ! MMY@23Apr2023 accept as it's new from CABLE-GW
...
!$          otgg,    & ! soil temperature in K  ! This is 2-dim in MMY (see below) -- rk4417 ! MMY@23Apr2023 accept as CABLE-GW increases otgg's dimensions
     ...
     REAL, DIMENSION(:,:), POINTER ::                                         &
          ...
          smass,      & ! snow mass
          ssdn,       & ! snow densities
          otgg,       & ! soil temperature in K  ! inserted as per MMY (no longer 1-dim) -- rk4417 ! MMY@23Apr2023 accept as CABLE-GW increases otgg's dimensions
     ...
     REAL(r_2), DIMENSION(:,:), POINTER  ::                                     &
          ...
          qhlev,   &
          smp_hys, & !soil swc props dynamic from hysteresis ! added as per MMY -- rk4417 ! MMY@23Apr2023 accept as it is new from CABLE-GW
          wb_hys,  &
          sucs_hys,&
          ssat_hys,&
          watr_hys,&
...
CONTAINS
  ...
  SUBROUTINE alloc_soil_parameter_type(var, mp)
    ...
    var%watr(:,:) = 0.05            ! line not found in MYY code -- rk4417 ! MMY@23Apr2023 accept as it's just an initialization and doesn't really matter
    ALLOCATE( var%wbc_GW(mp) )      ! 4 lines added by rk4417 as per MMY ! MMY@23Apr2023 accept as they are new from CABLE-GW
    ...
    ALLOCATE( var%hkrz(mp) )   ! 6 lines inserted by rk4417 as per MMY ! MMY@23Apr2023 accept as they are new from CABLE-GW
...
!$    ALLOCATE( var%otgg(mp) )     ! replaced as per MMY -- rk4417 ! ! MMY@23Apr2023 delete this line as CABLE-GW increased otgg's dimensions
    ...
    ALLOCATE(var % smp_hys(mp,ms) )   !1      ! block added as per MMY -- rk4417 ! MMY@23Apr2023 accept as they are new from CABLE-GW
...
#ifdef UM_BUILD                                
     ! note that this if block is not found in MMY code -- rk4417   ! MMY@23Apr2023 need to ask Claire
  ...
  SUBROUTINE dealloc_soil_parameter_type(var)
    ...
    DEALLOCATE( var%wbc_GW )   ! 4 lines inserted by rk4417 as per MMY ! MMY@23Apr2023 accept it as it's new from CABLE-GW
    ...
    DEALLOCATE( var%drain_dens ) ! 7 lines inserted by rk4417 as per MMY ! MMY@23Apr2023 accept it as it's new from CABLE-GW
    DEALLOCATE( var%hkrz )
  ...
  SUBROUTINE dealloc_soil_snow_type(var)
    ...
    DEALLOCATE(var % smp_hys ) ! 7 lines added by rk4417 as per MMY  ! MMY@23Apr2023 accept it as it's new from CABLE-GW
