MODULE cable_psm
  ...
  USE cable_io_vars_module, only: wlogn   ! added by rk4417 as per MMY code but seems to do nothing in cable_io_vars_module or here ! MMY@23Apr2023 delete this line as it does anything in this file
  ...
  ! these precomputed values are taken by the sample code in Wikipedia, ! comment inserted by rk4417 as per MMY code   ! MMY@23Apr2023 delete rk4417's comment
...
CONTAINS
  ...
  SUBROUTINE or_soil_evap_resistance(soil,air,met,canopy,ssnow,veg,rough)
    ...
    REAL(r_2), DIMENSION(mp) :: sublayer_dz, eddy_shape,eddy_mod,soil_moisture_mod, &  ! sublayer_dz is not used and is also missing ! MMY@23Apr2023 delete sublayer_dz as it is replaced by canopy%sublayer_dz in the code below
...
!$    canopy%sublayer_dz(:) = 0.005   ! this line appears in MMY code but will leave commented for now -- rk4417 ! MMY@23Apr2023 need to check whether it should be commented
   ...
   ! ________ MMY@23Apr2023 delete this block as the one from Trunk is better _______
   ...
   litter_dz(:) = 0.0                ! this block replaces the if block above in MMY code -- rk4417 ! MMY@23Apr2023 delete the comments
   if (cable_user%litter) then
    ...
    ! ___________ MMY@23Apr2023 delete this block as calculation from CABLE-GW is more formal __________
...
!$          hk_zero_sat(i) = max(0.001*soil%hyds_vec(i,1),1e-12)   ! 2 lines here replace ones above in MMY code -- rk4417 ! MMY@23Apr2023 delete the two lines
          ...
          END IF
! _______________ MMY@23Apr2023 delete this block as the above one in CABLE-GW looks better _________
