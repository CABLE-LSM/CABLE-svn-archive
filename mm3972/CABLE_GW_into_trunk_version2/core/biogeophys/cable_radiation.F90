...
CONTAINS
  ...
  SUBROUTINE init_radiation( met, rad, veg, canopy )
    ...
    LOGICAL, DIMENSION(mp)    :: mask   ! select points for calculation  ! added line as per MMY -- rk4417 ! MMY@23Apr2023 keep, as mask is used in the later codes
    ...
    mask = canopy%vlaiw > C%LAI_THRESH  .AND.                          & ! added line as per MMY -- rk4417 ! MMY@23Apr2023 accept it since it looks like won't cause any issue
...
!$   WHERE (canopy%vlaiw > C%LAI_THRESH .and. rad%fbeam(:,1).GE.C%RAD_THRESH   ) ! MMY has this line instead ? -- rk4417 ! MMY@23Apr2023 need to ask Claire
...
!$       WHERE(rad%fbeam(:,1) < C%RAD_THRESH )    ! MMY has this line instead ? -- rk4417 ! MMY@23Apr2023 need to ask Claire
