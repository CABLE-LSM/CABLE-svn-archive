...
CONTAINS
  ...
  SUBROUTINE open_output_file(dels, soil, veg, bgc, rough)
    ...
    END IF
    ! _______________________ MMY ___________________________
...
!! vh_js !!
    ...
    IF(output%flux .OR. output%PotEvap) THEN
       ...
       IF(writenow) THEN
          ...
          ! Write value to file:
          CALL write_ovar(out_timestep, ncid_out, ovid%PotEvap, 'PotEvap',         &
                        out%PotEvap, ranges%PotEvap, patchout%PotEvap, 'default', met)
          ! Reset temporary output variable:
          out%PotEvap = 0.0
       END IF
    END IF
