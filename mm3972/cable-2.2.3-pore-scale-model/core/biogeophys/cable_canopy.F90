...
SUBROUTINE fwsoil_calc_linear(fwsoil, soil, ssnow, veg)
   ...
   REAL(r_2), DIMENSION(mp,ms) :: wbliq

   INTEGER :: i,k

   wbliq(:,:) = max(0._r_2, ssnow%wb - ssnow%wbice )
...
SUBROUTINE or_soil_evap_resistance(soil,air,met,canopy,ssnow,veg,rough)
   ...
   !hk_zero = max(0.001*soil%hksat(:,1)*(min(max(rel_s,0.001_r_2),1._r_2)**(2._r_2*soil%clappB(:,1)+3._r_2) ),1e-8) ! MMY
   !hk_zero_sat = max(0.001*soil%hksat(:,1),1e-8) ! MMY
  
   hk_zero = max(0.001*soil%hksat(:,1)*(min(max(rel_s,0.001_r_2),1._r_2)**(2._r_2*soil%clappB(:,1)+3._r_2)),1e-12) ! MMY
   hk_zero_sat = max(0.001*soil%hksat(:,1),1e-12) ! MMY
   ...
   print *,"MMY ssnow%sv_rtevap is ", ssnow%sv_rtevap ! MMY
   print *,"MMY ssnow%bl_rtevap is ", ssnow%bl_rtevap ! MMY
