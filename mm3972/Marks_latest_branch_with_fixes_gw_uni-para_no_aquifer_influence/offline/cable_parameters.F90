...
!    IF(ASSOCIATED(soiltype_metfile)) DEALLOCATE(soiltype_metfile)
  ...
  ! ssnow%smp_hys  = -1.0e+36      ! MMY
   ssnow%smp_hys  = -soil%sucs_vec ! MMY
  ...
  do k=1,ms
     do i=1,mp
        if (ssnow%wb_hys(i,k) .lt. 0._r_2) then
           ...
           ! PRINT *, "ssnow%wb_hys  = ssnow%wb", ssnow%wb_hys(i,k) ! MMY
        end if
        ! PRINT *, "Check POINT 3.1" !MMY
        ssnow%wb_hys(i,k)  = max(soil%watr(i,k) ,min(soil%ssat_vec(i,k), ssnow%wb_hys(i,k)))
        ! PRINT *, " ssnow%wb_hys,soil%watr,soil%ssat_vec", ssnow%wb_hys(i,k), soil%watr(i,k),soil%ssat_vec(i,k)  ! MMY
...
!    IF (cable_user%gw_model) then ! MMY
       ...
       IF ((nlon*nlat*npatch*nhorz .ne. xdimsize*ydimsize*ms*mpatch) .and. &
          ...
          WRITE(logn,*) 'Errors reading the dimensions from '//filename%type ! MMY
