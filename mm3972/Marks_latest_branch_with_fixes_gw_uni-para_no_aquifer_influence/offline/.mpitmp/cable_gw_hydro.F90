 ...
 !that is ok 
  ...
  IMPLICIT NONE
    ...
    do i=1,mp
       ...
       ! ____________ MMY put the redundant water of GW directly into drainage _______________
       ...
       ! ________________ MMY if GW is unsaturated, let GW saturated first ___________________
       if ((xsi .gt. 0._r_2) .and. (ssnow%GWwb(i) .lt. soil%GWssat_vec(i))) then
          ! PRINT *, "(xsi .gt. 0._r_2) .and. (ssnow%GWwb(i) .lt. soil%GWssat_vec(i))"
          if (xsi .lt. ( soil%GWssat_vec(i)-ssnow%GWwb(i) ) * (m2mm*soil%GWdz(i)) ) then
             ssnow%GWwb(i) = ssnow%GWwb(i) + xsi/(m2mm*soil%GWdz(i))
             xsi = 0._r_2
          else
             xsi = xsi - ( soil%GWssat_vec(i)-ssnow%GWwb(i) ) * (m2mm*soil%GWdz(i))
             ssnow%GWwb(i) = soil%GWssat_vec(i)
          end if
       end if
       ! _____________________________________________________________________________________
...
!END SUBROUTINE calc_soil_hydraulic_props
  ...
  IMPLICIT NONE
    ...
    select case (gw_params%aquifer_recharge_function)
       ...
       case default
          PRINT *,"aquifer_recharge_function = default" ! MMY
          do i=1,mp
             if ((ssnow%wtd(i) .le. csum_dzmm(i,ms)) .or. &
                ...
                ssnow%Qrecharge(i) = (ssnow%hk(i,ms)+ssnow%GWhk(i)) ! MMY
                if (ssnow%Qrecharge(i) .lt. 0.0) then        ! MMY
                    PRINT *, "ssnow%Qrecharge .lt. 0.0"      ! MMY
                end if                                       ! MMY
             ! ______________________ MMY COMMENT OUT _____________________________         
