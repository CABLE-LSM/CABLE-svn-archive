...
!! vh !! inserted min to avoid -ve values of GPP
 ...
 REAL FUNCTION xrdt(x)
  ...
  SUBROUTINE fwsoil_calc_std(fwsoil, soil, ssnow, veg)
    ...
    if (.not.cable_user%gw_model) THEN
       ...
       ! ______________________________ MMY _______________________________________
       rwater = MAX(1.0e-9,                                                    &
                SUM(veg%froot * MAX(1.0e-9, MIN( 1.0, &
 ...
 SUBROUTINE fwsoil_calc_hie_watpot(fwsoil, soil, ssnow, veg)
   ...
   ! NB : this parameters in the subroutine are only suitable to cable_user%GS_SWITCH == 'medlyn'

   ! Calculate soil hydraulic resistance following SPA approach (Williams et al.)
   ! and water potential dependent fwsoil
   !
   ...
   ! Martin De Kauwe, 3rd June, 2019
    ...
    REAL, PARAMETER :: gC2DM = 1./0.49   ! ratio Dry matter mass to g(C)
    ...
    REAL                :: root_mass, rs, Ksoil, root_biomass
    REAL                :: soil_resist
    ...
    DO i = 1, mp ! mp: Total number of patches (countPatch)
      ...
      DO j = 1, ms ! Loop over ms soil layers
        ...
        IF (soilR(i,j) .GT. 0.0) THEN
           est_evap(j) = MAX(0.0, &
                      (psi_soil(i,j) - min_root_wp) / soilR(i,j))
      ...
      IF (total_est_evap > 0.0) THEN
        ...
        weighted_psi_soil(i) = weighted_psi_soil(i) / total_est_evap
      ELSE ! all abs(ssnow%smp/1000. * M_HEAD_TO_MPa) <= min_root_wp
           ! soil moisture is too low that plants cannot absorb in a normal way
        weighted_psi_soil(i) = 0.0
        DO j = 1, ms ! Loop over ms soil layers
           weighted_psi_soil(i) = weighted_psi_soil(i) + &
                                  psi_soil(i,j) * soil%zse(j)
        END DO
        weighted_psi_soil(i) = weighted_psi_soil(i) / SUM(soil%zse)
      END IF
    ...
    ! Remove vbeta #56
    IF(cable_user%GS_SWITCH == 'medlyn') THEN
       fwsoil = MAX(1.0e-4,MIN(1.0, fwsoil))
    ENDIF
