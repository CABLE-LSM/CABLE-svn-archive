...
SUBROUTINE spincasacnp(fcnpspin,dels,kstart,kend,mloop,veg,soil,casabiome,casapool, &
  ...
  ! more variables to store the spinup pool size over the last 10 loops. Added
  ! by Yp Wang 30 Nov 2012
...
!      CALL read_casa_dump(casafile%cnpspin, casamet, casaflux, idoy,
!      kend/ktauday )
...
!          CALL read_casa_dump(casafile%cnpspin, casamet, casaflux, idoy, kend )
  ...
  DO nvt=1,mvtype
     IF(bmarea(nvt) >0.0) THEN
         ...
         IF (icycle >1) THEN 
             DO kloop=1,5
               WRITE(92,922) nvt, bmnplant(kloop,nvt,:),bmnlitter(kloop,nvt,:), &
                                          bmnsoil(kloop,nvt,:),bmnsoilmin(kloop,nvt) 
             ENDDO
         ENDIF
...
!----------------------------------------------------------------------------------------
 ...
 SUBROUTINE totcnppools(kloop,veg,casamet,casapool,bmcplant,bmnplant,bmpplant,bmclitter,bmnlitter,bmplitter, &
  ...
  ! this subroutine is temporary, and its needs to be modified for multiple
  ! tiles within a cell
  USE cable_def_types_mod
  USE casadimension
  USE casaparm
