...
!  TYPE (veg_parameter_type), INTENT(IN) :: veg
 ...
 ! npt=26493
 ! print *, 'pool sizes for 26493 from restart: ', casapool%cplant(npt,:),
 ! casapool%nplant(npt,:)
...
!  npt=26493
!  print *, 'pool sizes for 26493 after: ',
!  casamet%glai(npt),casapool%cplant(npt,:),  &
!            casapool%nplant(npt,:), casapool%pplant(npt,:),
!            &
!            casapool%clitter(npt,:),
!            casapool%nlitter(npt,:),casapool%plitter(npt,:), &
!            casapool%csoil(npt,:),   casapool%nsoil(npt,:),
!            casapool%psoil(npt,:),    &
!            casapool%nsoilmin(npt),
!            casapool%psoillab(npt),casapool%psoilsorb(npt),
!            casapool%psoilocc(npt)
...
SUBROUTINE casa_poolout(ktau,veg,soil,casabiome,casapool,casaflux,casamet, &
  ...
  USE casavariable
  USE phenvariable
  !for casa spin? added amu561 Feb '16
  ...
  ! variables for netcdf (casa spin? added amu561 Feb '16)
...
!92    format(5(i6,',',2x),5(f15.6,',',2x),i6,',',2x,100(f15.6,',',2x))
  ...
  !Code below for writing variables during spin-up? added amu561 Feb '16
...
SUBROUTINE biogeochem(ktau,dels,idoY,veg,soil,casabiome,casapool,casaflux, &
 ...
 ! local variables added by ypwang following Chris Lu 5/nov/2012
  ...
  real, dimension(mp), INTENT(OUT)   :: cleaf2met,cleaf2str,croot2met,&
                                        croot2str,cwood2cwd,         &
                                        nleaf2met,nleaf2str,nroot2met,&
                                        nroot2str,nwood2cwd,         &
                                        pleaf2met,pleaf2str,proot2met,&
                                        proot2str,pwood2cwd
