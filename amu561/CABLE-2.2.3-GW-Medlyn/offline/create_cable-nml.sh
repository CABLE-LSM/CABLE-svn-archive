...
&cable
   ...
   filename%type        = "../../Inputs/CABLE_GSWP3_HGSD_Surface_Data.nc"
   filename%veg         = "../../Inputs/$vegfile"
   filename%soil        = "../../Inputs/$soilfile"
   filename%gw_elev     = "../../Inputs/GSWP3_elevation_slope_stddev.nc"
   ...
   output%CASA   = .FALSE.   ! write CASA outputs to nc file?
   icycle = 0                ! 0 for not using casaCNP, 1 for C, 2 for C+N, 3 for C+N+P
   casafile%cnpipool=  './CASA_test_inputs/CASA_ins/poolcnpOut_old_STU100.csv'
   casafile%cnpbiome=  './CASA_test_inputs/CASA_ins/pftlookup_csiro_v16_17tiles.csv'
   ...
   casafile%phen=      './CASA_test_inputs/CASA_ins/modis_phenology_csiro.txt'
   ...
   cable_user%GS_SWTICH = "medlyn"              ! choices are:
                                                ! 1. leuning
                                                ! 2. medlyn
   ...
   gw_params%MaxSatFraction     = 0.0           !Anna changed to 0 for site runs
   gw_params%MaxHorzDrainRate   = 9e-3          !qh = qhmax*hksat*tan(slope), tan(slope) -> 1e-5 ->0.02
   gw_params%EfoldHorzDrainRate = 2.0 
   gw_params%EfoldMaxSatFrac    = 4.0 
   gw_params%hkrz               = 0.5
