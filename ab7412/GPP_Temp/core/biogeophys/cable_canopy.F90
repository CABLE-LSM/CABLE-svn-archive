       ...
       DO i=1,mp
          ...
          IF (canopy%vlaiw(i) > C%LAI_THRESH .AND. abs_deltlf(i) > 0.1) THEN
             ...
             if (.not.cable_user%acclimate_photosyn) then
              ...
              !  call xvcmxt4_acclim(tlfx(i), climate%mtemp(i), veg%Eav_int(i), veg%Eav_slope(i), veg%dSv_int(i), & !AB: commented out to remove C4 acclimation
              ...
              !  temp_sun_c4(i)   = temp_c4(i) * veg%vcmax_sun(i) * veg%frac4(i) !AB: commented out to remove C4 acclimation
              !  temp_shade_c4(i) = temp_c4(i) * veg%vcmax_shade(i) * veg%frac4(i) !AB: commented out to remove C4 acclimation
                temp_sun_c4(i)   = xvcmxt4(tlfx(i), veg%Eav(i),veg%dSv(i)) * veg%vcmax_sun(i) * veg%frac4(i)  !AB: copied from above to remove C4 acclimation
                temp_shade_c4(i) = xvcmxt4(tlfx(i), veg%Eav(i),veg%dSv(i)) * veg%vcmax_shade(i) * veg%frac4(i) !AB: copied from above to remove C4 acclimation
