...
USE switches, ONLY : l_spec_albedo, l_albedo_obs, l_spec_alb_bs
...
ELSE IF ( l_urban2t .AND. .NOT. l_moruses_emissivity ) THEN
  IF ( firstcall ) THEN
    CALL jules_print('physiol_jls','URBAN-2T canyon emissivity used', &
        level=PrNorm)
  END IF
  n = urban_canyon
  DO m = 1, tile_pts(n)
    ...
    DO n=npft+1,ntype
      IF ( n == urban_canyon ) THEN
        DO m=1,tile_pts(n)
          l = tile_index(m,n)
          IF ( firstcall ) THEN
            WRITE(jules_message,*) 'EMIS_TILE: Emissivity of canyon used',  &
                n,m,l
      ...
      ELSE IF ( n == urban_roof ) THEN
        DO m=1,tile_pts(n)
          ...
          IF ( v_sat(l,1) > 0.0 ) THEN ! Check for ice-tile
            IF ( firstcall ) THEN
              WRITE(jules_message,*) 'EMIS_TILE: Emissivity of roof used', &
                  n,m,l
              CALL jules_print('physiol_jls',jules_message,level=PrNorm)
            END IF
            emis_tile(l,1) = emis_tile(l,1)                           &
