...
SUBROUTINE surface_albedosn(ssnow, veg, met, soil)
   ...
   WHERE (ssnow%snowd <= 1.0) ssnow%snage = 0.
   tmp = ssnow%isflag * ssnow%tggsn(:,1) + ( 1 - ssnow%isflag ) * ssnow%tgg(:,1)
   tmp1 = MIN( tmp, C%TFRZ )
...
!            * ssnow%tgg(:,1)
!      tmp1 = MIN( tmp, C%TFRZ )
      ar1 = 5000. * (1. / (C%TFRZ-0.01) - 1. / tmp1) ! crystal growth  (-ve)
      ...
      !END WHERE
      !WHERE (soil%isoilm == 9 ) snrat = 1.
 
     ! permanent ice: hard-wired number to be removed in future version
      !WHERE (soil%isoilm == 9 .and. tmp .lt. C%TFRZ - 2. .and. ssnow%iantrct .gt. 0.5)
      WHERE (ssnow%iantrct .eq. 1.0 .and. tmp1 .lt. C%TFRZ - 5. )
         !  assumes pristine snow; ignores soot 
         ar3 = .0000001
         dnsnow = max (dnsnow, .1) !refreshing of snow albedo in Antarctic
      !   snrat = 1.
      !ELSEWHERE (soil%isoilm == 9 .and. tmp .ge. C%TFRZ - 2.0) 
      !   ar3 = .1
         snrat = 1.
      END WHERE
      WHERE (soil%isoilm == 9 .and. ssnow%iantrct .eq. 0.0 .and. tmp1 .lt. C%TFRZ - 5. )
         !  assumes pristine snow; ignores soot 
         ar3 = .001
         dnsnow = max (dnsnow, .05) !refreshing of snow albedo in Antarctic
      !   snrat = 1.
      !ELSEWHERE (soil%isoilm == 9 .and. tmp .ge. C%TFRZ - 2.0) 
      !   ar3 = .1
         snrat = 1.
...
!      END WHERE
   ...
   WHERE (ssnow%snowd > 1.0 .and. cable_runtime%um_radiation )
