...
CONTAINS
  ...
  ! Subroutine arguments
    ...
    REAL,    INTENT(IN) :: AlbSoil(mp,nrb) !NB to become IN OUT because of 
                                           !soil colour parameterization
  ...
  ! Local variables
    REAL ::                                                                   &
        alv(mp),     &  ! Snow albedo for visible
        alir(mp),    &  ! Snow albedo for near infra-red
        fage(mp),    &  ! age factor
        fzenm(mp),   &  ! zenith factor
        sfact(mp),   &  ! soil factor
        snrat(mp),   &  ! (1-) fraction of soil 'seen' when evaluating surface 
                        ! albedo
        tmp(mp),     &  ! temporary value
        SoilAlbsoilF(mp)
    ...
    WHERE ( SurfaceType == lakes_cable )
      SoilAlbsoilF = -0.022*( MIN( 275.0, MAX( 260.0, SoilTemp) ) - 260.0 )    &
                     + 0.45
    END WHERE
