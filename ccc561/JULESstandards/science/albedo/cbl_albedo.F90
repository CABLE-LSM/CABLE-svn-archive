...
CONTAINS
  ...
  ! Subroutine arguments
    ...
    REAL,    INTENT(OUT) :: AlbSnow(mp,nrb)           ! Alb adjusted for snow 
                                                      ! (ssnow%albsoilsn)
    REAL,    INTENT(OUT) :: EffSurfRefl_beam(mp,nrb)  ! Direct Beam [SW] 
                                                      ! (rad%reffbm)
    REAL,    INTENT(OUT) :: EffSurfRefl_dif(mp,nrb)   ! Diffuse [SW]  
                                                      ! (rad%reffdf)
    REAL,    INTENT(OUT) :: RadAlbedo(mp,nrb)         ! Tot albedo given 
                                                      ! RadFbeam (rad%albedo)
    ...
    REAL,    INTENT(IN)  :: VegTaul(mp,nrb) ! PARAMETER leaf transmisivity 
                                            ! (veg%taul)
    REAL,    INTENT(IN)  :: VegRefl(mp,nrb) ! PARAMETER leaf reflectivity 
                                            ! (veg%refl)
    INTEGER, INTENT(IN)  :: SurfaceType(mp) ! Integer index of Surface type 
                                            ! (veg%iveg)
    INTEGER, INTENT(IN)  :: SoilType(mp)    ! Integer index of Soil    type 
                                            ! (soil%isoilm)

    REAL,    INTENT(IN)  :: reducedLAIdue2snow(mp) ! Reduced LAI given 
                                                   ! snow coverage

    ! Albedos
    REAL,    INTENT(IN)  :: AlbSoil(mp,nrb)  ! Param'ed Bare Soil Albedo
                                             ! (soil%albsoil)
    ...
    !Prognostics
    REAL,    INTENT(IN)  :: SnowDepth(mp)    ! Total Snow depth - 
                                             ! water equivalent
    REAL,    INTENT(IN)  :: SnowDensity(mp)  ! Total Snow density 
                                             ! (assumes 1 layer)
    REAL,    INTENT(IN)  :: SoilTemp(mp)     ! Top layer Soil Temp. 
                                             ! (ssnow%tgg)
    REAL,    INTENT(IN)  :: SnowAge(mp)      ! Snow age (assumes 1 layer )

    ! Variables shared primarily between radiation and albedo 
    ! and possibly elsewhere
    REAL,    INTENT(IN)  :: RadFbeam(mp,nrb) ! Beam Fraction of total SW 
                                             ! (rad%fbeam)
    ...
    REAL,    INTENT(OUT) :: CanopyRefl_beam(mp,nrb)     ! Beam Canopy refl 
                                                        ! (rad%rhocbm)
    REAL,    INTENT(OUT) :: CanopyRefl_dif(mp,nrb)      ! Difuse Canopy refl
                                                        ! (rad%rhocdf)
    REAL,    INTENT(OUT) :: CanopyTransmit_beam(mp,nrb) ! Beam Transmit   
                                                        ! (rad%cexpkdm)
    REAL,    INTENT(OUT) :: CanopyTransmit_dif(mp,nrb)  ! Difuse Transmit 
                                                        ! (rad%cexpkbm)
  ...
  !----------------------------------------------------------------------------
    ...
    CanopyRefl_beam(:,:) = 0.0
    CanopyRefl_dif(:,:) = 0.0
    !CanopyTransmit_beam(:,:) = 1.0 ! Flushing this =1or0 changes answer - 
                                    ! implying?
    !CanopyTransmit_dif(:,:) = 1.0  ! MPI (at least inits this = 1.0 at dt=0)

    !Modify parametrised soil albedo based on snow coverage
    CALL surface_albedosn( AlbSnow, AlbSoil, mp, nrb, ICE_SoilType,            &
    ...
    CALL CanopyReflectance( CanopyRefl_beam, CanopyRefl_dif, mp, nrb,          &
                            CGauss_w,veg_mask, AlbSnow, xk,rhoch,              &
    ...
    ! Even when there is no vegetation, albedo is at least snow modified soil 
    ! albedo
  ...
  SUBROUTINE CanopyReflectance( CanopyRefl_beam, CanopyRefl_dif, mp, nrb,      &
                                CGauss_w, veg_mask, AlbSnow, xk, rhoch,        &
                                ExtCoeff_beam, ExtCoeff_dif )
  ! Description:
  ...
  ! Subroutine arguments
    INTEGER, INTENT(IN)  :: mp                      ! total number of "tiles"
    INTEGER, INTENT(IN)  :: nrb                     ! # rad bands [VIS,NIR(,LW)]
    REAL,    INTENT(OUT) :: CanopyRefl_dif(mp,nrb)  ! Beam Canopy refl 
                                                    ! (rad%rhocbm)
    REAL,    INTENT(OUT) :: CanopyRefl_beam(mp,nrb) ! Difuse Canopy refl
                                                    ! (rad%rhocdf)
    REAL,    INTENT(IN)  :: Cgauss_w(nrb)
    LOGICAL, INTENT(IN)  :: veg_mask(mp)            ! TRUE where vegetated
  ...
  ! Subroutine arguments
    INTEGER, INTENT(IN)  :: mp                          ! total number of "tile
    INTEGER, INTENT(IN)  :: nrb                         ! # rad bands [VIS,NIR(,
                                                        ! LW)]
    REAL,    INTENT(OUT) :: CanopyTransmit_dif(mp,nrb)  ! Transmitance 
                                                        ! (rad%cexpkd)
    REAL,    INTENT(OUT) :: CanopyTransmit_beam(mp,nrb) ! Transmitance 
                                                        ! (rad%cexpkb)
    LOGICAL, INTENT(IN)  :: mask(mp)                    ! TRUE where vegetated
    REAL,    INTENT(IN)  :: reducedLAIdue2snow(mp)      ! Reduced LAI given snow
    REAL,    INTENT(IN)  :: EffExtCoeff_beam(mp,nrb)    ! Extinction coefficient
    REAL,    INTENT(IN)  :: EffExtCoeff_dif(mp,nrb)     ! Extinction coefficient
  ...
  !----------------------------------------------------------------------------
    ...
    DO i = 1,mp
      DO b = 1,(nrb-1)
        IF ( mask(i) ) THEN
          dummy(i,b) = MIN( EffExtCoeff_beam(i,b) * reducedLAIdue2snow(i),     &
                            20.0 )
  ...
  ! Subroutine argmuents
    INTEGER, INTENT(IN)  :: mp                         ! total number of "tiles"
    INTEGER, INTENT(IN)  :: nrb                        ! # rad bands [VIS,NIR(,
                                                       !LW)]
    REAL,    INTENT(OUT) :: EffSurfRefl_dif(mp,nrb)    ! Diffuse Surf Refl 
                                                       ! (rad%reffdf)
    REAL,    INTENT(OUT) :: EffSurfRefl_beam(mp,nrb)   ! Beam Surf Refl        
                                                       ! (rad%reffbm)
    LOGICAL, INTENT(IN)  :: veg_mask(mp)               ! TRUE where vegetated
    REAL,    INTENT(IN)  :: CanopyRefl_beam(mp,nrb)    ! Beam Canopy refl 
                                                       ! (rad%rhocbm)
    REAL,    INTENT(IN)  :: CanopyRefl_dif(mp,nrb)     ! Difuse Canopy refl
                                                       ! (rad%rhocdf)
    REAL,    INTENT(IN)  :: CanopyTransmit_dif(mp,nrb) ! Transmitance 
                                                       ! (rad%cexpkdm)
    REAL,    INTENT(IN)  :: CanopyTransmit_beam(mp,nrb)! Transmitance   
                                                       ! (rad%cexpkbm)
    REAL,    INTENT(IN)  :: AlbSnow(mp,nrb)            ! snow adjustd Alb 
                                                       ! ssnow%albsoilsn

  !----------------------------------------------------------------------------

    !Surface reflectance to Difuse Radiation
    CALL EffectiveReflectance( EffSurfRefl_dif, mp, nrb, CanopyRefl_dif,       &
                               AlbSnow,  CanopyTransmit_dif, veg_mask )

    !Surface reflectance to Direct Beam  Radiation
    CALL EffectiveReflectance( EffSurfRefl_beam, mp, nrb, CanopyRefl_beam,     &
                               AlbSnow, CanopyTransmit_beam, veg_mask )
  ...
  ! Subroutine arguments
    INTEGER, INTENT(IN)  :: mp                     ! total number of "tiles"
    INTEGER, INTENT(IN)  :: nrb                    ! # rad bands [VIS,NIR(,LW)
    REAL,    INTENT(OUT) :: EffRefl(mp,nrb)        ! Effective Surface 
                                                   ! Reflectance
    REAL,    INTENT(IN)  :: AlbSnow(mp,nrb)        ! snow adjustd Alb 
                                                   ! ssnow%albsoilsn
    REAL,    INTENT(IN)  :: CanopyRefl(mp,nrb)     ! Beam Canopy refl 
                                                   ! (rad%rhocbm)
    REAL,    INTENT(IN)  :: CanopyTransmit(mp,nrb) ! Difuse Canopy refl
                                                   ! (rad%rhocdf nrb)
    LOGICAL, INTENT(IN)  :: mask(mp)               ! TRUE where vegetated
  ...
  ! Subroutine arguments
    INTEGER, INTENT(IN)  :: mp                      ! total number of "tiles"
    INTEGER, INTENT(IN)  :: nrb                     ! # rad bands [VIS,NIR(,LW)
    REAL,    INTENT(OUT) :: RadAlbedo(mp,nrb)       ! RadFbeam albedo 
                                                    ! (rad%albedo)
    LOGICAL, INTENT(IN)  :: veg_mask(mp)            ! TRUE where vegetated
    REAL,    INTENT(IN)  :: AlbSnow(mp,nrb)         ! snow adjustd Alb 
                                                    ! (ssnow%albsoilsn
    REAL,    INTENT(IN)  :: RadFbeam(mp,nrb)        ! Beam Fraction of SW  
                                                    ! (rad%fbeam)
    REAL,    INTENT(IN)  :: EffSurfRefl_dif(mp,nrb) ! Direct Beam [SW] 
                                                    ! (rad%reffbm)
