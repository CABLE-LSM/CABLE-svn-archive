...
while getopts "h:m:j:o:l:i:r:v:s:y:e:g:a:f:c:p:d:x:t:u:n:b:k:q:w:W:z:" OPTION
...
if [ -z $logfile ]; then
  ...
  if [ $year -ne 0 ]; then
      logfile="log_cable.txt"          #if gswp append year to log
  fi
...
if [ -z $vegfile ]; then
  vegfile='def_veg_params_zr_clitt.txt'
...
if [ -z "$SD_fwsoil_path" ]; then
    if [ "$SD_fwsoil" == "plus_SD" ]; then
	echo "You cannot pass a SD option on fwsoil without providing the SD data. Now falling back to no SD."
    fi
    if [ "$SD_fwsoil" == "minus_SD" ]; then
	echo "You cannot pass a SD option on fwsoil without providing the SD data. Now falling back to no SD."
    fi
    SD_fwsoil_path="no_path"
    SD_fwsoil="no_SD"
fi
...
&cable
   ...
   filename%gw_elev = "${in_path}/GSWP3_elevation_slope_stddev.nc"
   ...
   cable_user%FWSOIL_SWITCH = '${fwsoil}'       ! choices are:
                                                ! 1. standard
                                                ! 2. non-linear extrapolation
                                                ! 3. Lai and Ktaul 2000
                                                ! 4. Haverd2013
   cable_user%SD_FWSOIL_SWITCH = '${SD_fwsoil}' ! choices are:
                                                ! 1. no_SD
                                                ! 2. plus_SD
                                                ! 3. minus_SD
   cable_user%SD_FWSOIL_PATH = "${SD_fwsoil_path}" ! this could be a file path or could be nothing, depending on SD switch
