...
MODULE cable_gw_hydro_module
   ...
   USE cable_soil_snow_module, ONLY : snowdensity, snow_melting, snowcheck, &
                                      ...
                                      C,cgsnow,csice,cswat,rhowat,snmin,snmin,&
...
SUBROUTINE GWsoilfreeze(dels, soil, ssnow)
   ...
   INTEGER k
   REAL(r_2),DIMENSION(mp,ms) :: max_ice_fraction,iceF  !Decker and Zeng 2009
   ...
   where (ssnow%tgg .le. C%TFRZ)
      max_ice_fraction(:,:) = (1. - exp(-2.*(ssnow%wb(:,:)/soil%ssat_vec(:,:))**4.0 *&
                       (ssnow%tgg(:,:)-C%TFRZ)))/exp(1. - ssnow%wb(:,:)/soil%ssat_vec(:,:))
      max_ice_fraction(:,:) = max(0.4,max_ice_fraction(:,:))
   elsewhere (ssnow%tgg .gt. C%TFRZ)
      max_ice_fraction(:,:) = 0.
   endwhere

   !allow more freezing for permenant glacier ice regions
   where ( spread(soil%isoilm,2,ms) .eq. 9 ) max_ice_fraction(:,:) = 0.95
   ...
   DO k = 1, ms
      ...
      WHERE (ssnow%tgg(:,k) .lt. C%TFRZ .and. max_ice_fraction(:,k) * ssnow%wb(:,k) - ssnow%wbice(:,k) > .001)
         
         sicefreeze = MIN( MAX( 0.0_r_2, ( max_ice_fraction(:,k) * ssnow%wb(:,k) -      &
         ...
         ssnow%wbice(:,k) = MIN( ssnow%wbice(:,k) + sicefreeze / (soil%zse(k)  &
                            * 1000.0), max_ice_fraction(:,k) * ssnow%wb(:,k) )
...
SUBROUTINE remove_transGW(dels, soil, ssnow, canopy, veg)
   ...
   IF (trim(cable_user%FWSOIL_switch) .ne. 'Haverd2013') THEN
...
END SUBROUTINE calc_soil_hydraulic_props
  ...
  SUBROUTINE pore_space_relative_humidity(ssnow,soil,veg)
    ...
    unsat_smp = sign(soil%sucs_vec(:,1),-1.0) * ( (unsat_wb-soil%watr(:,1))/(soil%ssat_vec(:,1)-soil%watr(:,1)) ) ** (-soil%bch_vec(:,1))
  ...
  SUBROUTINE sli_hydrology(dels,ssnow,soil,veg,canopy,run_hydro)
    ...
    CALL point2constants( C )  !not sure if done otherwise
