...
USE cable_gw_hydro, only : saturated_fraction
USE cable_data
implicit none
...
SUBROUTINE or_soil_evap_resistance(soil,air,met,canopy,ssnow,veg,rough,snow_covered,dz_litter)
   ...
   !else
   !ensure we calc the sat fraction
   if (first_call .or. .not.cable_user%gw_model) call saturated_fraction(ssnow,soil)
   ...
   !for sensible heat across sublayer 
   !litter added by VH
