...
MODULE cable_offline_driver_module
  ...
  USE cable_common_module,  ONLY: ktau_gl, kend_gl, knode_gl, cable_user,     &
       ...
       report_version_no, kwidth_gl, gw_params,optimized_vegin
...
contains 
  ...
  subroutine cable_offline_driver(param_values,qle_fluxes,qh_fluxes)

  real(kind=8), dimension(:) :: param_values
  real(kind=8), dimension(:) :: qh_fluxes,qle_fluxes

  integer :: n_params,n_fluxes
  ...
  real(r_2), allocatable :: qle_tmp,qh_tmp
  ! END header
  ...
  n_params = size(param_values,dim=1)

  !set what we need
  gw_params%MaxHorzDrainRate=real(param_values(1))
  gw_params%EfoldHorzDrainRate=real(param_values(2))
  gw_params%hkrz=real(param_values(3))
  gw_params%zdepth=real(param_values(4))

  n_params = n_params - 4

  allocate(optimized_vegin(n_params))

  optimized_vegin(:) = real(param_values(5:size(param_values,dim=1)))

  !canopy hgt(m),leaf angle,lf width(m),lf length(m)
  !rholeaf-vis,nir,therm, rhowood-vis,nir,therm
  !tau- rholeaf-vis,nir,therm, rhowood-vis,nir,therm  (same as above
  !xalbnir
  !woodlai, cansto
  !
       ...
       DO ktau=kstart, kend
          ...
          IF ( .NOT. CASAONLY ) THEN
             ...
             ! Call land surface scheme for this timestep, all grid points:
                 ...
                 if (spinConv) then
                    qh_fluxes(ktau) = real(canopy%fh(1),kind=c_double)
                    qle_fluxes(ktau) = real(canopy%fe(1),kind=c_double)
                 end if
...
end MODULE cable_offline_driver_module
