...
SUBROUTINE or_soil_evap_resistance(soil,air,met,canopy,ssnow,veg,rough,snow_covered,tile_index,dz_litter)
   ...
   integer, intent(in), optional :: snow_covered
   ...
   if (.not.present(tile_index)) then  !called from default cable canopy
      ...
      pore_radius((tile_index)) =0.148/ (1000.0*9.81*abs(soil%smpsat((tile_index),1))/1000.0)  !should replace 0.148 with surface tension, unit coversion, and angle
      ...
      if (first_call) then
         ...
         wb_liq(tile_index) = real(max(1.0e-7,min(pi/4.0, (ssnow%wb(tile_index,1)-ssnow%wbice(tile_index,1) -ssnow%satfrac(tile_index)*soil%watsat(tile_index,1))/(1._r_2 - ssnow%satfrac(tile_index)) ) ) )
      end if
   
      rel_s(tile_index) = real( max(wb_liq(tile_index)-soil%watr(tile_index,1),0._r_2)/(soil%watsat(tile_index,1)-soil%watr(tile_index,1)) )
      hk_zero(tile_index) = max(0.001*soil%hksat(tile_index,1)*(min(max(rel_s(tile_index),0.001_r_2),1._r_2)**(2._r_2*soil%clappB(tile_index,1)+3._r_2)),1e-8)
      hk_zero_sat(tile_index) = max(0.001*soil%hksat(tile_index,1),1e-8)
   
      soil_moisture_mod(tile_index)     = 1.0/pi/sqrt(wb_liq(tile_index))* ( sqrt(pi/(4.0*wb_liq(tile_index)))-1.0)
      soil_moisture_mod_sat(tile_index) = 1.0/pi/sqrt(soil%watsat(tile_index,1))* (sqrt(pi/(4.0*soil%watsat(tile_index,1)))-1.0)
      ...
      if (present(snow_covered)) then
         ...
         ssnow%rtevap_unsat(tile_index) = min(rough%z0soil(tile_index)/max(canopy%sublayer_dz(tile_index),1e-5)* ((canopy%sublayer_dz(tile_index) + pore_size(tile_index) * soil_moisture_mod(tile_index)) / Dff),&
                            rtevap_max )
         ssnow%rtevap_sat(tile_index)  =min(rough%z0soil(tile_index)/max(canopy%sublayer_dz(tile_index),1e-5)* ((canopy%sublayer_dz(tile_index) + pore_size(tile_index) * soil_moisture_mod_sat(tile_index))/ Dff),&
                            rtevap_max )
