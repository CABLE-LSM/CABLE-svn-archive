...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!   
  ...
  IMPLICIT NONE
   ...
   do i=1,mp
      ...
      if (satfrac(i) .lt. 0.99_r_2) then
         tmpb = (tmpa - satfrac(i))/(1._r_2 - satfrac(i))
      else
         tmpb = (tmpa - satfrac(i)) / 0.001_r_2
      end if
      tmpb = max(0._r_2,tmpb)
      tmpa = -2._r_2*soil%clappB(i,1)*soil%smpsat(i,1)/dzmm
      qinmax = (1._r_2 + tmpa*(tmpb-1._r_2))*soil%hksat(i,1)

      if (ssnow%fwtop(i) .gt. qinmax) then
         !ssnow%rnof1(i) =  satfrac(i) * ssnow%fwtop(i)  + &
         ssnow%qsrf_gen(i) =  satfrac(i) * ssnow%fwtop(i)  + &
                           (1._r_2-satfrac(i)) * (ssnow%fwtop(i)-qinmax)  !in mm/s
      else
         !ssnow%rnof1(i) = satfrac(i) * ssnow%fwtop(i)  !in mm/s
         ssnow%qsrf_gen(i) = satfrac(i) * ssnow%fwtop(i)  !in mm/s
      end if

      ssnow%fwtop(i) = ssnow%fwtop(i) - ssnow%qsrf_gen(i) !ssnow%rnof1(i)
   ...
   IF (nglacier == 2) THEN
      ...
      ssnow%qsrf_gen = ssnow%qsrf_gen + rnof5/dels
   END IF
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!  
  ...
  IMPLICIT NONE
    ...
    do i=1,mp
       ...
       !where (s_mid .lt. 0.001_r_2) s_mid = 0.001_r_2
       s_mid(i) = min(max(s_mid(i),0.001_r_2),1._r_2)
    ...
    do k=1,ms
       do i=1,mp
          ssnow%wbliq(i,k) = ssnow%wbliq(i,k) + rt(i,k)! - qhlev(i,1:ms)*dels/dzmm   !volutermic liquid
       end do
    end do
    do i=1,mp
...
SUBROUTINE soil_snow_gw(dels, soil, ssnow, canopy, met, bal, veg)
   ...
   call subgrid_sm_transfer(dels,ktau,ssnow,soil)    !inter tile surf. and sub flow
...
SUBROUTINE subgrid_sm_transfer(dels,ktau,ssnow,soil)
  ...
  !landpt(ee)%cstart:landpt(ee)%cend
  ! note: mland=number of gridcells
  ! mp = total number of tiles
  ...
  do i=1,mland
    ...
    do ii=ib+1,ie
      ...
      def   = sum((soil%watsat(ii,:)-ssnow%wbliq(ii,:))*soil%zse(:))*1000.0
      def   = def + (soil%GWwatsat(ii) - ssnow%GWwb(ii))*soil%GWdz(ii)*1000.0
      
      q_tot(ii) = ani*soil%hksat(ii,ms)*sin(slope)* (exp(-f*sum(soil%zse)*cos(slope)) - exp(-f*def*cos(slope)))*soil%area(ii)
      
    end do
    
    ii = ib
    slope = (soil%elevation(ii+1) - soil%elevation(ii)) / (soil%distance(ii+1)-soil%distance(ii))
    def   = sum(max(soil%watsat(ii,:)-ssnow%wbliq(ii,:),0.)*soil%zse(:))*1000.0
    def   = def + (soil%GWwatsat(ii) - ssnow%GWwb(ii))*soil%GWdz(ii)*1000.0
    q_tot(ii) = ani*soil%hksat(ii,ms)*sin(slope)* (exp(-f*sum(soil%zse)*cos(slope)) - exp(-f*def*cos(slope)))*soil%area(ii)
  ...
  do i=1,mland
    ...
    do ii=ib,ie
      wb_avg(ii) =max( 1e-6, &
                     sum( max(ssnow%wbliq(ii,:) - soil%sfc(ii),0.)*soil%zse(:)*1000.0*ssnow%fracice(ii,:) ) + &
                        (ssnow%GWwb(ii) - soil%sfc(ii))*soil%GWdz(ii)*1000.0*ssnow%fracice(ii,ms)    )
    end do
  end do
  ...
  do i=1,mland
    ...
    ie = landpt(i)%cend
    do ii=ib,ie
      q_lev(ii,1:ms) = q_tot(ii) * max(ssnow%wb(ii,:) - soil%sfc(ii),0.)*soil%zse(:)*1000.0*ssnow%fracice(ii,:) / wb_avg(ii)
      q_lev(ii,ms+1) = q_tot(ii) * max(ssnow%GWwb(ii) - soil%sfc(ii),0.)*soil%GWdz(ii)*1000.0*ssnow%fracice(ii,ms) / wb_avg(ii)
    end do
  end do
  ...
  do k=1,ms
    do i=1,mland
      ...
      do ii=ib,ie
        if (ii .lt. ie) then
          ssnow%wb(ii,k) = ssnow%wb(ii,k) + (q_lev(ii+1,k) - q_lev(ii,k))*dels / (soil%zse(k)*1000.0) / soil%area(ii)   !mm^3
        else
          ssnow%wb(ii,k) = ssnow%wb(ii,k) - q_lev(ii,k)*dels / (soil%zse(k)*1000.0) / soil%area(ii)
        end if
      end do  !ib,ie
    end do  !mland
  end do  !ms
  ...
  do i=1,mland
    ...
    ssnow%rnof2(ib) = sum(q_lev(ib,:))/soil%area(ib)
    ssnow%rnof2(ib+1:ie) = 0.0
  end do
