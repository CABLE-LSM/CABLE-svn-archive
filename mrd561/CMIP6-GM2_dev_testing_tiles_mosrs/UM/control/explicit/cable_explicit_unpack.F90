...
use arraydiag_m
  ...
  call cable_fprintf( cDiag2, vname, rad_trad, mp, L_fprint )
   ...
   !write(6,*) 'rad_trad max ',maxval(rad_trad)
   !write(6,*) 'rad_trad min ',minval(rad_trad)

   !write(6,*) 'rough_z0m max ',maxval(rough_z0m)
   !write(6,*) 'rough_z0m min ',minval(rough_z0m)

   !write(6,*) 'canopy_us max ',maxval(canopy_us)
   !write(6,*) 'canopy_us min ',minval(canopy_us)

   !write(6,*) 'canopy_rnet max ',maxval(canopy_rnet)
   !write(6,*) 'canopy_rnet min ',minval(canopy_rnet)

   !write(6,*) 'canopy_fh max ',maxval(canopy_fh)
   !write(6,*) 'canopy_fh min ',minval(canopy_fh)

   !write(6,*) 'canopy_fe max ',maxval(canopy_fe)
   !write(6,*) 'canopy_fe min ',minval(canopy_fe)

   !write(6,*) 'canopy_epot max ',maxval(canopy_epot)
   !write(6,*) 'canopy_epot min ',minval(canopy_epot)
