      ...
      IF( first_call ) THEN 
         ...
         do k=1,ms
             !should read texture by layer evantually
              ...
              soil%Fsilt(:,k) = soil%silt(:)
         end do

         do k=1,ms
            do i=1,mp

               if ( (soil%Fsilt(i,k) .gt. 0.99) .or. &
                    (soil%Fsilt(i,k) .lt. 0.01) .or. &
                    (soil%Fsand(i,k) .gt. 0.99) .or. &
                    (soil%Fsand(i,k) .lt. 0.01) .or. &
                    (soil%Fclay(i,k) .gt. 0.99) .or. &
                    (soil%Fclay(i,k) .lt. 0.01) ) then
            ...
            do k=1,ms
               do i=1,mp
                  ...
                  if (soil%isoilm(i) .ne. 9) then
                     ...
                     soil%Ktherm_const(i,k,1)   = 1.06 * 0.001 * &
                                                  rho_soil_bulk(i,k)

                     soil%Ktherm_const(i,k,2)   = 2.6 / sqrt(min(1.0,max(0.1,&
                                                  soil%Fclay(i,k)))) !prevent / 0

                     soil%Ktherm_const(i,k,3) = 0.03 + 0.1* &
...
SUBROUTINE init_veg_pars_fr_vegin() 
   ...
   CALL init_veg_from_vegin(1, mp, veg) 
    ...
    !not yet tested
!    if (cable_user%fix_access_roots) then
!       ! calculate vegin%froot from using rootbeta and soil depth
!       ! (Jackson et al. 1996, Oceologica, 108:389-411)
!       totdepth = 0.0
!       do k = 1, ms
!          totdepth = totdepth + soil%zse(k) * 100.0  ! unit id
!          do j=1,mp
!             !only use for non perm ice and actual veg types cable uses
!             if (soil%isoilm(j) .ne. 9 .and. veg%iveg(j) .le. 10) then
!                veg%froot(j,k) = MIN(1.0, 1.0-vegin%rootbeta(veg%iveg(j))**totdepth)
!             end if
!          end do
!       end do
!       do k = ms, 2, -1
!          do j=1,mp
!             veg%froot(j,k) = veg%froot(j,k)-veg%froot(j,k-1)
!          end do
!       end do
!       sum_froot(:) = 0.0
!       do j=1,mp  !normalize tp be 1 if there is an issue
!          do k=1,ms
!             sum_froot(j) = sum_froot(j) + veg%froot(j,k)
!          end do
!       end do
!
!       do j=1,mp
!          if (abs(sum_froot(j)-1.0) .gt. 0.001) then !normalize
!             do k=1,ms
!                veg%froot(j,k) = veg%froot(j,k) / sum_froot(j)
!             end do
!          end if
!       end do 
!
