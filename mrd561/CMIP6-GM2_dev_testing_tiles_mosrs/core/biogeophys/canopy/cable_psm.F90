...
implicit none
   ...
   REAL(r_2), parameter :: rt_Dff=2.5e-5, &  !diffusivity water vapor in air
...
SUBROUTINE or_soil_evap_resistance(soil,air,met,canopy,ssnow,veg,rough)
   ...
   pore_size(:) = pore_radius(:)*sqrt(pi)
      ...
      eddy_shape = 0.3*met%ua/ max(1.0e-4,max(1.0e-3,canopy%us)*exp(-rough%coexp*(1.0-canopy%sublayer_dz/max(1e-2,rough%hruff))))
