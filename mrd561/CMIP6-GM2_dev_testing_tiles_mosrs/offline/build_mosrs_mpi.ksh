...
build_build()
{
   ...
   core_utils="cable_common.F90 cable_define_types.F90 cable_constants.F90 cable_diag.F90  cable_data.F90"
   casa_fils="casa_dimension.F90 casa_phenology.F90 pop_io.F90 cable_phenology.F90 casa_inout.F90 casa_variable.F90 POPLUC.F90 casa_cable.F90 CASAONLY_LUC.F90 pop_mpi.F90 casa_cnp.F90 casa_param.F90 POP.F90 spincasacnp.F90"

   bio_dir="SLI control soilsnow surface canopy"


   for fl in $core_utils; do
      /bin/cp -p $CORE/utils/$fl ./.mpitmp
   done

   for fl in $casa_fils; do
      /bin/cp -p $CASA/$fl ./.mpitmp
   done

   for fl in $bio_dir; do
      /bin/cp -p $BIO/$fl/*F90 ./.mpitmp
   done
   
   /bin/cp -p $BIO/*90 ./.mpitmp
   ...
   /bin/cp -p Makefile_mosrs_mpi  ./.mpitmp
   
  cd .mpitmp/

   make -f Makefile_mosrs_mpi
}
