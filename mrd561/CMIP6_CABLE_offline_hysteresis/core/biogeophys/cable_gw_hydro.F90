...
!
!   POINTER :: brook_corey_swc_smp
!   POINTER :: hutson_cass_swc_smp
  ...
  ! if max_ice_frac always set to frozen_limit and tgg_tmp is always C%TFRZ
   ...
   do i=1,mp
      ...
      elseif ((ssnow%tgg(i,k) .gt. C%TFRZ) .and. &
             ...
             delta_ice_vol(i,k) = max(0._r_2,ssnow%wbice(i,k))
   ...
   DO i=1,mp
      ...
      if (abs(delta_ice_vol(i,k)) .gt. 0._r_2) then
         ...
         if ( (ssnow%tgg(i,k) .lt. C%TFRZ) .and. &
            ...
            delta_wbliq = max(0._r_2,min(ssnow%wbliq(i,k),&
                           delta_ice_vol(i,k)*real(C%density_ice/C%density_liq,r_2)))
...
SUBROUTINE soil_snow_gw(dels, soil, ssnow, canopy, met, bal, veg)
   ...
   IF (gw_params%BC_hysteresis)  &
             CALL swc_hyst_direction(soil,ssnow)
       ...
       do k=1,ms
          ...
          do i=1,mp
             if (soil%isoilm(i) .eq. 9) then
                ...
                hk_ice_factor(i,k) = sqrt(10.0**(gw_params%ice_impedence*ssnow%wbice(i,k)/(ssnow%ssat_hys(i,k)-ssnow%watr_hys(i,k))) *&
                                          10.0**(gw_params%ice_impedence*ssnow%wbice(i,kk)/(ssnow%ssat_hys(i,kk)-ssnow%watr_hys(i,kk))))
             end if
          end do
       end do
    ...
    do i=1,mp
       if (veg%iveg(i) .lt. 16 .and. soil%isoilm(i) .ne. 9 .and. &
          ...
          unsat_smp(i) = sign(ssnow%sucs_hys(i,1),-1.0) * min(1.0, &
                         (max(0.001, (unsat_wb(i)-ssnow%watr_hys(i,1))/(ssnow%ssat_hys(i,1)-&
   ...
   real(r_2) :: tmp_smp
    ...
    do k=1,ms 
       do i=1,mp
          ...
          ssnow%dsmpdw(i,k) = -soil%bch_vec(i,k)*ssnow%smp(i,k)/&
                    (max(s_mid(i)*(ssnow%ssat_hys(i,k)-ssnow%watr_hys(i,k)),0.01_r_2)) 
       end do   
    end do
...
subroutine swc_hyst_direction(soil,ssnow)
   type(soil_parameter_type), intent(inout) :: soil
   type(soil_snow_type),      intent(inout) :: ssnow
   ...
   delta_wbliq = ssnow%wbliq - ssnow%wbliq_old
    ...
    do k=1,ms
       do i=1,mp
          if (delta_wbliq(i,k) .gt. 0.0 .and. ssnow%hys_fac(i,k) .gt. 0.75.and.ssnow%smp(i,k).lt.-0.5*soil%sucs_vec(i,k)) then  !avoid testing .eq. 1.0
           !layer was drying now it is wetting!
              ...
              ssnow%sucs_hys(i,k) = ssnow%hys_fac(i,k)*soil%sucs_vec(i,k)
          elseif (delta_wbliq(i,k) .le. 0.0 .and. ssnow%hys_fac(i,k) .lt. 0.75 .and. ssnow%smp(i,k).lt.-soil%sucs_vec(i,k)) then
