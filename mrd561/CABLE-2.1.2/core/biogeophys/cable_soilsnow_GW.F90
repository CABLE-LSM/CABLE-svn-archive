...
SUBROUTINE soilfreeze(dels, soil, ssnow)
   ...
   DO k = 1, ms
      ...
      ice_mass = ssnow%wbice(:,k)*real(soil%zse(k)*denice,r_2)
      liq_mass = ssnow%wbliq(:,k)*real(soil%zse(k)*denliq,r_2)
      ...
      WHERE (ssnow%tgg(:,k) < C%TFRZ &
         ...
         sicefreeze = MIN( MAX( 0.0_r_2, ( frozen_limit * ssnow%wb(:,k) -      &
                      ssnow%wbice(:,k) ) ) * soil%zse(k) * denice,             &
      ...
      ssnow%wbliq(:,k) = liq_mass / real(soil%zse(k)*denliq,r_2)
      ssnow%wbice(:,k) = ice_mass / real(soil%zse(k)*denice,r_2)
   ...
   DO k = 1,ms
      ...
      WHERE (canopy%fevc > 0._r_2 )     ! convert to mm/dels
         ...
         diff(:,k) = MAX( 0._r_2, ssnow%wbliq(:,k) - soil%swilt) &      ! m3/m3  
                     * real(soil%zse(k)*denliq,r_2)
         xxd = xx - diff(:,k)
       
         WHERE ( xxd .GT. 0._r_2 )
            ssnow%wbliq(:,k) = ssnow%wbliq(:,k) - diff(:,k)/real(soil%zse(k)*denliq,r_2)  !volume
            diff(:,k) = xxd
         ELSEWHERE
            ssnow%wbliq(:,k) = ssnow%wbliq(:,k) - xx/real(soil%zse(k)*denliq,r_2)  !volume
            diff(:,k) = 0._r_2
         END WHERE

         ssnow%wmliq(:,k) = ssnow%wbliq(:,k)*real(soil%zse(k)*denliq,r_2)  !mass
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!   
  !-------------------------------------------------------------------------
  SUBROUTINE ovrlndflx (dels, ssnow, soil,md_prin )
USE cable_common_module
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!  
  ...
  ! water table depth
  !  
  SUBROUTINE calcwtd (ssnow, soil, veg, md_prin)
  IMPLICIT NONE
     ...
     do i=1,mp
       ...
       morethan_loop: DO while (jlp .le. wtd_iter_max)
          ...
          if (ssnow%wtd(i) .lt. zimm(ms)) then
           ...
           !deffunc = soil%watsat(i,ms)*ssnow%wtd(i) + tempa*tempb 
       ...
       !end if
           ...
           !tempa = soil%watsat(i,ms)*Nsmpsat(i)**invB(i)
           !tempb = (Nsmpsat(i) + ssnow%wtd(i) - zimm(ms))**(-invB(i))
           !derv  = tempa*tempb

           !write(*,*) 'derv  .gt. '
           !write(*,*) tempa,tempb,derv
           ...
           !tempb = soil%watsat(i,ms)*Nsmpsat(i)**(invB(i)) / (1._r_2-invB(i))
           !deffunc = soil%watsat(i,ms)*zimm(ms) + tempa*tempb

           !write(*,*) 'func  .gt. '
           !write(*,*) tempa,tempb,deffunc
           !write(*,*) deffunc/derv
  ...
  !
  SUBROUTINE smoistgw (dels,ssnow,soil,md_prin)
  USE cable_common_module
  ...
  IMPLICIT NONE
    ...
    masswatmin(:,ms+1) = volwatmin * real(denliq,r_2) * soil%GWdz
...
SUBROUTINE soil_snow_gw(dels, soil, ssnow, canopy, met, bal, veg)
   ...
   wbtot_ic  = sum(ssnow%wbliq(:,:)*denliq*spread(soil%zse,1,mp),2) + &
               sum(ssnow%wbice(:,:)*denice*spread(soil%zse,1,mp),2) + &
               ssnow%GWwb(:)*soil%GWdz*denliq
   ...
   CALL calcwtd (ssnow, soil, veg, md_prin)                  !update the wtd

   CALL ovrlndflx (dels, ssnow, soil, md_prin )         !surface runoff, incorporate ssnow%pudsto?
   ...
   CALL smoistgw (dels,ssnow,soil,md_prin)               !vertical soil moisture movement. 
   ...
   ssnow%wbtot = sum(ssnow%wbliq(:,:)*real(denliq,r_2)*real(spread(soil%zse,1,mp),r_2),2) + &
                 sum(ssnow%wbice(:,:)*real(denice,r_2)*real(spread(soil%zse,1,mp),r_2),2) + &
                 ssnow%GWwb(:)*soil%GWdz*real(denliq,r_2)
...
SUBROUTINE calc_srf_wet_fraction(ssnow,soil,veg)
  ...
  IMPLICIT NONE
    ...
    INTEGER, SAVE :: ktau_srf_frac = 0

    ktau_srf_frac = ktau_srf_frac + 1
    !if first time step we don't yet have the water table depth so calc it
    if (ktau_srf_frac .le. 1) CALL calcwtd(ssnow, soil, veg, .false.)                  !update the wtd
    ...
    xxx(:) = max(1.e-6, min(1., real((ssnow%wbliq(:,1)-0.5*soil%swilt(:))/&
