...
!    (BP may2010)
    ...
    DO e = 1, mland ! over all land grid points
      ...
      DO klev=1,ms
        ...
        soil%watr(landpt(e)%cstart:landpt(e)%cend,klev) =  0.1 *              &
             inswilt(landpt(e)%ilon, landpt(e)%ilat)
      END DO
      ...
      soil%GWwatr(landpt(e)%cstart:landpt(e)%cend) =  0.1 *                   &
         inswilt(landpt(e)%ilon, landpt(e)%ilat)
       ...
       DO h = landpt(e)%cstart, landpt(e)%cend ! over each patch in current grid
          ...
          IF (.NOT. soilparmnew) THEN   ! Q,Zhang @ 12/20/2010
            ...
            do klev=1,ms
              ...
              soil%watr(h,klev)    = 0.1*soil%swilt(h)!soilin%hyds(soil%isoilm(h))
            end do
            ...
            soil%GWwatr(h)    = 0.1*soil%swilt(h)!soilin%hyds(soil%isoilm(h))
