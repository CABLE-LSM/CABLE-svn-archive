...
host_ccrc()
{
   ...
   export FC=ifort
   export CFLAGS='-O3 -shared-intel -mcmodel=medium -xhost -parallel -ftrapuv  -fpmodel precise -fpmodel except'   #-traceback
   if [[ $1 = 'debug' ]]; then
...
host_squa()
{
   ...
   export CFLAGS='-O2 -shared-intel -mcmodel=medium -xavx -parallel -ftrapuv  -fp-model precise'   #-traceback
   if [[ $1 = 'debug' ]]; then
      export CFLAGS='-O0 -traceback -g -fp-model precise -ftz -fpe0 -shared-intel -mcmodel=medium' 
   fi
   export LD='-lnetcdf -lnetcdff'
