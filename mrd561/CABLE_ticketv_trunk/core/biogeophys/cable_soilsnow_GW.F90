...
MODULE cable_soil_snow_gw_module
   ...
   REAL(r_2), PARAMETER :: sucmin  = -1.0e8, &! minimum soil pressure head [mm]
                      ...
                      wtd_max      = 50000.0,    &! maximum wtd [mm]
   ...
   PUBLIC iterative_wtd,ovrlndflx,subsurface_drainage,saturated_fraction,aquifer_recharge

   PRIVATE snowdensity, snow_melting, snowcheck, snowl_adjust ,snow_accum, stempv,trimb
   PRIVATE calc_equilibrium_water_content
   PRIVATE GWsoilfreeze, remove_trans,simple_wtd
   PRIVATE smoistgw
...
SUBROUTINE GWstempv(dels, canopy, ssnow, soil)
   ...
   DO k = 1, ms
      ...
      DO j = 1, mp
         ...
         ELSE  !test alt formation for thermal conductivity
            ew(j) =min(1.0_r_2, (ssnow%wmliq(j,k)/rhowat + ssnow%wmice(j,k)/(rhowat*dri))/(1000.*soil%zse(k)*soil%ssat_vec(j,k)))
            if (ew(j) .gt. 1.0e-7) then
               ...
               if (ssnow%tgg(j,k) .gt. 273.15) then
                  ...
                  tmpsat = tkmg(j,k)*0.57_r_2**soil%ssat_vec(j,k)
                else                               ! Frozen soil
                  tmp =  ew(j)
                  tmpsat = tkmg(j,k)*0.249_r_2**(liq_frac*soil%ssat_vec(k,j))*2.29_r_2**soil%ssat_vec(k,j)
                endif
   ...
   WHERE( ssnow%isflag == 0 )
      ...
      ssnow%gammzz(:,k) = MAX( ( 1.0 - soil%ssat_vec(:,k) ) * soil%css * soil%rhosoil   &
                          + soil%ssat_vec(:,k) * ( wblfsp * cswat * rhowat +            &
   ...
   DO k = 1, ms
      ...
      WHERE( ssnow%isflag /= 0 )
         ...
         ssnow%gammzz(:,k) = MAX( ( 1.0 - soil%ssat_vec(:,k) ) * soil%css *             &
                             soil%rhosoil + soil%ssat_vec(:,k) * ( wblfsp * cswat *     &
...
!            ccnsw(j,k) = snow_ccnsw
!         ELSE
!!            ew(j) = ssnow%wblf(j,k) * soil%ssat_vec(j,k)
!!            exp_arg = ( ew(j) * LOG( 60.0 ) ) + ( ssnow%wbfice(j,k)            &
!!                      * soil%ssat_vec(j,k) * LOG( 250.0 ) )
!!
...
!        
!
!            ew(j) = log10(soil%sucs_vec(j,k)/10.0 * (ssnow%wb(j,k)/soil%ssat_vec(j,k))**(-soil%bch_vec(j,k)))
!            if (ew(j) .lt. 5.1) then
!               ccnsw(j,k) = exp(-(ew(j)+2.71)) * 10.0*4.18
!            else
!               ccnsw(j,k) = 0.1714
!            end if
!
...
!!            write(wlogn,*) ccnsw(i,k-1),ccnsw(i,k),ssnow%wblf(i,k),i
!!            call flush(wlogn)
!!         end if
!!     end do
!      WHERE (ssnow%isflag == 0)
...
SUBROUTINE GWsoilfreeze(dels, soil, ssnow)
   ...
   where (ssnow%tgg .le. C%TFRZ)
      frozen_limit(:,:) = (1. - exp(-2.*(ssnow%wb(:,:)/soil%ssat_vec(:,:))**4.0 *&
                       (ssnow%tgg(:,:)-273.16)))/exp(1. - ssnow%wb(:,:)/soil%ssat_vec(:,:))
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!   
  !-------------------------------------------------------------------------
  SUBROUTINE ovrlndflx (dels, ktau, ssnow, soil,veg, from_sli )
  USE cable_common_module
  ...
  IMPLICIT NONE
    ...
    LOGICAL, INTENT(IN)                      :: from_sli  !called from sli? ignore glacier for now
  ...
  endwhere
   ...
   IF (nglacier == 2 .and. .not.from_sli) THEN
      smelt1=0.
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!  
  ...
  ! water table depth
  !  
  SUBROUTINE iterative_wtd (ssnow, soil, veg, ktau, include_gw)
  IMPLICIT NONE
  TYPE (soil_snow_type), INTENT(INOUT)      :: ssnow ! soil and snow variables
  TYPE (soil_parameter_type), INTENT(IN)    :: soil  ! soil parameters
  TYPE (veg_parameter_type), INTENT(IN)     :: veg
  INTEGER, INTENT(IN)                       :: ktau  ! integration step number
  LOGICAL, INTENT(IN)                       :: include_gw  !include GW in the calc of wtd?
  ...
  do k=1,ms
       def(:) = def(:) +                                                           &
                max(0._r_2,(soil%ssat_vec(:,k)-(ssnow%wbliq(:,k)+dri*ssnow%wbice(:,k)))*dzmm_mp(:,k))
  end do  !ms

  if (include_gw) then
    def(:) = def(:) + max(0._r_2,soil%GWssat_vec(:)-ssnow%GWwb(:))*soil%GWdz(:)*1000._r_2
  end if
...
!
!         do k=k_drain,ms
!             ssnow%qhlev(i,k) = ssnow%qhz(i)*max(ssnow%wbliq(i,k)-soil%watr(i,k),0._r_2)/sm_tot(i)*ssnow%hk(i,k)!*dzmm(k)/sm_tot(i)
!         end do
!
!       else
...
!    end do
    ...
    CALL aquifer_recharge(dels,ssnow,soil,veg,zaq,zmm)
...
!   write(wlogn,*) 'before stempv max soil slope is ',maxval(soil%slope)
!   write(wlogn,*) 'before stempv min soil slope is ',minval(soil%slope)
!   write(wlogn,*) 'before stempv max soil slope_std is ',maxval(soil%slope_std)
!   write(wlogn,*) 'before stempv min soil slope_std is ',minval(soil%slope_std)
!
!   write(wlogn,*) 'before stempv max soil temp is ',maxval(ssnow%tgg(:,1))
!   write(wlogn,*) 'before stempv max ground heat is ',maxval(canopy%ga)
!   write(wlogn,*) 'before stempv max canopy temp is ',maxval(canopy%tv)
!   write(wlogn,*) 'before stempv min ground heat is ',minval(canopy%ga)
!   i = maxloc(ssnow%tgg(:,1),dim=1)
!   k = maxloc(canopy%ga,dim=1)
!   if (i .ne. k) then
!       write(wlogn,*) 'before - max location of tgg not equal to max ga'
!       write(wlogn,*) 'before - location ga',k
!    end if
!   k = minloc(canopy%ga,dim=1)
!   if (i .ne. k) then
!       write(wlogn,*) 'before - max location of tgg not equal to min ga'
!       write(wlogn,*) 'before - min location ga',k
!    end if
!    write(wlogn,*) 'before - location tgg',i
!    write(wlogn,*) 'before - sh i,',canopy%fhs(i)
!    write(wlogn,*) 'before - lh i,',canopy%fess(i)
!    write(wlogn,*) 'before - fevc i,',canopy%fevc(i)
!    write(wlogn,*) 'before - sh k,',canopy%fhs(k)
!    write(wlogn,*) 'before - lh k,',canopy%fess(k)
!    write(wlogn,*) 'before - fevc k,',canopy%fevc(k)
        
   CALL stempv(dels, canopy, ssnow, soil)
!   write(wlogn,*) 'after stempv max soil temp is ',maxval(ssnow%tgg(:,1))
!   write(wlogn,*) 'after stempv max ground heat is ',maxval(canopy%ga)
!   write(wlogn,*) 'after stempv max canopy temp is ',maxval(canopy%tv)
!   i = maxloc(ssnow%tgg(:,1),dim=1)
!   k = maxloc(canopy%ga,dim=1)
!   if (i .ne. k) then
!       write(wlogn,*) 'after - max location of tgg not equal to max ga'
!       write(wlogn,*) 'after - location ga',k
!    end if
!   k = minloc(canopy%ga,dim=1)
!   if (i .ne. k) then
!       write(wlogn,*) 'after - max location of tgg not equal to min ga'
!       write(wlogn,*) 'after - min location ga',k
!    end if
!    write(wlogn,*) 'after - location tgg',i
!    write(wlogn,*) 'after - sh i,',canopy%fhs(i)
!    write(wlogn,*) 'after - lh i,',canopy%fess(i)
!    write(wlogn,*) 'after - fevc i,',canopy%fevc(i)
!    write(wlogn,*) 'after - sh k,',canopy%fhs(k)
!    write(wlogn,*) 'after - lh k,',canopy%fess(k)
!    write(wlogn,*) 'after - fevc k,',canopy%fevc(k)
!   !CALL GWstempv(dels, canopy, ssnow, soil)
!    write(wlogn,*) 'after - wb i', ssnow%wb(i,:)
!    write(wlogn,*) 'after - wb k', ssnow%wb(k,:)
   ...
   CALL iterative_wtd (ssnow, soil, veg, ktau, .true.)  
   !CALL simple_wtd(ssnow, soil, veg, ktau, md_prin)

   CALL ovrlndflx (dels, ktau, ssnow, soil, veg, .false. )         !surface runoff, incorporate ssnow%pudsto?
  ...
  IF (cable_user%FWSOIL_switch.ne.'Haverd2013') THEN
     ...
     DO k = 1,ms
        ...
        WHERE (canopy%fevc > 0.0 )     ! convert to mm/dels
           ...
           diff(:,k) = MAX( 0.0_r_2, ssnow%wbliq(:,k) - soil%swilt_vec(:,k)) &      ! m3/m3
                * soil%zse(k)*1000.0
           xxd = xx - diff(:,k)
...
END SUBROUTINE remove_trans
  ...
  SUBROUTINE aquifer_recharge(dels,ssnow,soil,veg,zaq,zmm)
  USE cable_common_module
  ...
  IMPLICIT NONE
    ...
    !Doing the recharge outside of the soln of Richards Equation makes it easier to track total recharge amount.
    !Add to ssnow at some point 
    !this is closer to what was in paper though removed from Soln to Richards
    !Eqn

    do i=1,mp
       if ((veg%iveg(i) .ge. 16) .or. (soil%isoilm(i) .eq. 9))  then
          ssnow%Qrecharge(i) = 0._r_2
       else
          ssnow%Qrecharge(i) = -ssnow%hk(i,ms)*((ssnow%GWsmp(i)-ssnow%smp(i,ms)) - (ssnow%GWzq(i)-ssnow%zq(i,ms)))/(zaq(i) - zmm(ms))
       end if
