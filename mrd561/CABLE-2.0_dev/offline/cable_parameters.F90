...
!    (BP may2010)
    ...
    DO e = 1, mland ! over all land grid points
      ...
      DO klev=1,ms
        ...
        soil%hksat(landpt(e)%cstart:landpt(e)%cend,k) =                         &
                                         inhyds(landpt(e)%ilon, landpt(e)%ilat)/1000.0  !convert to mm                         
                                         
        soil%clappB(landpt(e)%cstart:landpt(e)%cend,k) =                         &
                                         inbch(landpt(e)%ilon, landpt(e)%ilat)                       
                                         
        soil%Fclay(landpt(e)%cstart:landpt(e)%cend,k) =                         &
                                         inclay(landpt(e)%ilon, landpt(e)%ilat)                      
                                         
        soil%Fsand(landpt(e)%cstart:landpt(e)%cend,k) =                         &
                                         insand(landpt(e)%ilon, landpt(e)%ilat)                       
                                         
        soil%densoil(landpt(e)%cstart:landpt(e)%cend,k) =                         &
                                         inrhosoil(landpt(e)%ilon, landpt(e)%ilat)                      
                                         
        soil%watsat(landpt(e)%cstart:landpt(e)%cend,k) =                         &
                                         inssat(landpt(e)%ilon, landpt(e)%ilat) 
                                         
        soil%watr(landpt(e)%cstart:landpt(e)%cend,k) =  0.01  !use a simple constant for now
      END DO
      !Aquifer properties
