...
PROGRAM cable_offline_driver
  ...
  INTEGER    :: &
    ...
    mloop,      &  ! no. of loops for CASA-CNP spinup
    ii, iday       ! counters for CASA-CNP spinup
  ! endday         ! no. of days for CASA-CNP spinup (from casa_spin_mod)
  ...
  LOGICAL, SAVE              :: &
    vegparmnew  = .TRUE.,       & ! using new format input file (BP dec 2007)
    spinup = .FALSE.,           & ! model spinup to soil state equilibrium?
    spinConv = .FALSE.,         & ! has spinup converged?
    spincasainput = .FALSE.,    & ! old switch to be scraped
    spincasa = .FALSE.,         & ! TRUE: CASA-CNP Will spin mloop times,
                                  ! FALSE: no spin up
    l_casacnp = .FALSE.,        & ! using CASA-CNP with CABLE
    l_laiFeedbk = .FALSE.,      & ! using prognostic LAI
    l_vcmaxFeedbk = .FALSE.       ! using prognostic Vcmax
  ! verbose     = .TRUE.          ! preset switch in cable_IO_vars_module
  ! soilparmnew = .TRUE.          ! preset switch in cable_IO_vars_module
  ! cable_user%CASA_DUMP_READ  = .FALSE., ! preset switch in cable_common_module
  ! cable_user%CASA_DUMP_WRITE = .FALSE., ! preset switch in cable_common_module
  ...
  NAMELIST/CABLE/                  &
                 ...
                 vegparmnew,       & ! jhan: use new veg  param. format
                 soilparmnew,      & ! jhan: use new soil param. format
  ...
  IF (spincasa) THEN     ! assume CABLE has spinned up GPP to steady-state
    IF (.NOT. l_casacnp) STOP 'CASA-CNP required for spin up'
    IF (mloop <= 0) STOP 'cannot perform spincasa when mloop <= 0'
    spinConv = .TRUE.    ! no need to further spin up CABLE
  ELSE
    IF (spinup) THEN
      spinConv = .FALSE.
    ELSE
  ...
  ! constants used for CASA-CNP
  ktauday=int(24.0*3600.0/dels)
  nyear =INT((kend-kstart+1)/(365*ktauday))

  ! spinup CASA-CNP 
  IF (spincasa) THEN
    CALL open_casa_dump( casafile%cnpspin )
    CALL spin_init
    DO ii = 0, mloop
      DO iday = 1, endday
        ktau = iday * ktauday
        CALL read_casa_dump( casamet, casaflux, iday )
        CALL biogeochem
        IF (ii == 0) CALL spin_update( casaflux, casapool )
      ENDDO
      IF (ii == 0) THEN
        CALL spin_averaging
      ...
      ENDIF
    ENDDO
    CALL close_dump( casafile%dump_cnpspin )
  ENDIF
  IF (cable_user%CASA_DUMP_WRITE) THEN
    endday = kend/ktauday
    CALL open_out_casa_dump( casafile%dump_cnpspin )
  ENDIF
...
!      IF (ktau_gl <= 1) THEN
!        DO ii = 1, mp
!          IF (ssnow%isflag(ii) == 1) THEN
!            ssnow%tggsn(ii,1) = (ssnow%tggsn(ii,1) + met%tk(ii)) / 2.0
!          ELSE
!            ssnow%tgg(ii,1) = (ssnow%tgg(ii,1) + met%tk(ii)) / 2.0
!          ENDIF
!        ENDDO
!      ENDIF
!      ! BP jun2012 end
      ...
      IF(icycle >0) THEN
!        IF ((.NOT. spincasa .AND. GPPconv .AND. spinConv)                  &
!            .OR. (spincasa .AND. GPPconv)) cable_user%CASA_dump_write = .TRUE.
      ...
      IF(.NOT. spinup .OR. (spinup .AND. spinConv)) THEN
        CALL write_output( dels, met, canopy, ssnow,                     &
                           ...
                           C%EMLEAF, C%EMSOIL )
        IF (cable_user%CASA_DUMP_WRITE .AND. MOD(ktau,ktauday)==0)       &
          CALL write_casa_dump( casamet, casaflux, ktau/ktauday)
      ENDIF
  ...
  END DO  ! outer loop: ktau_tot

  IF (icycle > 0) THEN
    ! Write pool size file for restarting CASA-CNP
  ...
  IF (cable_user%CASA_DUMP_WRITE) CALL close_dump( casafile%dump_cnpspin )
