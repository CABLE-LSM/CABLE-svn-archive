...
CONTAINS
  ...
  ! MPI: master only; necessary info will be received by MPI below
  !CALL load_parameters(met,air,ssnow,veg,bgc,soil,canopy, &
  ...
  DO
    ...
    DO ktau = kstart, kend
      ...
      ! CALL get_met_data(spinup,spinConv,ktau,met,soil,                &
      ...
      !       & (ktau,dels,met,canopy,ssnow,rad,bal,air,soil,veg)
  ...
  IF(output%restart) THEN
     ...
     !  soil,veg,ssnow,canopy,rough,rad,bgc,bal)
  END IF
