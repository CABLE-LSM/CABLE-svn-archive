MODULE snow_aging_mod
  ...
  !This routine evaluates the effecive age of the snow pack
  !It is called from soilsnow_main and output SnowAge is used in cbl_snow_albedo
...
CONTAINS
  ...
  SUBROUTINE snow_aging(SnowAge,mp,dels,SnowDepth,SnowODepth,SnowTemp, &
    ...
    !need to eliminate this USE data statements  
    USE cable_phys_constants_mod, ONLY : CTFRZ => TFRZ

    IMPLICIT NONE

    REAL, INTENT(OUT)   :: SnowAge(mp)
    ...
    REAL    :: tmp      !local effective temperauture

    !default to values if no snow
    ar1(:)=0.0
    ar2(:)=0.0
    ar3(:)=0.0
    DO i=1,mp
       IF (SnowDepth(i)>snow_depth_thresh) THEN
          ...
          dnsnow(i) = MIN (1.0, .1 * MAX( 0., SnowDepth(i) - SnowODepth(i) ) )
          ...
          tmp = MIN( tmp, CTFRZ )
          ar1(i) = 5000. * (1. / (CTFRZ-0.01) - 1. / tmp) ! crystal growth  (-ve)
          ar2(i) = 10. * ar1(i) ! freezing of melt water
