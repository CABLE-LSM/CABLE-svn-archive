...
11  format(i2)
  ...
  !!CLN   SUBROUTINE HANDLE_ERR( status )
  !!CLN      use netcdf90
  !!CLN     INTEGER, INTENT(IN) :: status
  !!CLN     IF(status /= NF90_noerr) THEN
  !!CLN        PRINT*, TRIM(NF90_strerror(status))
  !!CLN        STOP "Stopped"
  !!CLN     END IF
  !!CLN   END SUBROUTINE HANDLE_ERR
