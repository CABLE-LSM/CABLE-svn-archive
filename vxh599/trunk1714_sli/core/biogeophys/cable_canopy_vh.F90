...
CONTAINS
  ...
  SUBROUTINE define_canopy_vh(ktau,bal,rad,rough,air,met,dels,ssoil,soil, &
    ...
    elseif (cable_user%soil_struc=='sli') then
       ssoil%tss = real(ssoil%Tsurface) + tfrz
    endif
    tss4      = ssoil%tss**4
    ...
    DO WHILE (iter<3) ! iterate over in-canopy T and q
       ...
       IF (cable_user%soil_struc=='default') THEN
          ...
          ssoil%potev = cc1 * (canopy%fns - canopy%ga) + cc2 * air%rho  &
               * real(air%rlam) * (qsatf((met%tk-tfrz),met%pmb) - met%qv) / ssoil%rtsoil
          ! Soil latent heat:
          ...
          canopy%ga = canopy%fns - canopy%fhs - real(canopy%fes)*ssoil%cls
