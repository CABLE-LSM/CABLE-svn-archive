...
! offline only above
       ...
       DO h = landpt(e)%cstart, landpt(e)%cend ! over each patch in current grid
          ...
          veg%g0c3(h)     = vegin%g0c3(veg%iveg(h)) ! M.De Kauwe, 19/03/2014.
          veg%g0c4(h)     = vegin%g0c4(veg%iveg(h)) ! M.De Kauwe, 19/03/2014.
          veg%g1c3(h)     = vegin%g1c3(veg%iveg(h)) ! M.De Kauwe, 19/03/2014.
          veg%g1c4(h)     = vegin%g1c4(veg%iveg(h)) ! M.De Kauwe, 19/03/2014.
...
!    IF(ASSOCIATED(soiltype_metfile)) DEALLOCATE(soiltype_metfile)
    DEALLOCATE(soilin%silt, soilin%clay, soilin%sand, soilin%swilt,            &
               ...
               vegin%xalbnir, vegin%length, vegin%width, vegin%g0c3,           &
               vegin%g0c4, vegin%g1c3, vegin%g1c4 )
